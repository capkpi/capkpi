...
def start(transaction_type="request", method=None, kwargs=None):
	if capkpi.conf.monitor:
		capkpi.local.monitor = Monitor(transaction_type, method, kwargs)


def stop(response=None):
	if hasattr(capkpi.local, "monitor"):
		capkpi.local.monitor.dump(response)


def log_file():
	return os.path.join(capkpi.utils.get_bench_path(), "logs", "monitor.json.log")
...
class Monitor:
	...
	def collect_request_meta(self):
		self.data.request = capkpi._dict(
			{
				"ip": capkpi.local.request_ip,
				"method": capkpi.request.method,
				"path": capkpi.request.path,
			}
		)

	def collect_job_meta(self, method, kwargs):
		self.data.job = capkpi._dict({"method": method, "scheduled": False, "wait": 0})
		...
		try:
			...
			if self.data.transaction_type == "request":
				...
				if hasattr(capkpi.local, "rate_limiter"):
					limiter = capkpi.local.rate_limiter
	...
	def store(self):
		if capkpi.cache().llen(MONITOR_REDIS_KEY) > MONITOR_MAX_ENTRIES:
			capkpi.cache().ltrim(MONITOR_REDIS_KEY, 1, -1)
		serialized = json.dumps(self.data, sort_keys=True, default=str)
		capkpi.cache().rpush(MONITOR_REDIS_KEY, serialized)
...
def flush():
	try:
		...
		if logs:
			logs = list(map(capkpi.safe_decode, logs))
			with open(log_file(), "a", os.O_NONBLOCK) as f:
				f.write("\n".join(logs))
				f.write("\n")
			# Remove fetched entries from cache
			capkpi.cache().ltrim(MONITOR_REDIS_KEY, len(logs) - 1, -1)
	except Exception:
		traceback.print_exc()
