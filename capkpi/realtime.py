...
def publish_realtime(
	...
	if event is None:
		if getattr(capkpi.local, "task_id", None):
			event = "task_progress"
		else:
			event = "global"
	...
	if not room:
		if not task_id and hasattr(capkpi.local, "task_id"):
			task_id = capkpi.local.task_id
	...
	if after_commit:
		...
		if not params in capkpi.local.realtime_log:
			capkpi.local.realtime_log.append(params)
	else:
...
def emit_via_redis(event, message, room):
	...
	try:
		r.publish("events", capkpi.as_json({"event": event, "message": message, "room": room}))
...
def get_redis_server():
	...
	if not redis_server:
		...
		redis_server = Redis.from_url(capkpi.conf.redis_socketio or "redis://localhost:12311")
	return redis_server
...
def can_subscribe_doc(doctype, docname):
	...
	from capkpi.exceptions import PermissionError
	from capkpi.sessions import Session

	session = Session(None, resume=True).get_session_data()
	if not capkpi.has_permission(user=session.user, doctype=doctype, doc=docname, ptype="read"):
...
def get_doc_room(doctype, docname):
	return "".join([capkpi.local.site, ":doc:", doctype, "/", docname])


def get_user_room(user):
	return "".join([capkpi.local.site, ":user:", user])


def get_site_room():
	return "".join([capkpi.local.site, ":all"])


def get_task_progress_room(task_id):
	return "".join([capkpi.local.site, ":task_progress:", task_id])


def get_chat_room(room):
	room = "".join([capkpi.local.site, ":room:", room])
