...
from capkpi.desk.form.utils import get_pdf_link
from capkpi.desk.notifications import clear_doctype_notifications
from capkpi.model.document import Document
from capkpi.model.workflow import (
	apply_workflow,
	get_workflow_name,
...
from capkpi.utils import get_datetime, get_url
...
def on_doctype_update():
	capkpi.db.add_index("Workflow Action", ["status", "user"])
...
def get_permission_query_conditions(user):
	...
	return "(`tabWorkflow Action`.`user`={user})".format(user=capkpi.db.escape(user))
...
def confirm_action(doctype, docname, user, action):
	...
	logged_in_user = capkpi.session.user
	...
	if logged_in_user == "Guest":
		capkpi.set_user(logged_in_user)
...
def return_success_page(doc):
	capkpi.respond_as_web_page(
		...
		_("{0}: {1} is set to state {2}").format(
			doc.get("doctype"), capkpi.bold(doc.get("name")), capkpi.bold(get_doc_workflow_state(doc))
		),
...
def return_link_expired_page(doc, doc_workflow_state):
	capkpi.respond_as_web_page(
		...
		_("Document {0} has been set to state {1} by {2}").format(
			capkpi.bold(doc.get("name")),
			capkpi.bold(doc_workflow_state),
			capkpi.bold(capkpi.get_value("User", doc.get("modified_by"), "full_name")),
		),
		indicator_color="blue",
	)
...
def clear_old_workflow_actions(doc, user=None):
	user = user if user else capkpi.session.user
	capkpi.db.sql(
	...
	for transition in transitions:
		...
		for user in filtered_users:
			if not user_data_map.get(user):
				user_data_map[user] = capkpi._dict(
					{
						"possible_actions": [],
						"email": capkpi.db.get_value("User", user, "email"),
					}
				)
...
def send_workflow_action_email(users_data, doc):
	...
	for d in users_data:
		...
		enqueue(method=capkpi.sendmail, queue="short", **email_args)
...
def get_workflow_action_url(action, doc, user):
	apply_action_method = (
		"/api/method/capkpi.workflow.doctype.workflow_action.workflow_action.apply_action"
	)
...
def get_confirm_workflow_action_url(doc, action, user):
	confirm_action_method = (
		"/api/method/capkpi.workflow.doctype.workflow_action.workflow_action.confirm_action"
	)
...
def get_common_email_args(doc):
	...
	if email_template:
		subject = capkpi.render_template(email_template.subject, vars(doc))
		response = capkpi.render_template(email_template.response, vars(doc))
	else:
	...
	common_args = {
		...
		"attachments": [capkpi.attach_print(doctype, docname, file_name=docname, doc=doc)],
		"subject": subject,
		"message": response,
	}
	return common_args
...
def get_email_template(doc):
	...
	template_name = capkpi.db.get_value(
	...
	if not template_name:
		return
	return capkpi.get_doc("Email Template", template_name)
