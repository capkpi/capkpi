...
class TestWorkflow(unittest.TestCase):
	...
	def tearDown(self):
		capkpi.delete_doc("Workflow", "Test ToDo")
	...
	def test_if_workflow_actions_were_processed(self):
		capkpi.db.sql("delete from `tabWorkflow Action`")
		...
		workflow_actions = capkpi.get_all("Workflow Action", fields=["*"])
		...
		workflow_actions = capkpi.get_all("Workflow Action", fields=["status"])
		self.assertEqual(len(workflow_actions), 1)
...
def create_todo_workflow():
	if capkpi.db.exists("Workflow", "Test ToDo"):
		capkpi.delete_doc("Workflow", "Test ToDo")

	if not capkpi.db.exists("Role", "Test Approver"):
		capkpi.get_doc(dict(doctype="Role", role_name="Test Approver")).insert(ignore_if_duplicate=True)
	workflow = capkpi.new_doc("Workflow")
...
def create_new_todo():
	return capkpi.get_doc(dict(doctype="ToDo", description="workflow " + random_string(10))).insert()
