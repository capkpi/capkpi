capkpi.provide("capkpi.core")

capkpi.ui.form.on("Workflow", {
	onload: function(frm) {
	...
	update_field_options: function(frm) {
		...
		if (doc.document_type) {
			const get_field_method = 'capkpi.workflow.doctype.workflow.workflow.get_fieldnames_for';
			capkpi.xcall(get_field_method, { doctype: doc.document_type })
				.then(resp => {
	...
	set_table_html: function(frm) {
		...
		const promises = frm.states.map(r => {
			...
			return capkpi.utils.get_indicator_color(state).then(color => {
				return `<tr>
				<td>
	...
	get_orphaned_states_and_count: function(frm) {
		...
		return capkpi.xcall('capkpi.workflow.doctype.workflow.workflow.get_workflow_state_count', {
	...
	render_state_table: function(frm) {
		if (frm.states && frm.states.length) {
			...
			$(frm.state_table).find('a.orphaned-state').on('click', (e) => {
				...
				capkpi.set_route('List', frm.doc.document_type, filters);
			});
		}
	}
...
capkpi.ui.form.on("Workflow Document State", {
