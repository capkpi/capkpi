...
def get_file_url(file_data_name):
	data = capkpi.db.get_value("File", file_data_name, ["file_name", "file_url"], as_dict=True)
	return data.file_url or data.file_name


def upload():
	# get record details
	dt = capkpi.form_dict.doctype
	dn = capkpi.form_dict.docname
	file_url = capkpi.form_dict.file_url
	filename = capkpi.form_dict.filename
	capkpi.form_dict.is_private = cint(capkpi.form_dict.is_private)

	if not filename and not file_url:
		capkpi.msgprint(_("Please select a file or url"), raise_exception=True)

	file_doc = get_file_doc()

	comment = {}
	if dt and dn:
		comment = capkpi.get_doc(dt, dn).add_comment(
			"Attachment",
			_("added {0}").format(
...
def save_url(file_url, filename, dt, dn, folder, is_private, df=None):
	...
	# 	capkpi.msgprint("URL must start with 'http://' or 'https://'")
	# 	return None, None

	file_url = unquote(file_url)
	file_size = capkpi.form_dict.file_size
	...
	try:
		...
		return capkpi.get_doc("File", f.duplicate_entry)
	return f


def get_uploaded_content():
	# should not be unicode when reading a file, hence using capkpi.form
	if "filedata" in capkpi.form_dict:
		if "," in capkpi.form_dict.filedata:
			capkpi.form_dict.filedata = capkpi.form_dict.filedata.rsplit(",", 1)[1]
		capkpi.uploaded_content = base64.b64decode(capkpi.form_dict.filedata)
		capkpi.uploaded_filename = capkpi.form_dict.filename
		return capkpi.uploaded_filename, capkpi.uploaded_content
	else:
		capkpi.msgprint(_("No file attached"))
		return None, None
...
def get_file_data_from_hash(content_hash, is_private=0):
	for name in capkpi.db.sql_list(
		...
		b = capkpi.get_doc("File", name)
		return {k: b.get(k) for k in capkpi.get_hooks()["write_file_keys"]}
	return False
	...
	try:
		for fid in capkpi.db.sql_list(
			...
			if from_delete:
				...
				capkpi.delete_doc("File", fid, ignore_permissions=True, delete_permanently=delete_permanently)
			else:
...
def remove_file(
	...
	if attached_to_doctype and attached_to_name and not from_delete:
		doc = capkpi.get_doc(attached_to_doctype, attached_to_name)
		ignore_permissions = doc.has_permission("write") or False
		if capkpi.flags.in_web_form:
			ignore_permissions = True
		if not file_name:
			file_name = capkpi.db.get_value("File", fid, "file_name")
...
def get_file_name(fname, optional_suffix):
	...
	n_records = capkpi.db.sql("select name from `tabFile` where file_name=%s", fname)
...
def download_file(file_url):
	...
	Method : GET
	Endpoint : capkpi.utils.file_manager.download_file
	...
	capkpi.local.response.filecontent = filedata
...
def extract_images_from_html(doc, content):
	...
	def _save_file(match):
		...
		if not capkpi.flags.has_dataurl:
