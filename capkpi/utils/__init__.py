...
from capkpi.utils.html_utils import sanitize_html
...
def get_fullname(user=None):
	...
	if not hasattr(capkpi.local, "fullnames"):
		capkpi.local.fullnames = {}

	if not capkpi.local.fullnames.get(user):
		p = capkpi.db.get_value("User", user, ["first_name", "last_name"], as_dict=True)
		if p:
			capkpi.local.fullnames[user] = (
				" ".join(filter(None, [p.get("first_name"), p.get("last_name")])) or user
			)
		else:
			capkpi.local.fullnames[user] = user

	return capkpi.local.fullnames.get(user)
...
def get_email_address(user=None):
	...
	return capkpi.db.get_value("User", user, "email")
...
def validate_phone_number(phone_number, throw=False):
	...
	if not match and throw:
		capkpi.throw(
			capkpi._("{0} is not a valid Phone Number").format(phone_number), capkpi.InvalidPhoneNumberError
		)
...
def validate_name(name, throw=False):
	...
	if not match and throw:
		capkpi.throw(capkpi._("{0} is not a valid Name").format(name), capkpi.InvalidNameError)
...
def validate_email_address(email_str, throw=False):
	...
	def _check(e):
		...
		if not _valid:
			if throw:
				invalid_email = capkpi.utils.escape_html(e)
				capkpi.throw(
					capkpi._("{0} is not a valid Email Address").format(invalid_email),
					capkpi.InvalidEmailAddressError,
				)
			return None
		else:
			return matched
...
def validate_url(txt, throw=False, valid_schemes=None):
	...
	if not is_valid and throw:
		capkpi.throw(capkpi._("'{0}' is not a valid URL").format(capkpi.bold(txt)))
...
def has_gravatar(email):
	...
	if capkpi.flags.in_import or capkpi.flags.in_install or capkpi.flags.in_test:
		# no gravatar if via upload
		# since querying gravatar for every item will be slow
		return ""

	hexdigest = hashlib.md5(capkpi.as_unicode(email).encode("utf-8")).hexdigest()
...
def get_gravatar(email):
	from capkpi.utils.identicon import Identicon
...
def get_defaults(key=None):
	...
	"""
	return capkpi.db.get_defaults(key)
...
def set_default(key, val):
	...
	"""
	return capkpi.db.set_default(key, val)
...
def get_path(*path, **kwargs):
	...
	if not base:
		base = capkpi.local.site_path
	return os.path.join(base, *path)


def get_site_base_path(sites_dir=None, hostname=None):
	return capkpi.local.site_path
...
def get_bench_path():
	return os.path.realpath(os.path.join(os.path.dirname(capkpi.__file__), "..", "..", ".."))
...
def get_site_url(site):
	return "http://{site}:{port}".format(site=site, port=capkpi.get_conf(site).webserver_port)
...
def get_test_client():
	from capkpi.app import application

	return Client(application)


def get_hook_method(hook_name, fallback=None):
	method = capkpi.get_hooks().get(hook_name)
	if method:
		method = capkpi.get_attr(method[0])
		return method
	if fallback:
		return fallback
...
def call_hook_method(hook, *args, **kwargs):
	out = None
	for method_name in capkpi.get_hooks(hook):
		out = out or capkpi.get_attr(method_name)(*args, **kwargs)
...
def update_progress_bar(txt, i, l):
	...
	if not getattr(capkpi.local, "request", None) or is_cli():
		lt = len(txt)
		try:
	...
	if os.path.exists(print_path):
		...
		for include_directive, path in re.findall("""({% include ['"]([^'"]*)['"] %})""", html_format):
			for app_name in capkpi.get_installed_apps():
				include_path = capkpi.get_app_path(app_name, *path.split(os.path.sep))
...
def get_sites(sites_path=None):
	if not sites_path:
		sites_path = getattr(capkpi.local, "sites_path", None) or "."
...
def markdown(text, sanitize=True, linkify=True):
	html = text if is_html(text) else capkpi.utils.md_to_html(text)
...
def get_site_info():
	...
	from capkpi.email.queue import get_emails_sent_this_month
	...
	system_settings = capkpi.db.get_singles_dict("System Settings")
	space_usage = capkpi._dict((capkpi.local.conf.limits or {}).get("space_usage", {}))
	...
	site_info = {
		...
		"last_logins": capkpi.get_all("Activity Log", **kwargs),
	}

	# from other apps
	for method_name in capkpi.get_hooks("get_site_info"):
		site_info.update(capkpi.get_attr(method_name)(site_info) or {})

	# dumps -> loads to prevent datatype conflicts
	return json.loads(capkpi.as_json(site_info))
...
def get_db_count(*args):
	...
	Example:
	        via terminal:
	                bench --site erp.local execute capkpi.utils.get_db_count --args "['DocType', 'Communication']"
	"""
	db_count = {}
	for doctype in args:
		db_count[doctype] = capkpi.db.count(doctype)

	return json.loads(capkpi.as_json(db_count))
...
def call(fn, *args, **kwargs):
	...
	Pass a doctype or a series of doctypes to get the count of docs in them
	Parameters:
	        fn: capkpi function to be called
	...
	Example:
	        via terminal:
	                bench --site erp.local execute capkpi.utils.call --args '''["capkpi.get_all", "Activity Log"]''' --kwargs '''{"fields": ["user", "creation", "full_name"], "filters":{"Operation": "Login", "Status": "Success"}, "limit": "10"}'''
	"""
	return json.loads(capkpi.as_json(capkpi.call(fn, *args, **kwargs)))
...
def get_safe_filters(filters):
	try:
		...
		if isinstance(filters, (int, float)):
			filters = capkpi.as_unicode(filters)
...
def set_request(**kwargs):
	...
	capkpi.local.request = Request(builder.get_environ())
...
def get_build_version():
	try:
		return str(os.path.getmtime(os.path.join(capkpi.local.sites_path, ".build")))
	except OSError:
		# .build can sometimes not exist
		# this is not a major problem so send fallback
		return capkpi.utils.random_string(8)
