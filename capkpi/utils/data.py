...
def get_first_day_of_the_week():
	return capkpi.get_system_settings("first_day_of_the_week") or "Sunday"
...
def getdate(string_date: Optional[str] = None):
	...
	try:
		...
		capkpi.throw(
			capkpi._("{} is not a valid date string.").format(capkpi.bold(string_date)),
			title=capkpi._("Invalid Date"),
		)
...
def add_to_date(
	...
	if isinstance(date, str):
		...
		try:
			...
			capkpi.throw(capkpi._("Please select a valid date filter"), title=capkpi._("Invalid Date"))
...
def _get_time_zone():
	return capkpi.db.get_system_setting("time_zone") or "Asia/Kolkata"  # Default to India ?!
...
def get_time_zone():
	...
	return capkpi.cache().get_value("time_zone", _get_time_zone)
...
def now():
	...
	if capkpi.flags.current_date:
		return (
			getdate(capkpi.flags.current_date).strftime(DATE_FORMAT)
			+ " "
...
def get_user_date_format():
	...
	if getattr(capkpi.local, "user_date_format", None) is None:
		capkpi.local.user_date_format = capkpi.db.get_default("date_format")

	return capkpi.local.user_date_format or "yyyy-mm-dd"
...
def get_user_time_format():
	...
	if getattr(capkpi.local, "user_time_format", None) is None:
		capkpi.local.user_time_format = capkpi.db.get_default("time_format")

	return capkpi.local.user_time_format or "HH:mm:ss"
	...
	* dd/mm/yyyy
	...
	try:
		formatted_date = babel.dates.format_date(
			date, format_string, locale=(capkpi.local.lang or "").replace("-", "_")
		)
	...
	* HH:mm
	...
	try:
		formatted_time = babel.dates.format_time(
			time_, format_string, locale=(capkpi.local.lang or "").replace("-", "_")
		)
	except UnknownLocaleError:
	...
	* mm-dd-yyyy HH:mm
	...
	try:
		formatted_datetime = babel.dates.format_datetime(
			datetime, format_string, locale=(capkpi.local.lang or "").replace("-", "_")
		)
	except UnknownLocaleError:
...
def validate_duration_format(duration):
	...
	if not is_valid_duration:
		capkpi.throw(
			capkpi._("Value {0} must be in the valid duration format: d h m s").format(
				capkpi.bold(duration)
			)
		)
...
def global_date_format(date, format="long"):
	...
	date = getdate(date)
	formatted_date = babel.dates.format_date(
		date, locale=(capkpi.local.lang or "en").replace("-", "_"), format=format
	)
	return formatted_date
...
def cast_fieldtype(fieldtype, value, show_warning=True):
	if show_warning:
		message = (
			"Function `capkpi.utils.data.cast_fieldtype` has been deprecated in favour"
			" of `capkpi.utils.data.cast`. Use the newer util for safer type casting."
		)
...
def cast(fieldtype, value=None):
	"""Cast the value to the Python native object of the CapKPI fieldtype provided.
	...
	Mapping of Python types => CapKPI types:
...
def cstr(s, encoding="utf-8"):
	return capkpi.as_unicode(s, encoding)
...
def round_based_on_smallest_currency_fraction(value, currency, precision=2):
	smallest_currency_fraction_value = flt(
		capkpi.db.get_value("Currency", currency, "smallest_currency_fraction_value", cache=True)
	)
...
def fmt_money(amount, precision=None, currency=None, format=None):
	...
	"""
	number_format = format or capkpi.db.get_default("number_format") or "#,###.##"
	if precision is None:
		precision = cint(capkpi.db.get_default("currency_precision")) or None
	...
	if decimal_str:
		...
		if precision > 2:
			if len(decimals) < 3:
				if currency:
					fraction = capkpi.db.get_value("Currency", currency, "fraction_units", cache=True) or 100
	...
	if currency and capkpi.defaults.get_global_default("hide_currency_symbol") != "Yes":
		symbol = capkpi.db.get_value("Currency", currency, "symbol", cache=True) or currency
...
def money_in_words(
	...
	"""
	from capkpi.utils import get_defaults
	...
	if not fraction_currency:
		fraction_currency = capkpi.db.get_value("Currency", main_currency, "fraction", cache=True) or _(
			"Cent"
		)

	number_format = (
		capkpi.db.get_value("Currency", main_currency, "number_format", cache=True)
		or capkpi.db.get_default("number_format")
		or "#,###.##"
	)
...
def in_words(integer, in_million=True):
	...
	locale = "en_IN" if not in_million else capkpi.local.lang
	integer = int(integer)
	try:
...
def get_thumbnail_base64_for_image(src):
	...
	from capkpi import cache, safe_decode
	from capkpi.core.doctype.file.file import get_local_image

	if not src:
		capkpi.throw("Invalid source for image: {0}".format(src))
	...
	def _get_base64():
		file_path = capkpi.get_site_path("public", src.lstrip("/"))
...
def comma_or(some_list, add_quotes=True):
	return comma_sep(some_list, capkpi._("{0} or {1}"), add_quotes)


def comma_and(some_list, add_quotes=True):
	return comma_sep(some_list, capkpi._("{0} and {1}"), add_quotes)
...
def comma_sep(some_list, pattern, add_quotes=True):
	if isinstance(some_list, (list, tuple)):
		...
		if not some_list:
			...
			return pattern.format(", ".join(capkpi._(s) for s in some_list[:-1]), some_list[-1])
	else:
		return some_list
...
def get_url(uri=None, full_address=False):
	...
	host_name = capkpi.local.conf.host_name or capkpi.local.conf.hostname
	...
	if not host_name:
		...
		elif capkpi.local.site:
			protocol = "http://"

			if capkpi.local.conf.ssl_certificate:
				protocol = "https://"

			elif capkpi.local.conf.wildcard:
				domain = capkpi.local.conf.wildcard.get("domain")
				if (
					domain
					and capkpi.local.site.endswith(domain)
					and capkpi.local.conf.wildcard.get("ssl_certificate")
				):
					protocol = "https://"

			host_name = protocol + capkpi.local.site

		else:
			host_name = capkpi.db.get_value("Website Settings", "Website Settings", "subdomain")
	...
	if not uri and full_address:
		uri = capkpi.get_request_header("REQUEST_URI", "")

	port = capkpi.conf.http_port or capkpi.conf.webserver_port

	if (
		not (capkpi.conf.restart_supervisor_on_update or capkpi.conf.restart_systemd_on_update)
		and host_name
...
def get_host_name_from_request():
	if hasattr(capkpi.local, "request") and capkpi.local.request and capkpi.local.request.host:
		protocol = (
			"https://" if "https" == capkpi.get_request_header("X-Forwarded-Proto", "") else "http://"
		)
		return protocol + capkpi.local.request.host
...
def get_filter(doctype: str, f: Union[Dict, List, Tuple], filters_config=None) -> "capkpi._dict":
	...
	if not isinstance(f, (list, tuple)):
		capkpi.throw(capkpi._("Filter must be a tuple or list (in a list)"))
	...
	if len(f) == 3:
		...
		capkpi.throw(
			capkpi._("Filter must have 4 values (doctype, fieldname, operator, value): {0}").format(str(f))
		)

	f = capkpi._dict(doctype=f[0], fieldname=f[1], operator=f[2], value=f[3])
	...
	if f.operator.lower() not in valid_operators:
		capkpi.throw(capkpi._("Operator must be one of {0}").format(", ".join(valid_operators)))

	if f.doctype and (f.fieldname not in default_fields + optional_fields):
		# verify fieldname belongs to the doctype
		meta = capkpi.get_meta(f.doctype)
		if not meta.has_field(f.fieldname):

			# try and match the doctype name from child tables
			for df in meta.get_table_fields():
				if capkpi.get_meta(df.options).has_field(f.fieldname):
					f.doctype = df.options
					break

	try:
		df = capkpi.get_meta(f.doctype).get_field(f.fieldname)
	except capkpi.exceptions.DoesNotExistError:
		df = None
...
def sanitize_column(column_name):
	...
	if capkpi.db and capkpi.db.db_type == "mariadb":
	...
	def _raise_exception():
		capkpi.throw(_("Invalid field name {0}").format(column_name), capkpi.DataError)
...
def generate_hash(*args, **kwargs) -> str:
	return capkpi.generate_hash(*args, **kwargs)
...
def get_user_info_for_avatar(user_id: str) -> Dict:
	...
	try:
		user_info["email"] = capkpi.get_cached_value("User", user_id, "email")
		user_info["name"] = capkpi.get_cached_value("User", user_id, "full_name")
		user_info["image"] = capkpi.get_cached_value("User", user_id, "user_image")
	except Exception:
...
def validate_python_code(string: str, fieldname=None, is_expression: bool = True) -> None:
	...
	try:
		...
		msg = capkpi._("{} Invalid python code on line {}").format(
			fieldname + ":" if fieldname else "", line_no + 1
		)
		msg += f"<br><pre>{error_line}</pre>"
		msg += f"<pre>{' ' * offset}^</pre>"

		capkpi.throw(msg, title=capkpi._("Syntax Error"))
	except Exception as e:
		capkpi.msgprint(
			capkpi._("{} Possibly invalid python code. <br>{}").format(fieldname + ": " or "", str(e)),
			indicator="orange",
		)
...
def format_timedelta(o: datetime.timedelta) -> str:
	...
	# but capkpi doesnt - i think via babel : only allows 0..23 range for hour
