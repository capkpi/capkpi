...
from capkpi.boot import get_allowed_reports
from capkpi.core.doctype.domain_settings.domain_settings import get_active_modules
from capkpi.permissions import get_roles, get_valid_perms
from capkpi.utils import cint


class UserPermissions:
	"""
	A user permission object can be accessed as `capkpi.get_user()`
	"""

	def __init__(self, name=""):
		self.defaults = None
		self.name = name or capkpi.session.get("user")
		self.roles = []
	...
	def setup_user(self):
		def get_user_doc():
			user = None
			try:
				user = capkpi.get_doc("User", self.name).as_dict()
			except capkpi.DoesNotExistError:
				pass
			except Exception as e:
				# install boo-boo
				if not capkpi.db.is_table_missing(e):
					raise

			return user

		if not capkpi.flags.in_install_db and not capkpi.flags.in_test:
			user_doc = capkpi.cache().hget("user_doc", self.name, get_user_doc)
			if user_doc:
				self.doc = capkpi.get_doc(user_doc)
	...
	def build_perm_map(self):
		...
		for r in get_valid_perms():
			...
			for k in capkpi.permissions.rights:
	...
	def build_permissions(self):
		...
		self.build_perm_map()
		user_shared = capkpi.share.get_shared_doctypes()
		no_list_view_link = []
		...
		self.shared = capkpi.db.sql_list(
		...
		if "System Manager" in self.get_roles():
			docs = capkpi.get_all("DocType", {"allow_import": 1})
		...
		capkpi.cache().hset("can_import", capkpi.session.user, self.can_import)

	def get_defaults(self):
		import capkpi.defaults

		self.defaults = capkpi.defaults.get_defaults(self.name)
...
def get_fullname_and_avatar(user):
	first_name, last_name, avatar, name = capkpi.db.get_value(
...
def add_role(user, role):
	capkpi.get_doc("User", user).add_roles(role)
...
def is_website_user():
	return capkpi.db.get_value("User", capkpi.session.user, "user_type") == "Website User"


def is_system_user(username):
	return capkpi.db.get_value("User", {"email": username, "enabled": 1, "user_type": "System User"})
...
def get_users():
	...
	users = []
	system_managers = capkpi.utils.user.get_system_managers(only_name=True)
	for user in get_system_users():
		users.append(
			{
				"full_name": capkpi.utils.user.get_user_fullname(user),
				"email": user,
...
def set_last_active_to_now(user):
	...
	capkpi.db.set_value("User", user, "last_active", now_datetime())


def reset_simultaneous_sessions(user_limit):
	for user in capkpi.db.sql(
		...
		if user.simultaneous_sessions < user_limit:
			...
			capkpi.db.set_value("User", user.name, "simultaneous_sessions", 1)
