...
def get_queues_timeout():
	common_site_config = capkpi.get_conf()
...
def enqueue(
	...
	**kwargs
	...
	:param now: if now=True, the method is executed via capkpi.call
	:param kwargs: keyword arguments to be passed to the method
	"""
	# To handle older implementations
	is_async = kwargs.pop("async", is_async)

	if now or capkpi.flags.in_migrate:
		return capkpi.call(method, **kwargs)
	...
	if enqueue_after_commit:
		if not capkpi.flags.enqueue_after_commit:
			capkpi.flags.enqueue_after_commit = []

		capkpi.flags.enqueue_after_commit.append(
			{"queue": queue, "is_async": is_async, "timeout": timeout, "queue_args": queue_args}
		)
		return capkpi.flags.enqueue_after_commit
...
def enqueue_doc(
	...
	return enqueue(
		"capkpi.utils.background_jobs.run_doc_method",
		doctype=doctype,
		name=name,
		doc_method=method,
		queue=queue,
...
def run_doc_method(doctype, name, doc_method, **kwargs):
	getattr(capkpi.get_doc(doctype, name), doc_method)(**kwargs)
...
def execute_job(site, method, event, job_name, kwargs, user=None, is_async=True, retry=0):
	...
	capkpi.monitor.start("job", method_name, kwargs)
	try:
		method(**kwargs)

	except (capkpi.db.InternalError, capkpi.RetryBackgroundJobError) as e:
		capkpi.db.rollback()

		if retry < 5 and (
			isinstance(e, capkpi.RetryBackgroundJobError)
			or (capkpi.db.is_deadlocked(e) or capkpi.db.is_timedout(e))
		):
			# retry the job if
			# 1213 = deadlock
			...
			capkpi.log_error(title=method_name)
			raise
...
def validate_queue(queue, default_queue_list=None):
	...
	if queue not in default_queue_list:
		capkpi.throw(_("Queue should be one of {0}").format(", ".join(default_queue_list)))
...
def get_redis_conn():
	if not hasattr(capkpi.local, "conf"):
		raise Exception("You need to call capkpi.init")

	elif not capkpi.local.conf.redis_queue:
		raise Exception("redis_queue missing in common_site_config.json")

	global redis_connection

	if not redis_connection:
		redis_connection = redis.from_url(capkpi.local.conf.redis_queue)

	return redis_connection


def enqueue_test_job():
	enqueue("capkpi.utils.background_jobs.test_job", s=100)
