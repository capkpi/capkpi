...
def get_pdf(html, options=None, output=None):
	...
	try:
		...
		if any([error in str(e) for error in PDF_CONTENT_ERRORS]):
			if not filedata:
				capkpi.throw(_("PDF generation failed because of broken image links"))
	...
	if "password" in options:
		...
		if six.PY2:
			password = capkpi.safe_encode(password)
...
def prepare_options(html, options):
	...
	# page size
	pdf_page_size = (
		options.get("page-size") or capkpi.db.get_single_value("Print Settings", "pdf_page_size") or "A4"
	)

	if pdf_page_size == "Custom":
		options["page-height"] = options.get("page-height") or capkpi.db.get_single_value(
			"Print Settings", "pdf_page_height"
		)
		options["page-width"] = options.get("page-width") or capkpi.db.get_single_value(
...
def prepare_header_footer(soup):
	...
	css = capkpi.read_file(os.path.join(capkpi.local.sites_path, "assets/css/printview.css"))
	...
	for html_id in ("header-html", "footer-html"):
		...
		if content:
			...
			# create temp file
			fname = os.path.join("/tmp", "capkpi-pdf-{0}.html".format(capkpi.generate_hash()))
			with open(fname, "wb") as f:
				f.write(html.encode("utf-8"))

			# {"header-html": "/tmp/capkpi-pdf-random.html"}
...
def get_wkhtmltopdf_version():
	wkhtmltopdf_version = capkpi.cache().hget("wkhtmltopdf_version", None)

	if not wkhtmltopdf_version:
		try:
			res = subprocess.check_output(["wkhtmltopdf", "--version"])
			wkhtmltopdf_version = res.decode("utf-8").split(" ")[1]
			capkpi.cache().hset("wkhtmltopdf_version", None, wkhtmltopdf_version)
		except Exception:
			pass
