...
def before_install():
	...
	capkpi.reload_doc("core", "doctype", "docperm")
...
def after_install():
	...
	capkpi.db.set_global("installed_apps", '["capkpi"]')

	create_user_type()
	install_basic_docs()

	from capkpi.core.doctype.file.file import make_home_folder
	...
	# all roles to admin
	capkpi.get_doc("User", "Administrator").add_roles(
		*capkpi.db.sql_list("""select name from tabRole""")
	)

	# update admin password
	update_password("Administrator", get_admin_password())

	if not capkpi.conf.skip_setup_wizard:
		capkpi.db.set_default("desktop:home_page", "setup-wizard")
...
def create_user_type():
	for user_type in ["System User", "Website User"]:
		if not capkpi.db.exists("User Type", user_type):
			capkpi.get_doc({"doctype": "User Type", "name": user_type, "is_standard": 1}).insert(
...
def get_admin_password():
	...
	admin_password = capkpi.conf.get("admin_password")
...
def before_tests():
	if len(capkpi.get_installed_apps()) > 1:
		# don't run before tests if any other app is installed
		return

	capkpi.db.sql("delete from `tabCustom Field`")
	capkpi.db.sql("delete from `tabEvent`")
	capkpi.db.commit()
	capkpi.clear_cache()

	# complete setup if missing
	if not int(capkpi.db.get_single_value("System Settings", "setup_complete") or 0):
...
def import_country_and_currency():
	from capkpi.geo.country_info import get_all
	...
	for i, name in enumerate(data):
		...
		country = capkpi._dict(data[name])
	...
	for currency in ("INR", "USD", "GBP", "EUR", "AED", "AUD", "JPY", "CNY", "CHF"):
		capkpi.db.set_value("Currency", currency, "enabled", 1)


def add_country_and_currency(name, country):
	if not capkpi.db.exists("Country", name):
		capkpi.get_doc(
			{
				"doctype": "Country",
				"country_name": name,
	...
	if country.currency and not capkpi.db.exists("Currency", country.currency):
		capkpi.get_doc(
			{
				"doctype": "Currency",
...
def add_standard_navbar_items():
	...
	standard_navbar_items = [
		...
		},
		{
			...
			"item_type": "Action",
			"action": "capkpi.ui.toolbar.route_to_user()",
			"is_standard": 1,
		},
		{
			"item_label": "Session Defaults",
			"item_type": "Action",
			"action": "capkpi.ui.toolbar.setup_session_defaults()",
			"is_standard": 1,
		},
		{
			"item_label": "Reload",
			"item_type": "Action",
			"action": "capkpi.ui.toolbar.clear_cache()",
			"is_standard": 1,
		},
		{
			"item_label": "View Website",
			"item_type": "Action",
			"action": "capkpi.ui.toolbar.view_website()",
			"is_standard": 1,
		},
		{
	...
	standard_help_items = [
		{
			"item_label": "About",
			"item_type": "Action",
			"action": "capkpi.ui.toolbar.show_about()",
			"is_standard": 1,
		},
		{
			"item_label": "Keyboard Shortcuts",
			"item_type": "Action",
			"action": "capkpi.ui.toolbar.show_shortcuts(event)",
			"is_standard": 1,
		},
	]
