...
def get_jenv():
	...
	if not getattr(capkpi.local, "jenv", None):
		from jinja2 import DebugUndefined
		from jinja2.sandbox import SandboxedEnvironment

		# capkpi will be loaded last, so app templates will get precedence
		...
		capkpi.local.jenv = jenv

	return capkpi.local.jenv
...
def validate_template(html):
	...
	try:
		...
		capkpi.msgprint("Line {}: {}".format(e.lineno, e.message))
		capkpi.throw(capkpi._("Syntax error in template"))
...
def render_template(template, context, is_path=None, safe_render=True):
	...
	from capkpi import _, get_traceback, throw
...
def get_jloader():
	...
	if not getattr(capkpi.local, "jloader", None):
		from jinja2 import ChoiceLoader, PackageLoader, PrefixLoader

		if capkpi.local.flags.in_setup_help:
			apps = ["capkpi"]
		else:
			apps = capkpi.get_hooks("template_apps")
			if not apps:
				apps = capkpi.local.flags.web_pages_apps or capkpi.get_installed_apps(sort=True)
				apps.reverse()

		if "capkpi" not in apps:
			apps.append("capkpi")

		capkpi.local.jloader = ChoiceLoader(
	...
	return capkpi.local.jloader


def set_filters(jenv):
	import capkpi
	from capkpi.utils import cint, cstr, flt, global_date_format, markdown
	from capkpi.website.utils import abs_url, get_shade

	jenv.filters["global_date_format"] = global_date_format
	jenv.filters["markdown"] = markdown
	jenv.filters["json"] = capkpi.as_json
...
def get_jenv_customization(customization_type):
	...
	out = {}
	if not getattr(capkpi.local, "site", None):
		return out

	values = capkpi.get_hooks("jenv", {}).get(customization_type)
	if not values:
		return out

	for value in values:
		fn_name, fn_string = value.split(":")
		out[fn_name] = capkpi.get_attr(fn_string)
...
def resolve_class(classes):
	...
	if isinstance(classes, capkpi.string_types):
		return classes
...
def web_blocks(blocks):
	from capkpi import _, _dict, throw
	from capkpi.website.doctype.web_page.web_page import get_web_blocks_html
