...
import capkpi
from capkpi.utils import get_sites
...
def get_logger(
	...
	if allow_site is True:
		site = getattr(capkpi.local, "site", None)
	...
	try:
		return capkpi.loggers[logger_name]
	except KeyError:
		pass
	...
	logger.setLevel(capkpi.log_level or default_log_level)
	...
	capkpi.loggers[logger_name] = logger
...
class SiteContextFilter(logging.Filter):
	...
	def filter(self, record):
		if "Form Dict" not in text_type(record.msg):
			site = getattr(capkpi.local, "site", None)
			form_dict = getattr(capkpi.local, "form_dict", None)
...
def set_log_level(level):
	...
	capkpi.log_level = getattr(logging, (level or "").upper(), None) or default_log_level
	capkpi.loggers = {}
