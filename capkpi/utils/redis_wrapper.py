...
class RedisWrapper(redis.Redis):
	...
	def get_value(self, key, generator=None, user=None, expires=False):
		...
		:param expires: If the key is supposed to be with an expiry, don't store it in capkpi.local
		"""
		original_key = key
		key = self.make_key(key, user)

		if key in capkpi.local.cache:
			val = capkpi.local.cache[key]
	...
	def get_keys(self, key):
		...
		try:
			...
			return [k for k in list(capkpi.local.cache) if regex.match(cstr(k))]
	...
	def hset(self, name, key, value, shared=False):
		...
		# set in local
		if not _name in capkpi.local.cache:
			capkpi.local.cache[_name] = {}
		capkpi.local.cache[_name][key] = value
	...
	def hget(self, name, key, generator=None, shared=False):
		...
		if key in capkpi.local.cache[_name]:
			return capkpi.local.cache[_name][key]
	...
	def hdel(self, name, key, shared=False):
		...
		if _name in capkpi.local.cache:
			if key in capkpi.local.cache[_name]:
				del capkpi.local.cache[_name][key]
		try:
	...
	def hdel_keys(self, name_starts_with, key):
		...
		for name in capkpi.cache().get_keys(name_starts_with):
			name = name.split("|", 1)[1]
			self.hdel(name, key)
