...
import capkpi
from capkpi.core.page.permission_manager.permission_manager import get_standard_permissions
from capkpi.custom.doctype.custom_field.custom_field import create_custom_field
from capkpi.custom.doctype.customize_form.customize_form import (
	docfield_properties,
	doctype_properties,
)
from capkpi.custom.doctype.property_setter.property_setter import make_property_setter
from capkpi.modules import get_module_path, scrub, utils
from capkpi.permissions import setup_custom_perms

branch = "develop"


def reset_all():
	for doctype in capkpi.db.get_all("DocType", dict(custom=0)):
		print(doctype.name)
...
def reset_doc(doctype):
	...
	try:
		git_link = "/".join(
			["https://raw.githubusercontent.com/capkpi", app, branch, doc_path.split("apps/" + app)[1]]
		)
...
def make_custom_fields(doctype, local_doc, original_doc):
	...
	doctype_doc = capkpi.get_doc("DocType", doctype)
...
def get_custom_docfield_properties():
	fields_meta = capkpi.get_meta("Custom Field").fields
	fields = {}
...
def setup_perms_for(doctype):
	perms = capkpi.get_all("DocPerm", fields="*", filters=dict(parent=doctype), order_by="idx asc")
	# get default perms
	try:
	...
	if len(standard_perms) != len(perms):
		...
		for i, p in enumerate(perms):
			...
			for fieldname in capkpi.get_meta("DocPerm").get_fieldnames_with_value():
