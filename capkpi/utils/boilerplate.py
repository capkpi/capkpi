...
import capkpi
from capkpi.utils import cstr, touch_file
...
def make_boilerplate(dest, app_name):
	...
	app_name = capkpi.scrub(app_name)
	...
	capkpi.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, capkpi.scrub(hooks.app_title)), with_init=True
	)
	capkpi.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, "templates"), with_init=True
	)
	capkpi.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "www"))
	capkpi.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "pages"), with_init=True
	)
	capkpi.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "includes"))
	capkpi.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "config"), with_init=True)
	capkpi.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "css"))
	capkpi.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "js"))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "__init__.py"), "w") as f:
		f.write(capkpi.as_unicode(init_template))

	with open(os.path.join(dest, hooks.app_name, "MANIFEST.in"), "w") as f:
		f.write(capkpi.as_unicode(manifest_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, ".gitignore"), "w") as f:
		f.write(capkpi.as_unicode(gitignore_template.format(app_name=hooks.app_name)))

	with open(os.path.join(dest, hooks.app_name, "requirements.txt"), "w") as f:
		f.write("# capkpi -- https://github.com/capkpi/capkpi is installed via 'bench init'")
	...
	with open(os.path.join(dest, hooks.app_name, "license.txt"), "w") as f:
		f.write(capkpi.as_unicode("License: " + hooks.app_license))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "modules.txt"), "w") as f:
		f.write(capkpi.as_unicode(hooks.app_title))
	...
	with open(os.path.join(dest, hooks.app_name, "setup.py"), "w") as f:
		f.write(capkpi.as_unicode(setup_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "hooks.py"), "w") as f:
		f.write(capkpi.as_unicode(hooks_template.format(**hooks)))
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "config", "desktop.py"), "w") as f:
		f.write(capkpi.as_unicode(desktop_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "config", "docs.py"), "w") as f:
		f.write(capkpi.as_unicode(docs_template.format(**hooks)))
...
# Desk Notifications
# ------------------
# See capkpi.core.notifications.get_notification_config
...
#	"capkpi.desk.doctype.event.event.get_events": "{app_name}.event.get_events"
# }}
#
# each overriding function accepts a `data` argument;
# generated from the base implementation of the doctype dashboard,
# along with any modifications made in other CapKPI apps
...
desktop_template = """from capkpi import _
