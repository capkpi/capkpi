...
def main():
	...
	commands.update({"get-capkpi-commands": get_capkpi_commands, "get-capkpi-help": get_capkpi_help})
	click.Group(commands=commands)(prog_name="bench")


def get_app_groups():
	"""Get all app groups, put them in main group "capkpi" since bench is
	...
	ret = dict(capkpi=click.group(name="capkpi", commands=commands)(app_group))
	return ret
...
@click.command("get-capkpi-commands")
def get_capkpi_commands():
	commands = list(get_app_commands("capkpi"))
...
@click.command("get-capkpi-help")
def get_capkpi_help():
	print(click.Context(get_app_groups()["capkpi"]).get_help())


def get_apps():
	return capkpi.get_all_apps(with_internal_apps=False, sites_path=".")


if __name__ == "__main__":
	if not capkpi._dev_server:
