...
from capkpi.utils import cint, get_file_size, get_url, now, now_datetime
...
class BackupGenerator:
	...
	def __init__(
		...
		site = capkpi.local.site or capkpi.generate_hash(length=8)
	...
	def setup_backup_tables(self):
		...
		existing_doctypes = set([x.name for x in capkpi.get_all("DocType")])
		...
		specified_tables = get_tables(capkpi.conf.get("backup", {}).get("includes", []))
		include_tables = (specified_tables + base_tables) if specified_tables else []

		conf_tables = {
			"include": include_tables,
			"exclude": get_tables(capkpi.conf.get("backup", {}).get("excludes", [])),
		}
	...
	def print_summary(self):
		...
		print("Backup Summary for {0} at {1}".format(capkpi.local.site, now()))
	...
	def backup_files(self):
		for folder in ("public", "private"):
			files_path = capkpi.get_site_path(folder, "files")
	...
	def copy_site_config(self):
		...
		site_config_path = os.path.join(capkpi.get_site_path(), "site_config.json")
	...
	def take_dump(self):
		...
		if not (gzip_exc and db_exc[1]):
			...
			capkpi.throw(
				f"{_exc} not found in PATH! This is required to take a backup.", exc=capkpi.ExecutableNotFound
			)
		db_exc = db_exc[0]

		database_header_content = [
			f"Backup generated by CapKPI {capkpi.__version__} on branch {get_app_branch('capkpi') or 'N/A'}",
			"",
		]

		# escape reserved characters
		args = capkpi._dict(
			[item[0], capkpi.utils.esc(str(item[1]), "$ ")] for item in self.__dict__.copy().items()
		)
		...
		if self.partial:
			...
			database_header_content.extend(
				[
					f"Partial Backup of CapKPI Site {capkpi.local.site}",
		...
		if self.db_type == "postgres":
			...
			if self.backup_includes:
				...
				args["exclude"] = " ".join(
					[
						"--ignore-table='{0}.{1}'".format(capkpi.conf.db_name, table)
		...
		capkpi.utils.execute_in_shell(command, low_priority=True)

	def send_email(self):
		"""
		Sends the link to backup file located at erp/backups
		"""
		from capkpi.email import get_system_managers
...
download only after 24 hours.""" % {
		...
		capkpi.sendmail(recipients=recipient_list, msg=msg, subject=subject)
...
def fetch_latest_backups(partial=False):
	...
	odb = BackupGenerator(
		capkpi.conf.db_name,
		capkpi.conf.db_name,
		capkpi.conf.db_password,
		db_host=capkpi.db.host,
		db_type=capkpi.conf.db_type,
		db_port=capkpi.conf.db_port,
	)
...
def delete_temp_backups(older_than=24):
	...
	"""
	older_than = cint(capkpi.conf.keep_backups_for_hours) or older_than
	backup_path = get_backup_path()
...
def get_backup_path():
	backup_path = capkpi.utils.get_site_path(conf.get("backup_path", "private/backups"))
	return backup_path
