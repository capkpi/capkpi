...
from capkpi.utils import cint, strip_html_tags
...
def reset():
	...
	:return:
	"""
	capkpi.db.sql("DELETE FROM `__global_search`")
...
def get_doctypes_with_global_search(with_child_tables=True):
	...
	def _get():
		...
		for d in capkpi.get_all("DocType", fields=["name", "module"], filters=filters):
			meta = capkpi.get_meta(d.name)
			if len(meta.get_global_search_fields()) > 0:
				global_search_doctypes.append(d)

		installed_apps = capkpi.get_installed_apps()
		module_app = capkpi.local.module_app

		doctypes = [
			d.name
			for d in global_search_doctypes
			if module_app.get(capkpi.scrub(d.module))
			and module_app[capkpi.scrub(d.module)] in installed_apps
		]

		return doctypes

	return capkpi.cache().get_value("doctypes_with_global_search", _get)
...
def rebuild_for_doctype(doctype):
	...
	"""
	if capkpi.local.conf.get("disable_global_search"):
		return

	def _get_filters():
		filters = capkpi._dict({"docstatus": ["!=", 2]})
	...
	if cint(meta.istable) == 1:
		parent_doctypes = capkpi.get_all(
			"DocField",
			fields="parent",
	...
	all_records = capkpi.get_all(doctype, fields=fieldnames, filters=_get_filters())
		...
		if content:
			...
			try:
				if hasattr(get_controller(doctype), "is_website_published") and meta.allow_guest_to_view:
					d = capkpi.get_doc(doctype, doc.name)
			...
			all_contents.append(
				{
					"doctype": capkpi.db.escape(doctype),
					"name": capkpi.db.escape(doc.name),
					"content": capkpi.db.escape(" ||| ".join(content or "")),
					"published": published,
					"title": capkpi.db.escape((title or "")[: int(capkpi.db.VARCHAR_LEN)]),
					"route": capkpi.db.escape((route or "")[: int(capkpi.db.VARCHAR_LEN)]),
				}
			)
	if all_contents:
...
def get_children_data(doctype, meta):
	...
	"""
	all_children = capkpi._dict()
	child_search_fields = capkpi._dict()

	for child in meta.get_table_fields():
		child_meta = capkpi.get_meta(child.options)
		...
		if search_fields:
			...
			child_records = capkpi.get_all(
				child.options, fields=child_fieldnames, filters={"docstatus": ["!=", 1], "parenttype": doctype}
			)

			for record in child_records:
				all_children.setdefault(record.parent, capkpi._dict()).setdefault(child.options, []).append(
					record
				)
...
def update_global_search(doc):
	...
	for field in doc.meta.get_global_search_fields():
		if doc.get(field.fieldname) and field.fieldtype not in capkpi.model.table_fields:
	...
	if content:
		...
		title = (doc.get_title() or "")[: int(capkpi.db.VARCHAR_LEN)]
...
def update_global_search_for_all_web_pages():
	if capkpi.conf.get("disable_global_search"):
		return
...
def get_routes_to_index():
	...
	routes_to_index = []
	for app in apps:
		base = capkpi.get_app_path(app, "www")
		path_to_index = capkpi.get_app_path(app, "www")
...
def add_route_to_global_search(route):
	...
	try:
		...
		sync_value_in_queue(value)
	except (capkpi.PermissionError, capkpi.DoesNotExistError, capkpi.ValidationError, Exception):
		pass
...
def get_formatted_value(value, field):
	...
	if getattr(field, "fieldtype", None) in ["Text", "Text Editor"]:
		value = unescape_html(capkpi.safe_decode(value))
...
def sync_global_search():
	...
	:return:
	"""
	while capkpi.cache().llen("global_search_queue") > 0:
		# rpop to follow FIFO
		# Last one should override all previous contents of same document
		value = json.loads(capkpi.cache().rpop("global_search_queue").decode("utf-8"))
		sync_value(value)
...
def sync_value_in_queue(value):
	try:
		...
		capkpi.cache().lpush("global_search_queue", json.dumps(value))
...
def search(text, start=0, limit=20, doctype=""):
	...
	from capkpi.query_builder.functions import Match
	...
	for doctype in allowed_doctypes:
		for index, r in enumerate(results):
			if r.doctype == doctype and r.rank > 0.0:
				try:
					meta = capkpi.get_meta(r.doctype)
					if meta.image_field:
						r.image = capkpi.db.get_value(r.doctype, r.name, meta.image_field)
				except Exception:
