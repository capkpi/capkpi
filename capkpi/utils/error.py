...
EXCLUDE_EXCEPTIONS = (
	capkpi.AuthenticationError,
	capkpi.CSRFTokenError,  # CSRF covers OAuth too
...
def make_error_snapshot(exception):
	if capkpi.conf.disable_error_snapshot:
		return

	if isinstance(exception, EXCLUDE_EXCEPTIONS):
		return

	logger = capkpi.logger(with_more_info=True)

	try:
		error_id = "{timestamp:s}-{ip:s}-{hash:s}".format(
			timestamp=cstr(datetime.datetime.now()),
			ip=capkpi.local.request_ip or "127.0.0.1",
			hash=capkpi.generate_hash(length=3),
		)
		snapshot_folder = get_error_snapshot_path()
		capkpi.create_folder(snapshot_folder)
		...
		with open(encode(snapshot_file_path), "wb") as error_file:
			error_file.write(encode(capkpi.as_json(snapshot)))
	...
	try:
		...
		for fname in os.listdir(path):
			...
			for field in ["locals", "exception", "frames"]:
				data[field] = capkpi.as_json(data[field])

			doc = capkpi.new_doc("Error Snapshot")
			doc.update(data)
			doc.save()
...
def get_error_snapshot_path():
	return capkpi.get_site_path("error-snapshots")
