...
import capkpi
from capkpi import _, safe_decode
...
def get_change_log(user=None):
	...
	last_known_versions = capkpi._dict(
		json.loads(capkpi.db.get_value("User", user, "last_known_versions") or "{}")
	)
	...
	if "capkpi" in current_versions:
		set_in_change_log("capkpi", current_versions["capkpi"], change_log)

	return change_log


def get_change_log_for_app(app, from_version, to_version):
	change_log_folder = os.path.join(capkpi.get_app_path(app), "change_log")
		...
		if folder in major_version_folders:
			for file in os.listdir(os.path.join(change_log_folder, folder)):
				...
				if from_version < version <= to_version:
					...
					content = capkpi.read_file(file_path)
...
def get_versions():
	...
	Example:
	        ...
	        {
	                "capkpi": {
	                        "title": "CapKPI Framework",
	                        "version": "5.0.0"
	                }
	        }"""
	versions = {}
	for app in capkpi.get_installed_apps(sort=True):
		app_hooks = capkpi.get_hooks(app_name=app)
		versions[app] = {
		...
		try:
			versions[app]["version"] = capkpi.get_attr(app + ".__version__")
...
def check_for_update():
	updates = capkpi._dict(major=[], minor=[], patch=[])
	apps = get_versions()
...
def check_release_on_github(app: str):
	"""
	Check the latest release for a given CapKPI application hosted on Github.

	Args:
	        app (str): The name of the CapKPI application.
...
def add_message_to_redis(update_json):
	...
	user_list = [x.name for x in capkpi.get_all("User", filters={"enabled": True})]
	system_managers = [user for user in user_list if "System Manager" in capkpi.get_roles(user)]
...
def show_update_popup():
	...
	if update_message:
		capkpi.msgprint(update_message, title=_("New updates are available"), indicator="green")
