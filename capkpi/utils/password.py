...
from capkpi.query_builder import Table
from capkpi.utils import cstr, encode
...
def get_decrypted_password(doctype, name, fieldname="password", raise_exception=True):
	...
	if result and result[0][0]:
		...
		capkpi.throw(_("Password not found"), capkpi.AuthenticationError)
...
def set_encrypted_password(doctype, name, pwd, fieldname="password"):
	...
	# TODO: Simplify this via aliasing methods in `capkpi.qb`
	if capkpi.db.db_type == "mariadb":
		query = query.on_duplicate_key_update(Auth.password, Values(Auth.password))
	elif capkpi.db.db_type == "postgres":
		query = query.on_conflict(Auth.doctype, Auth.name, Auth.fieldname).do_update(Auth.password)

	try:
		query.run()

	except capkpi.db.DataError as e:
		if capkpi.db.is_data_too_long(e):
			capkpi.throw(_("Most probably your password is too long."), exc=e)
		raise e
...
def check_password(user, pwd, doctype="User", fieldname="password", delete_tracker_cache=True):
	"""Checks if user and password are correct, else raises capkpi.AuthenticationError"""
	...
	if not result or not passlibctx.verify(pwd, result[0].password):
		raise capkpi.AuthenticationError(_("Incorrect User or Password"))
...
def delete_login_failed_cache(user):
	capkpi.cache().hdel("last_login_tried", user)
	capkpi.cache().hdel("login_failed_count", user)
	capkpi.cache().hdel("locked_account_time", user)
...
def rename_password(doctype, old_name, new_name):
	...
	capkpi.qb.update(Auth).set(Auth.name, new_name).where(
		(Auth.doctype == doctype) & (Auth.name == old_name)
	).run()


def rename_password_field(doctype, old_fieldname, new_fieldname):
	capkpi.qb.update(Auth).set(Auth.fieldname, new_fieldname).where(
...
def decrypt(pwd):
	try:
		...
		capkpi.throw(_("Encryption key is invalid, Please check site_config.json"))


def get_encryption_key():
	from capkpi.installer import update_site_config

	if "encryption_key" not in capkpi.local.conf:
		encryption_key = Fernet.generate_key().decode()
		update_site_config("encryption_key", encryption_key)
		capkpi.local.conf.encryption_key = encryption_key

	return capkpi.local.conf.encryption_key


def get_password_reset_limit():
	return capkpi.db.get_single_value("System Settings", "password_reset_limit") or 0
