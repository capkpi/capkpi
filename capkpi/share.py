...
def add(
	...
	if share_name:
		doc = capkpi.get_doc("DocShare", share_name)
	else:
		doc = capkpi.new_doc("DocShare")
		doc.update(
...
def remove(doctype, name, user, flags=None):
	share_name = capkpi.db.get_value(
		"DocShare", {"user": user, "share_name": name, "share_doctype": doctype}
	)

	if share_name:
		capkpi.delete_doc("DocShare", share_name, flags=flags)
...
def set_permission(doctype, name, user, permission_to, value=1, everyone=0):
	...
	if not share_name:
		...
		share = capkpi.get_doc("DocShare", share_name)
...
def check_share_permission(doctype, name):
	...
	if not capkpi.has_permission(doctype, ptype="share", doc=name):
		capkpi.throw(
			_("No permission to {0} {1} {2}").format("share", _(doctype), name), capkpi.PermissionError
		)
...
def notify_assignment(shared_by, doctype, doc_name, everyone, notify=0):
	...
	reference_user = get_fullname(capkpi.session.user)
	notification_message = _("{0} shared a document {1} {2} with you").format(
		capkpi.bold(reference_user), capkpi.bold(_(doctype)), get_title_html(title)
	)
