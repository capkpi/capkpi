...
# test_records = capkpi.get_test_records('DocType')


class TestDocType(unittest.TestCase):
	def test_validate_name(self):
		self.assertRaises(capkpi.NameError, new_doctype("_Some DocType").insert)
		self.assertRaises(capkpi.NameError, new_doctype("8Some DocType").insert)
		self.assertRaises(capkpi.NameError, new_doctype("Some (DocType)").insert)
		self.assertRaises(
			capkpi.NameError,
			new_doctype("Some Doctype with a name whose length is more than 61 characters").insert,
		)
		for name in ("Some DocType", "Some_DocType"):
			if capkpi.db.exists("DocType", name):
				capkpi.delete_doc("DocType", name)

			doc = new_doctype(name).insert()
			doc.delete()

	def test_doctype_unique_constraint_dropped(self):
		if capkpi.db.exists("DocType", "With_Unique"):
			capkpi.delete_doc("DocType", "With_Unique")

		dt = new_doctype("With_Unique", unique=1)
		dt.insert()

		doc1 = capkpi.new_doc("With_Unique")
		doc2 = capkpi.new_doc("With_Unique")
		...
		doc1.insert()
		self.assertRaises(capkpi.UniqueValidationError, doc2.insert)
	...
	def test_data_field_options(self):
		...
		valid_data_field_options = capkpi.model.data_field_options + ("",)
		invalid_data_field_options = ("Invalid Option 1", capkpi.utils.random_string(5))

		for field_option in valid_data_field_options + invalid_data_field_options:
			test_doctype = capkpi.get_doc(
				{
					"doctype": "DocType",
					"name": doctype_name,
					"module": "Core",
			...
			if field_option in invalid_data_field_options:
				# assert that only data options in capkpi.model.data_field_options are valid
				self.assertRaises(capkpi.ValidationError, test_doctype.insert)
			else:
				test_doctype.insert()
	...
	def test_sync_field_order(self):
		...
		capkpi.delete_doc_if_exists("DocType", "Test Field Order DocType")
		if os.path.isfile(path):
			os.remove(path)

		try:
			capkpi.flags.allow_doctype_export = 1
			test_doctype.save()

			# assert that field_order list is being created with the default order
			test_doctype_json = capkpi.get_file_json(path)
			...
			with open(path, "w+") as txtfile:
				txtfile.write(capkpi.as_json(test_doctype_json))
			...
			capkpi.reload_doctype(test_doctype.name, force=True)
			test_doctype.reload()
			...
			capkpi.flags.allow_doctype_export = 0
	...
	def test_cancel_link_doctype(self):
		...
		from capkpi.desk.form.linked_with import cancel_all_linked_docs, get_submitted_linked_docs
		...
		# create doctype data
		data_link_doc = capkpi.new_doc("Test Linked Doctype")
		data_link_doc.some_fieldname = "Data1"
		data_link_doc.insert()
		data_link_doc.save()
		data_link_doc.submit()

		data_doc = capkpi.new_doc("Test Doctype")
	...
	def test_ignore_cancelation_of_linked_doctype_during_cancel(self):
		...
		data_doc = capkpi.new_doc("Test Doctype 2")
		data_doc.some_fieldname = "Data1"
		...
		self.assertRaises(capkpi.LinkExistsError, data_link_doc_1.cancel)
	...
	def test_create_virtual_doctype(self):
		...
		virtual_doc.insert()
		virtual_doc.save()
		doc = capkpi.get_doc("DocType", "Test Virtual Doctype")

		self.assertEqual(doc.is_virtual, 1)
		self.assertFalse(capkpi.db.table_exists("Test Virtual Doctype"))
