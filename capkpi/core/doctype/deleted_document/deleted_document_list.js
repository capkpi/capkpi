capkpi.listview_settings["Deleted Document"] = {
	onload: function (doclist) {
		const action = () => {
			...
			if (selected_docs.length > 0) {
				...
				capkpi.call({
					method: "capkpi.core.doctype.deleted_document.deleted_document.bulk_restore",
					args: { docnames },
					callback: function (r) {
						if (r.message) {
							let body = (docnames) => {
							...
							capkpi.msgprint(summary, __("Document Restoration Summary"), true);
