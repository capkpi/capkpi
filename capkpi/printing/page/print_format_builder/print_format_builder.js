capkpi.pages['print-format-builder'].on_page_load = function(wrapper) {
	capkpi.print_format_builder = new capkpi.PrintFormatBuilder(wrapper);
	capkpi.breadcrumbs.add("Setup", "Print Format");
}

capkpi.pages['print-format-builder'].on_page_show = function(wrapper) {
	var route = capkpi.get_route();
	if(route.length>1) {
		capkpi.model.with_doc('Print Format', route[1], function() {
			capkpi.print_format_builder.print_format = capkpi.get_doc('Print Format', route[1]);
			capkpi.print_format_builder.refresh();
		});
	} else if(capkpi.route_options) {
		if(capkpi.route_options.make_new) {
			let { doctype, name, based_on } = capkpi.route_options;
			capkpi.route_options = null;
			capkpi.print_format_builder.setup_new_print_format(doctype, name, based_on);
		} else {
			capkpi.print_format_builder.print_format = capkpi.route_options.doc;
			capkpi.route_options = null;
			capkpi.print_format_builder.refresh();
		}
	}
...
capkpi.PrintFormatBuilder = Class.extend({
	...
	make: function() {
		...
		this.page.main = $('<div class="col-md-12 border print-format-builder-main capkpi-card"></div>').appendTo(this.page.main);
	...
	show_start: function() {
		this.page.main.html(capkpi.render_template("print_format_builder_start", {}));
	...
	start_edit_print_format: function() {
		...
		var me = this;
		this.print_format_input = capkpi.ui.form.make_control({
		...
		this.page.main.find(".btn-edit-print-format").on("click", function() {
			...
			if(!name) return;
			capkpi.model.with_doc("Print Format", name, function(doc) {
				capkpi.set_route('print-format-builder', name);
			});
		});
	},
	start_new_print_format: function() {
		var me = this;
		this.doctype_input = capkpi.ui.form.make_control({
		...
		this.name_input = capkpi.ui.form.make_control({
		...
		this.page.main.find(".btn-new-print-format").on("click", function() {
			...
			if(!(doctype && name)) {
				capkpi.msgprint(__("Both DocType and Name required"));
				return;
			}
	...
	setup_new_print_format: function(doctype, name, based_on) {
		capkpi.call({
			method: 'capkpi.printing.page.print_format_builder.print_format_builder.create_custom_format',
			args: {
				doctype: doctype,
				name: name,
				based_on: based_on
	...
	setup_print_format: function() {
		var me = this;
		capkpi.model.with_doctype(this.print_format.doc_type, function(doctype) {
			me.meta = capkpi.get_meta(me.print_format.doc_type);
			me.setup_sidebar();
			me.render_layout();
			...
			me.page.add_inner_button(__("Edit Properties"), function() {
				capkpi.set_route("Form", "Print Format", me.print_format.name);
			});
		});
	},
	setup_sidebar: function() {
		...
		this.page.sidebar.html(
			$(capkpi.render_template("print_format_builder_sidebar", {fields: fields}))
		);
	...
	render_layout: function() {
		...
		this.prepare_data();
		$(capkpi.render_template("print_format_builder_layout", {
		...
		$.each(this.data, function(i, f) {
			...
			if(!f.name && f.fieldname) {
				...
				if(f.fieldname==="_custom_html") {
					...
					f = $.extend(capkpi.meta.get_docfield(me.print_format.doc_type,
						f.fieldname) || {}, f);
				}
			}
	...
	init_visible_columns: function(f) {
		f.visible_columns = []
		$.each(capkpi.get_meta(f.options).fields, function(i, _f) {
			...
			onAdd: function(evt) {
				...
				if(!$item.hasClass("print-format-builder-field")) {
					...
					if(fieldname==="_custom_html") {
						...
						var field = capkpi.meta.get_docfield(me.print_format.doc_type,
							fieldname);
					}

					var html = capkpi.render_template("print_format_builder_field",
						{field: field, me:me});
	...
	setup_add_section: function() {
		...
		this.page.main.find(".print-format-builder-add-section").on("click", function() {
			...
			var $section = $(capkpi.render_template("print_format_builder_section",
	...
	setup_column_selector: function() {
		...
		this.page.main.on("click", ".select-columns", function() {
			...
			var doc_fields = capkpi.get_meta(doctype).fields;
			...
			// render checkboxes
			$(capkpi.render_template("print_format_builder_column_selector", {
				fields: fields,
