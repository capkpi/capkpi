...
from capkpi.twofactor import PARENT_FOR_DEFAULTS
from capkpi.utils.password import encrypt
...
def execute():
	table = capkpi.qb.DocType(DOCTYPE)

	# set parent for `*_otplogin`
	(
		capkpi.qb.update(table)
	...
	secrets = {
		key: value
		for key, value in capkpi.defaults.get_defaults_for(parent=OLD_PARENT).items()
	...
	defvalue_cases = capkpi.qb.terms.Case()
