...
def execute():
	if (
		capkpi.db.count("File", filters={"attached_to_doctype": "Prepared Report", "is_private": 0})
		> 10000
	):
	...
	for file_dict in files:
		...
		if capkpi.db.exists("Prepared Report", file_dict.attached_to_name):
			try:
				file_doc = capkpi.get_doc("File", file_dict.name)
				file_doc.is_private = 1
				file_doc.save()
			except Exception:
				# File might not exist on the file system in that case delete both Prepared Report and File doc
				capkpi.delete_doc("Prepared Report", file_dict.attached_to_name)
		else:
			# If Prepared Report doc doesn't exist then the file doc is useless. Delete it.
			capkpi.delete_doc("File", file_dict.name)
