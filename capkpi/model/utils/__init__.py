...
def set_default(doc, key):
	...
	if not doc.is_default:
		capkpi.db.set(doc, "is_default", 1)
...
def set_field_property(filters, key, value):
	...
	docs = [
		capkpi.get_doc("DocType", d.parent)
		for d in capkpi.get_all("DocField", fields=["parent"], filters=filters)
	]
...
class InvalidIncludePath(capkpi.ValidationError):
	pass
...
def render_include(content):
	...
	for i in range(5):
		if "{% include" in content:
			...
			if not paths:
				capkpi.throw(_("Invalid include path"), InvalidIncludePath)

			for path in paths:
				app, app_path = path.split("/", 1)
				with io.open(capkpi.get_app_path(app, app_path), "r", encoding="utf-8") as f:
					include = f.read()
