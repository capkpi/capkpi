...
def check_admin_or_system_manager(user=None):
	...
	if ("System Manager" not in capkpi.get_roles(user)) and (user != "Administrator"):
		capkpi.throw(_("Not permitted"), capkpi.PermissionError)


def print_has_permission_check_logs(func):
	def inner(*args, **kwargs):
		capkpi.flags["has_permission_check_logs"] = []
		result = func(*args, **kwargs)
		self_perm_check = True if not kwargs.get("user") else kwargs.get("user") == capkpi.session.user
		...
		if not result and self_perm_check and raise_exception:
			msgprint(("<br>").join(capkpi.flags.get("has_permission_check_logs", [])))
		capkpi.flags.pop("has_permission_check_logs", None)
		return result
...
def has_permission(
	...
	if doc:
		if isinstance(doc, string_types):
			doc = capkpi.get_doc(meta.name, doc)
		perm = get_doc_permissions(doc, user=user, ptype=ptype).get(ptype)
		if not perm:
			push_perm_check_log(
				_("User {0} does not have access to this document").format(capkpi.bold(user))
			)
	else:
		...
		if not perm:
			push_perm_check_log(
				_("User {0} does not have doctype access via role permission for document {1}").format(
					capkpi.bold(user), capkpi.bold(doctype)
				)
			)

	def false_if_not_shared():
		if ptype in ("read", "write", "share", "submit", "email", "print"):
			shared = capkpi.share.get_shared(
...
def get_doc_permissions(doc, user=None, ptype=None):
	...
	if capkpi.is_table(doc.doctype):
		return {"read": 1, "write": 1}

	meta = capkpi.get_meta(doc.doctype)

	def is_user_owner():
		return (doc.get("owner") or "").lower() == capkpi.session.user.lower()
...
def get_role_permissions(doctype_meta, user=None, is_owner=None):
	...
	if isinstance(doctype_meta, string_types):
		doctype_meta = capkpi.get_meta(doctype_meta)  # assuming doctype name was passed
	...
	if not capkpi.local.role_permissions.get(cache_key):
		perms = capkpi._dict(if_owner={})
		...
		capkpi.local.role_permissions[cache_key] = perms

	return capkpi.local.role_permissions[cache_key]


def get_user_permissions(user):
	from capkpi.core.doctype.user_permission.user_permission import get_user_permissions
...
def has_user_permission(doc, user=None):
	...
	def check_user_permission_on_link_fields(d):
		...
		meta = capkpi.get_meta(d.get("doctype"))
...
def has_controller_permissions(doc, ptype, user=None):
	...
	methods = capkpi.get_hooks("has_permission").get(doc.doctype, [])

	if not methods:
		return None

	for method in reversed(methods):
		controller_permission = capkpi.call(capkpi.get_attr(method), doc=doc, ptype=ptype, user=user)
...
def get_all_perms(role):
	...
	perms = capkpi.get_all("DocPerm", fields="*", filters=dict(role=role))
	custom_perms = capkpi.get_all("Custom DocPerm", fields="*", filters=dict(role=role))
	doctypes_with_custom_perms = capkpi.db.sql_list(
...
def get_roles(user=None, with_standard=True):
	...
	def get():
		if user == "Administrator":
			return [r[0] for r in capkpi.db.sql("select name from `tabRole`")]  # return all available roles
		else:
			return (
				[
					r[0]
	...
	roles = capkpi.cache().hget("roles", user, get)
...
def get_perms_for(roles, perm_doctype="DocPerm"):
	...
	return capkpi.db.get_all(perm_doctype, fields=["*"], filters=filters)


def get_doctypes_with_custom_docperms():
	"""Returns all the doctypes with Custom Docperms"""

	doctypes = capkpi.db.get_all("Custom DocPerm", fields=["parent"], distinct=1)
...
def set_user_permission_if_allowed(doctype, name, user, with_message=False):
	if get_role_permissions(capkpi.get_meta(doctype), user).set_user_permissions != 1:
...
def add_user_permission(
	...
	"""Add user permission"""
	from capkpi.core.doctype.user_permission.user_permission import user_permission_exists

	if not user_permission_exists(user, doctype, name, applicable_for):
		if not capkpi.db.exists(doctype, name):
			capkpi.throw(_("{0} {1} not found").format(_(doctype), name), capkpi.DoesNotExistError)
...
def remove_user_permission(doctype, name, user):
	user_permission_name = capkpi.db.get_value(
		"User Permission", dict(user=user, allow=doctype, for_value=name)
	)
	capkpi.delete_doc("User Permission", user_permission_name)
...
def clear_user_permissions_for_doctype(doctype, user=None):
	...
	if user:
		filters["user"] = user
	user_permissions_for_doctype = capkpi.db.get_all("User Permission", filters=filters)
...
def can_import(doctype, raise_exception=False):
	if not ("System Manager" in capkpi.get_roles() or has_permission(doctype, "import")):
		if raise_exception:
			raise capkpi.PermissionError("You are not allowed to import: {doctype}".format(doctype=doctype))
		else:
			return False
	return True
...
def can_export(doctype, raise_exception=False):
	if "System Manager" in capkpi.get_roles():
		return True
	else:
		role_permissions = capkpi.permissions.get_role_permissions(doctype)
		has_access = role_permissions.get("export") or role_permissions.get("if_owner").get("export")
		if not has_access and raise_exception:
			raise capkpi.PermissionError(_("You are not allowed to export {} doctype").format(doctype))
		return has_access


def update_permission_property(doctype, role, permlevel, ptype, value=None, validate=True):
	"""Update a property in Custom Perm"""
	from capkpi.core.doctype.doctype.doctype import validate_permissions_for_doctype
	...
	capkpi.qb.update(table).set(ptype, value).where(table.name == name).run()
...
def setup_custom_perms(parent):
	...
	if not capkpi.db.exists("Custom DocPerm", dict(parent=parent)):
		copy_perms(parent)
		return True
...
def copy_perms(parent):
	...
	for d in capkpi.get_all("DocPerm", fields="*", filters=dict(parent=parent)):
		custom_perm = capkpi.new_doc("Custom DocPerm")
...
def reset_perms(doctype):
	...
	from capkpi.desk.notifications import delete_notification_count_for

	delete_notification_count_for(doctype)

	capkpi.db.sql("""delete from `tabCustom DocPerm` where parent=%s""", doctype)
...
def get_linked_doctypes(dt):
	return list(
		set(
			...
			+ [
				d.options
				for d in capkpi.get_meta(dt).get(
					"fields",
...
def push_perm_check_log(log):
	if capkpi.flags.get("has_permission_check_logs") == None:
		return
	capkpi.flags.get("has_permission_check_logs").append(_(log))
