...
class ParallelTestRunner:
	def __init__(self, app, site, build_number=1, total_builds=1, with_coverage=False):
		...
		self.build_number = capkpi.utils.cint(build_number) or 1
		self.total_builds = capkpi.utils.cint(total_builds)
		self.setup_test_site()
		self.run_tests()

	def setup_test_site(self):
		capkpi.init(site=self.site)
		if not capkpi.db:
			capkpi.connect()
	...
	def before_test_setup(self):
		...
		for fn in capkpi.get_hooks("before_tests", app_name=self.app):
			capkpi.get_attr(fn)()

		test_module = capkpi.get_module(f"{self.app}.tests")
	...
	def get_module(self, path, filename):
		app_path = capkpi.get_pymodule_path(self.app)
	...
	def get_test_file_list(self):
		...
		split_size = capkpi.utils.ceil(len(test_list) / self.total_builds)
...
class ParallelTestWithOrchestrator(ParallelTestRunner):
	...
	def __init__(self, app, site, with_coverage=False):
		...
		if not self.orchestrator_url:
			...
			click.echo("Pass public URL after hosting https://github.com/capkpi/test-orchestrator")
			sys.exit(1)

		self.ci_build_id = os.environ.get("CI_BUILD_ID")
		self.ci_instance_id = os.environ.get("CI_INSTANCE_ID") or capkpi.generate_hash(length=10)
