...
capkpi.ui.form.on("Contact", {
	...
	refresh: function(frm) {
		...
		if(!frm.doc.user && !frm.is_new() && frm.perm[0].write) {
			frm.add_custom_button(__("Invite as User"), function() {
				return capkpi.call({
					method: "capkpi.contacts.doctype.contact.contact.invite_user",
					args: {
		...
		if (numbers && numbers.length && capkpi.phone_call.handler) {
			frm.add_custom_button(__('Call'), () => {
				numbers = frm.doc.phone_nos
					.sort((prev, next) => next.is_primary_mobile_no - prev.is_primary_mobile_no)
					.map(d => d.phone);
				capkpi.phone_call.handler(numbers);
			});
		}

		if (frm.doc.links) {
			capkpi.call({
				method: "capkpi.contacts.doctype.contact.contact.address_query",
	...
	sync_with_google_contacts: function(frm) {
		if (frm.doc.sync_with_google_contacts) {
			capkpi.db.get_value("Google Contacts", {"email_id": capkpi.session.user}, "name", (r) => {
				if (r && r.name) {
...
capkpi.ui.form.on("Dynamic Link", {
	link_name:function(frm, cdt, cdn){
		var child = locals[cdt][cdn];
		if(child.link_name) {
			capkpi.model.with_doctype(child.link_doctype, function () {
				var title_field = capkpi.get_meta(child.link_doctype).title_field || "name"
				capkpi.model.get_value(child.link_doctype, child.link_name, title_field, function (r) {
					capkpi.model.set_value(cdt, cdn, "link_title", r[title_field])
				})
			})
		}
	}
