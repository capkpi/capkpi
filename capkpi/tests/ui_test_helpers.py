...
def create_if_not_exists(doc):
	...
	if not capkpi.local.dev_server:
		capkpi.throw(_("This method can only be accessed in development"), capkpi.PermissionError)
	...
	for doc in docs:
		...
		filters = doc.copy()
		filters.pop("doctype")
		name = capkpi.db.exists(doc.doctype, filters)
		if not name:
			d = capkpi.get_doc(doc)
...
def create_todo_records():
	if capkpi.db.get_all("ToDo", {"description": "this is first todo"}):
		return
	...
	doc = capkpi.get_doc(
		{
			...
			"communication_date": capkpi.utils.now_datetime(),
		}
	)
	doc.insert()
	return doc
...
@capkpi.whitelist()
def setup_workflow():
	from capkpi.workflow.doctype.workflow.test_workflow import create_todo_workflow
...
def create_contact_phone_nos_records():
	if capkpi.db.get_all("Contact", {"first_name": "Test Contact"}):
		return

	doc = capkpi.new_doc("Contact")
...
def create_contact_records():
	if capkpi.db.get_all("Contact", {"first_name": "Test Form Contact 1"}):
		return
...
def create_multiple_todo_records():
	if capkpi.db.get_all("ToDo", {"description": "Multiple ToDo 1"}):
		return

	values = [("100{}".format(i), "Multiple ToDo {}".format(i)) for i in range(1, 1002)]

	capkpi.db.bulk_insert("ToDo", fields=["name", "description"], values=set(values))


def insert_contact(first_name, phone_number):
	doc = capkpi.get_doc({"doctype": "Contact", "first_name": first_name})
...
def create_form_tour():
	if capkpi.db.exists("Form Tour", {"name": "Test Form Tour"}):
		return

	def get_docfield_name(filters):
		return capkpi.db.get_value("DocField", filters, "name")
...
def create_web_page():
	web_page = capkpi.db.exists("Web Page", {"route": "test-page-discussions"})
	if not web_page:
		...
		web_page.append(
			"page_blocks",
			{
				...
				"web_template_values": capkpi.as_json(
...
def update_webform_to_multistep():
	if not capkpi.db.exists("Web Form", "update-profile-duplicate"):
		doc = capkpi.get_doc("Web Form", "edit-profile")
		_doc = capkpi.copy_doc(doc)
