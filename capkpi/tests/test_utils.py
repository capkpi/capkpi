...
from capkpi.utils.dateutils import get_dates_from_timegrain
	...
	def test_scrub_urls(self):
		html = """
			...
			<div class="more-info">
				...
				<img src="/assets/capkpi/test.jpg">
			</div>
			<div style="background-image: url('/assets/capkpi/bg.jpg')">
		...
		self.assertTrue('<img src="{0}/assets/capkpi/test.jpg">'.format(url) in html)
		self.assertTrue(
			"style=\"background-image: url('{0}/assets/capkpi/bg.jpg') !important\"".format(url) in html
		)
...
class TestValidationUtils(unittest.TestCase):
	def test_valid_url(self):
		...
		self.assertTrue(validate_url("http://capkpi.com", throw=True))
		...
		self.assertRaises(capkpi.ValidationError, validate_url, "capkpi", throw=True)

		# Scheme validation
		self.assertFalse(validate_url("https://google.com", valid_schemes="http"))
		self.assertTrue(validate_url("ftp://capkpi.cloud", valid_schemes=["https", "ftp"]))
		self.assertFalse(
			validate_url("bolo://capkpi.com", valid_schemes=("http", "https", "ftp", "ftps"))
		)
		self.assertRaises(
			capkpi.ValidationError, validate_url, "gopher://capkpi.com", valid_schemes="https", throw=True
		)
	...
	def test_valid_email(self):
		...
		# Valid addresses
		self.assertTrue(validate_email_address("someone@capkpi.com"))
		self.assertTrue(validate_email_address("someone@capkpi.com, anyone@capkpi.com"))
		...
		# Invalid with throw
		self.assertRaises(
			capkpi.InvalidEmailAddressError, validate_email_address, "someone.com", throw=True
		)


class TestImage(unittest.TestCase):
	def test_strip_exif_data(self):
		original_image = Image.open("../apps/capkpi/capkpi/tests/data/exif_sample_image.jpg")
		original_image_content = io.open(
			"../apps/capkpi/capkpi/tests/data/exif_sample_image.jpg", mode="rb"
		).read()
...
class TestPythonExpressions(unittest.TestCase):
	...
	def test_validation_for_bad_python_expression(self):
		...
		for expr in invalid_expressions:
			self.assertRaises(capkpi.ValidationError, validate_python_code, expr)
...
class TestDateUtils(unittest.TestCase):
	def test_first_day_of_week(self):
		...
		with patch.object(capkpi.utils.data, "get_first_day_of_the_week", return_value="Monday"):
			self.assertEqual(
				capkpi.utils.get_first_day_of_week("2020-12-25"), capkpi.utils.getdate("2020-12-21")
			)
			self.assertEqual(
				capkpi.utils.get_first_day_of_week("2020-12-20"), capkpi.utils.getdate("2020-12-14")
			)

		# Sunday as start of the week
		self.assertEqual(
			capkpi.utils.get_first_day_of_week("2020-12-25"), capkpi.utils.getdate("2020-12-20")
		)
		self.assertEqual(
			capkpi.utils.get_first_day_of_week("2020-12-21"), capkpi.utils.getdate("2020-12-20")
		)

	def test_last_day_of_week(self):
		self.assertEqual(
			capkpi.utils.get_last_day_of_week("2020-12-24"), capkpi.utils.getdate("2020-12-26")
		)
		self.assertEqual(
			capkpi.utils.get_last_day_of_week("2020-12-28"), capkpi.utils.getdate("2021-01-02")
		)
...
class TestResponse(unittest.TestCase):
	def test_json_handler(self):
		...
		GOOD_OBJECT = {
			...
			"doc": [
				capkpi.get_doc("System Settings"),
			],
			"iter": [
				{1, 2, 3},
				(1, 2, 3),
