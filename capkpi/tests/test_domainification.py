...
from capkpi.core.page.permission_manager.permission_manager import get_roles_and_doctypes
from capkpi.desk.doctype.desktop_icon.desktop_icon import (
	add_user_icon,
...
class TestDomainification(unittest.TestCase):
	...
	def tearDown(self):
		capkpi.db.sql("delete from tabRole where name='_Test Role'")
		capkpi.db.sql("delete from `tabHas Role` where role='_Test Role'")
		capkpi.db.sql("delete from tabDomain where name in ('_Test Domain 1', '_Test Domain 2')")
		capkpi.delete_doc("DocType", "Test Domainification")
		self.remove_from_active_domains(remove_all=True)
	...
	def add_active_domain(self, domain):
		...
		domain_settings = capkpi.get_doc("Domain Settings", "Domain Settings")
	...
	def new_domain(self, domain):
		# create new domain
		capkpi.get_doc({"doctype": "Domain", "domain": domain}).insert()
	...
	def test_active_domains(self):
		self.assertTrue("_Test Domain 1" in capkpi.get_active_domains())
		self.assertFalse("_Test Domain 2" in capkpi.get_active_domains())

		self.add_active_domain("_Test Domain 2")
		self.assertTrue("_Test Domain 2" in capkpi.get_active_domains())

		self.remove_from_active_domains("_Test Domain 1")
		self.assertTrue("_Test Domain 1" not in capkpi.get_active_domains())
	...
	def test_doctype_and_role_domainification(self):
		...
		test_role = capkpi.get_doc({"doctype": "Role", "role_name": "_Test Role"}).insert()
	...
	def test_module_def_for_domainification(self):
		...
		test_module_def = capkpi.get_doc("Module Def", "Contacts")
