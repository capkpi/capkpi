...
import capkpi
from capkpi.app import process_response
...
class TestCORS(unittest.TestCase):
	def make_request_and_test(self, origin="http://example.com", absent=False):
		...
		capkpi.utils.set_request(headers=headers)
	...
	def test_valid_origin(self):
		...
		capkpi.conf.allow_cors = "*"
		self.make_request_and_test()

		capkpi.conf.allow_cors = ["http://example.com", "https://example.com"]
		self.make_request_and_test()

	def test_invalid_origin(self):
		capkpi.conf.allow_cors = "http://example1.com"
		self.make_request_and_test(absent=True)

		capkpi.conf.allow_cors = ["http://example1.com", "https://example.com"]
		self.make_request_and_test(absent=True)
