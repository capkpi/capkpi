...
import capkpi
from capkpi.auth import HTTPRequest, get_login_attempt_tracker, validate_ip_address
...
class TestTwoFactor(unittest.TestCase):
	...
	def setUp(self):
		...
		self.login_manager = capkpi.local.login_manager
		self.user = self.login_manager.user
		update_system_settings({"allow_consecutive_login_attempts": 2})

	def tearDown(self):
		capkpi.local.response["verification"] = None
		capkpi.local.response["tmp_id"] = None
		disable_2fa()
		capkpi.clear_cache(user=self.user)
	...
	def test_authenticate_for_2factor(self):
		"""Verification obj and tmp_id should be set in capkpi.local."""
		authenticate_for_2factor(self.user)
		verification_obj = capkpi.local.response["verification"]
		tmp_id = capkpi.local.response["tmp_id"]
		self.assertTrue(verification_obj)
		self.assertTrue(tmp_id)
		for k in ["_usr", "_pwd", "_otp_secret"]:
			self.assertTrue(capkpi.cache().get("{0}{1}".format(tmp_id, k)), "{} not available".format(k))
	...
	def test_two_factor_is_enabled(self):
		...
		user.restrict_ip = capkpi.local.request_ip
		user.save()
		self.assertTrue(should_run_2fa(self.user))
	...
	def test_confirm_otp_token(self):
		...
		capkpi.flags.otp_expiry = 2
		authenticate_for_2factor(self.user)
		tmp_id = capkpi.local.response["tmp_id"]
		otp = "wrongotp"
		with self.assertRaises(capkpi.AuthenticationError):
		...
		capkpi.flags.otp_expiry = None
		if capkpi.flags.tests_verbose:
	...
	def test_render_string_template(self):
		...
		args = {"issuer_name": "CapKPI Technologies"}
		_str = "Verification Code from {{issuer_name}}"
		_str = capkpi.render_template(_str, args)
		self.assertEqual(_str, "Verification Code from CapKPI Technologies")
	...
	def test_otp_attempt_tracker(self):
		...
		with self.assertRaises(capkpi.SecurityException):
			confirm_otp_token(self.login_manager, otp=otp, tmp_id=tmp_id)
...
def create_http_request():
	...
	enable_2fa()
	capkpi.form_dict["usr"] = "test@example.com"
	capkpi.form_dict["pwd"] = "Eastern_43A1W"
	capkpi.local.form_dict["cmd"] = "login"
...
def toggle_2fa_all_role(state=None):
	...
	all_role = capkpi.get_doc("Role", "All")
	if state == None:
