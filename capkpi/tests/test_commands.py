...
from capkpi.installer import add_to_installed_apps, remove_app
from capkpi.utils import add_to_date, get_bench_relative_path, now
from capkpi.utils.backups import fetch_latest_backups


# TODO: check capkpi.cli.coloured_output to set coloured output!
def supports_color():
	"""
...
def exists_in_backup(doctypes, file):
	...
	"""
	predicate = 'COPY public."tab{}"' if capkpi.conf.db_type == "postgres" else "CREATE TABLE `tab{}`"
...
class TestCommands(BaseTestCommands):
	def test_execute(self):
		...
		self.execute("bench --site {site} execute capkpi.db.get_database_size")
		self.assertEquals(self.returncode, 0)
		self.assertIsInstance(float(self.stdout), float)

		# test 2: execute a command expecting an errored output as local won't exist
		self.execute("bench --site {site} execute capkpi.local.site")
		...
		self.execute("""bench --site {site} execute capkpi.bold --kwargs '{{"text": "DocType"}}'""")
		self.assertEquals(self.returncode, 0)
		self.assertEquals(self.stdout[1:-1], capkpi.bold(text="DocType"))
	...
	def test_backup(self):
		...
		site_backup_path = capkpi.utils.get_site_path("private", "backups")
		...
		# test 7: take a backup with capkpi.conf.backup.includes
		self.execute(
		...
		# test 8: take a backup with capkpi.conf.backup.excludes
		self.execute(
		...
		# test 9: take a backup with --include (with capkpi.conf.excludes still set)
		self.execute(
	...
	def test_restore(self):
		...
		global_config = {
			"admin_password": capkpi.conf.admin_password,
			"root_login": capkpi.conf.root_login,
			"root_password": capkpi.conf.root_password,
			"db_type": capkpi.conf.db_type,
		}
		site_data = {"another_site": f"{capkpi.local.site}-restore.test", **global_config}
		...
		self.execute(
			"bench --site {another_site} execute capkpi.utils.backups.fetch_latest_backups",
			site_data,
		)
		...
		self.execute(
			...
			" capkpi.utils.backups.fetch_latest_backups --kwargs {kw}",
			site_data,
		)
		site_data.update({"database": json.loads(self.stdout)["database"]})
		self.execute("bench --site {another_site} restore {database}", site_data)
	...
	def test_partial_restore(self):
			...
			capkpi.get_doc(
				{
					...
					"description": capkpi.mock("paragraph"),
				}
			).insert()
		capkpi.db.commit()
		todo_count = capkpi.db.count("ToDo")
		...
		capkpi.db.sql_ddl("DROP TABLE IF EXISTS `tabToDo`")
		capkpi.db.commit()

		self.execute("bench --site {site} partial-restore {path}", {"path": db_path})
		self.assertEquals(self.returncode, 0)
		self.assertEquals(capkpi.db.count("ToDo"), todo_count)
	...
	def test_recorder(self):
		...
		capkpi.local.cache = {}
		self.assertEqual(capkpi.recorder.status(), True)

		self.execute("bench --site {site} stop-recording")
		capkpi.local.cache = {}
		self.assertEqual(capkpi.recorder.status(), False)
	...
	def test_list_apps(self):
		...
		doctype = capkpi.get_single("Installed Applications").installed_applications
		if doctype:
			installed_apps = set([x.app_name for x in doctype])
		else:
			installed_apps = set(capkpi.get_installed_apps())
...
class RemoveAppUnitTests(unittest.TestCase):
	def test_delete_modules(self):
		...
		test_module = capkpi.new_doc("Module Def")

		test_module.update({"module_name": "RemoveThis", "app_name": "capkpi"})
		test_module.save()

		module_def_linked_doctype = capkpi.get_doc(
			{
				"doctype": "DocType",
		...
		self.assertFalse(capkpi.db.exists("Module Def", test_module.module_name))
		self.assertFalse(capkpi.db.exists("DocType", module_def_linked_doctype.name))

	def test_dry_run(self):
		"""Check if dry run in not destructive."""

		# nothing to assert, if this fails rest of the test suite will crumble.
		remove_app("capkpi", dry_run=True, yes=True, no_backup=True)
...
class TestAddNewUser(BaseTestCommands):
	def test_create_user(self):
		...
		capkpi.db.rollback()
		self.assertEqual(self.returncode, 0)
		user = capkpi.get_doc("User", "test@gmail.com")
