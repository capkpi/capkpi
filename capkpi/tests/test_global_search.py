...
from capkpi.utils import global_search
...
class TestGlobalSearch(unittest.TestCase):
	def setUp(self):
		...
		self.assertTrue("__global_search" in capkpi.db.get_tables())
		doctype = "Event"
		global_search.reset()
	...
	def tearDown(self):
		capkpi.db.sql("DELETE FROM `tabProperty Setter` WHERE `doc_type`='Event'")
	...
	def insert_test_events(self):
		...
		for text in phrases:
			capkpi.get_doc(
				dict(doctype="Event", subject=text, repeat_on="Monthly", starts_on=capkpi.utils.now_datetime())
			).insert()
	...
	def test_update_doc(self):
		...
		event = capkpi.get_doc("Event", capkpi.get_all("Event")[0].name)
	...
	def test_delete_doc(self):
		...
		event_name = capkpi.get_all("Event")[0].name
		event = capkpi.get_doc("Event", event_name)
		...
		capkpi.delete_doc("Event", event_name)
		global_search.sync_global_search()
	...
	def test_insert_child_table(self):
		...
		for text in phrases:
			doc = capkpi.get_doc(
				{"doctype": "Event", "subject": text, "starts_on": capkpi.utils.now_datetime()}
			)
			doc.insert()
