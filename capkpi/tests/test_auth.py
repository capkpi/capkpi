...
import capkpi
from capkpi.auth import HTTPRequest, LoginAttemptTracker
from capkpi.capkpiclient import AuthError, CapKPIClient
...
class TestAuth(unittest.TestCase):
	...
	def tearDown(self):
		capkpi.delete_doc("User", self.test_user_email, force=True)
	...
	def set_system_settings(self, k, v):
		capkpi.db.set_value("System Settings", "System Settings", k, v)
		capkpi.db.commit()
	...
	def test_allow_login_using_mobile(self):
		...
		CapKPIClient(capkpi.get_site_config().host_name, self.test_user_mobile, self.test_user_password)
		CapKPIClient(capkpi.get_site_config().host_name, self.test_user_email, self.test_user_password)

		# login by username should fail
		with self.assertRaises(AuthError):
			CapKPIClient(capkpi.get_site_config().host_name, self.test_user_name, self.test_user_password)
