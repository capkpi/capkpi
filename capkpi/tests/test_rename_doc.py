...
import capkpi
from capkpi.exceptions import DoesNotExistError
from capkpi.model.base_document import get_controller
from capkpi.modules.utils import get_doc_path
from capkpi.utils import add_to_date, now
...
class TestRenameDoc(unittest.TestCase):
	...
	def setUpClass(self):
		...
		self._original_developer_flag = capkpi.conf.developer_mode
		capkpi.conf.developer_mode = 1
	...
	def tearDownClass(self):
		...
		for docname in self.available_documents:
			capkpi.delete_doc(self.test_doctype, docname)

		for dt in self.doctype.values():
			if capkpi.db.exists("DocType", dt):
				capkpi.delete_doc("DocType", dt)
				capkpi.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{dt}`")

		capkpi.delete_doc_if_exists("Renamed Doc", "ToDo")

		# reset original value of developer_mode conf
		capkpi.conf.developer_mode = self._original_developer_flag
	...
	def test_rename_doc(self):
		"""Rename an existing document via capkpi.rename_doc"""
		old_name = choice(self.available_documents)
		new_name = old_name + ".new"
		self.assertEqual(new_name, capkpi.rename_doc(self.test_doctype, old_name, new_name, force=True))
		self.available_documents.remove(old_name)
		self.available_documents.append(new_name)

	def test_merging_docs(self):
		"""Merge two documents via capkpi.rename_doc"""
		first_todo, second_todo = sample(self.available_documents, 2)

		second_todo_doc = capkpi.get_doc(self.test_doctype, second_todo)
		second_todo_doc.priority = "High"
		second_todo_doc.save()

		merged_todo = capkpi.rename_doc(
			self.test_doctype, first_todo, second_todo, merge=True, force=True
		)
		merged_todo_doc = capkpi.get_doc(self.test_doctype, merged_todo)
		self.available_documents.remove(first_todo)

		with self.assertRaises(DoesNotExistError):
			capkpi.get_doc(self.test_doctype, first_todo)
	...
	def test_rename_controllers(self):
		...
		self.assertNotEqual(get_controller(self.doctype.old), capkpi.model.document.Document)

		old_doctype_path = get_doc_path("Custom", "DocType", self.doctype.old)

		# rename doc via wrapper API accessible via /desk
		capkpi.rename_doc("DocType", self.doctype.old, self.doctype.new)

		# check if database and controllers are updated
		self.assertTrue(capkpi.db.exists("DocType", self.doctype.new))
		self.assertFalse(capkpi.db.exists("DocType", self.doctype.old))
		self.assertFalse(os.path.exists(old_doctype_path))

	def test_rename_doctype(self):
		"""Rename DocType via capkpi.rename_doc"""
		from capkpi.core.doctype.doctype.test_doctype import new_doctype

		if not capkpi.db.exists("DocType", "Rename This"):
			new_doctype(
				"Rename This",
				fields=[
					{
		...
		to_rename_record = capkpi.get_doc(
			{"doctype": "Rename This", "linked_to_doctype": "Rename This"}
		).insert()

		# Rename doctype
		self.assertEqual(
			"Renamed Doc", capkpi.rename_doc("DocType", "Rename This", "Renamed Doc", force=True)
		)

		# Test if Doctype value has changed in Link field
		linked_to_doctype = capkpi.db.get_value(
		...
		new_name = "ToDo"
		self.assertEqual(new_name, capkpi.rename_doc("Renamed Doc", old_name, new_name, force=True))

		# delete_doc doesnt drop tables
		# this is done to bypass inconsistencies in the db
		capkpi.delete_doc_if_exists("DocType", "Renamed Doc")
		capkpi.db.sql_ddl("drop table if exists `tabRenamed Doc`")
