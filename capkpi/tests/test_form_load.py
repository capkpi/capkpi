...
from capkpi.desk.form.load import get_docinfo, getdoc, getdoctype
...
class TestFormLoad(unittest.TestCase):
	def test_load(self):
		getdoctype("DocType")
		meta = list(filter(lambda d: d.name == "DocType", capkpi.response.docs))[0]
		self.assertEqual(meta.name, "DocType")
		self.assertTrue(meta.get("__js"))

		capkpi.response.docs = []
		getdoctype("Event")
		meta = list(filter(lambda d: d.name == "Event", capkpi.response.docs))[0]
	...
	def test_fieldlevel_permissions_in_load(self):
		blog = capkpi.get_doc(
			{
				...
				"title": "_Test Blog Post {}".format(capkpi.utils.now()),
				"published": 0,
			}
		)
		...
		doc = capkpi.get_doc("Blog Post", blog.name)
		doc.published = 1
		doc.save()
		...
		blog_doc.delete()
		capkpi.delete_doc(blog_post_property_setter.doctype, blog_post_property_setter.name)
	...
	def test_fieldlevel_permissions_in_load_for_child_table(self):
		...
		contact = capkpi.get_doc("Contact", "_Test Contact 1")
	...
	def test_get_doc_info(self):
		...
		note.title = capkpi.generate_hash(length=20)
		note.insert()
		...
		get_docinfo(note)
		docinfo = capkpi._dict(capkpi.response["docinfo"])
