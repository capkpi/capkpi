...
import capkpi
from capkpi.utils.password import check_password, passlibctx, update_password


class TestPassword(unittest.TestCase):
	def setUp(self):
		capkpi.delete_doc("Email Account", "Test Email Account Password")
		capkpi.delete_doc("Email Account", "Test Email Account Password-new")
	...
	def make_email_account(self, name="Test Email Account Password"):
		if not capkpi.db.exists("Email Account", name):
			...
			return capkpi.get_doc("Email Account", name)
	...
	def test_hashed_password(self, user="test@example.com"):
		...
		self.assertRaises(capkpi.AuthenticationError, check_password, user, new_password)
	...
	def test_password_on_rename_user(self):
		...
		capkpi.rename_doc(doc.doctype, old_name, new_name)

		new_doc = capkpi.get_doc(doc.doctype, new_name)
		self.assertEqual(new_doc.get_password(), password)
		self.assertTrue(not get_password_list(doc))

		capkpi.rename_doc(doc.doctype, new_name, old_name)
