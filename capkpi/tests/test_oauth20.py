...
import capkpi
from capkpi.integrations.oauth2 import encode_params
...
class TestOAuth20(unittest.TestCase):
	def setUp(self):
		...
		client = capkpi.get_all("OAuth Client", fields=["*"])[0]
		...
		# Set CapKPI server URL reqired for id_token generation
		try:
			capkpi_login_key = capkpi.get_doc("Social Login Key", "capkpi")
		...
		capkpi_login_key.base_url = capkpi.utils.get_url()
	...
	def test_login_using_authorization_code(self):
		...
		try:
			session.get(
				get_full_url("/api/method/capkpi.integrations.oauth2.authorize"),
		...
		token_response = requests.post(
			get_full_url("/api/method/capkpi.integrations.oauth2.get_token"),
	...
	def test_revoke_token(self):
		client = capkpi.get_doc("OAuth Client", self.client_id)
		...
		revoke_token_response = requests.post(
			get_full_url("/api/method/capkpi.integrations.oauth2.revoke_token"),
			headers=self.form_header,
	...
	def test_login_using_implicit_token(self):
		oauth_client = capkpi.get_doc("OAuth Client", self.client_id)
...
def check_valid_openid_response(access_token=None):
	...
	openid_response = requests.get(
		get_full_url("/api/method/capkpi.integrations.oauth2.openid_profile"), headers=headers
	)
...
def get_full_url(endpoint):
	...
	return urljoin(capkpi.utils.get_url(), endpoint)


def update_client_for_auth_code_grant(client_id):
	client = capkpi.get_doc("OAuth Client", client_id)
	client.grant_type = "Authorization Code"
	client.response_type = "Code"
	client.save()
	capkpi.db.commit()
