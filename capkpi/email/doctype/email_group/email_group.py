...
from capkpi.utils import parse_addr, validate_email_address


class EmailGroup(Document):
	def onload(self):
		singles = [d.name for d in capkpi.db.get_all("DocType", "name", {"issingle": 1})]
		self.get("__onload").import_types = [
			{"value": d.parent, "label": "{0} ({1})".format(d.parent, d.label)}
			for d in capkpi.db.get_all("DocField", ("parent", "label"), {"options": "Email"})
	...
	def import_from(self, doctype):
		...
		for user in capkpi.db.get_all(doctype, [email_field, unsubscribed_field or "name"]):
			try:
		...
		capkpi.msgprint(_("{0} subscribers added").format(added))
	...
	def on_trash(self):
		for d in capkpi.get_all("Email Group Member", "name", {"email_group": self.name}):
			capkpi.delete_doc("Email Group Member", d.name)


@capkpi.whitelist()
def import_from(name, doctype):
	nlist = capkpi.get_doc("Email Group", name)
...
def add_subscribers(name, email_list):
	...
	template = capkpi.db.get_value("Email Group", name, "welcome_email_template")
	welcome_email = capkpi.get_doc("Email Template", template) if template else None
	...
	for email in email_list:
		...
		if parsed_email:
			if not capkpi.db.get_value("Email Group Member", {"email_group": name, "email": parsed_email}):
				capkpi.get_doc(
					{"doctype": "Email Group Member", "email_group": name, "email": parsed_email}
				).insert(ignore_permissions=capkpi.flags.ignore_permissions)
			...
			capkpi.msgprint(_("{0} is not a valid Email Address").format(email))

	capkpi.msgprint(_("{0} subscribers added").format(count))

	return capkpi.get_doc("Email Group", name).update_total_subscribers()
...
def send_welcome_email(welcome_email, email, email_group):
	...
	message = capkpi.render_template(email_message, args)
	capkpi.sendmail(email, subject=welcome_email.subject, message=message)
