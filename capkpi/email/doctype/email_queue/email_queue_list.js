capkpi.listview_settings['Email Queue'] = {
...
function show_toggle_sending_button(list_view) {
	...
	const sending_disabled = cint(capkpi.sys_defaults.suspend_email_queue);
	...
	list_view.page.add_inner_button(
		...
		async () => {
			await capkpi.xcall(
				"capkpi.email.doctype.email_queue.email_queue.toggle_sending",
			...
			capkpi.sys_defaults.suspend_email_queue = sending_disabled ? 0 : 1;
