...
from capkpi.modules.export_file import create_init_py, export_to_files


def get_mapping_module(module, mapping_name):
	app_name = capkpi.db.get_value("Module Def", module, "app_name")
	mapping_name = capkpi.scrub(mapping_name)
	module = capkpi.scrub(module)

	try:
		return capkpi.get_module(
...
class DataMigrationPlan(Document):
	def on_update(self):
		...
		if capkpi.flags.in_import or capkpi.flags.in_test:
			return

		if capkpi.local.conf.get("developer_mode"):
	...
	def make_custom_fields_for_mappings(self):
		capkpi.flags.ignore_in_install = True
		label = self.name + " ID"
		fieldname = capkpi.scrub(label)
		...
		for m in self.mappings:
			mapping = capkpi.get_doc("Data Migration Mapping", m.mapping)
