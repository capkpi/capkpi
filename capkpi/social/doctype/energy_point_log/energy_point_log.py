...
from capkpi.utils import cint, get_fullname, get_link_to_form, getdate
...
class EnergyPointLog(Document):
	def validate(self):
		...
		if self.type in ["Appreciation", "Criticism"] and self.user == self.owner:
			capkpi.throw(_("You cannot give review points to yourself"))
	...
	def map_milestone_reference(self):
		...
		if self.reference_doctype == "Milestone":
			self.reference_doctype, self.reference_name = capkpi.db.get_value(
	...
	def after_insert(self):
		...
		if alert_dict:
			capkpi.publish_realtime("energy_point_alert", message=alert_dict, user=self.user)

		capkpi.cache().hdel("energy_points", self.user)
		capkpi.publish_realtime("update_points", after_commit=True)

		if self.type != "Review" and capkpi.get_cached_value(
	...
	def on_trash(self):
		if self.type == "Revert":
			reference_log = capkpi.get_doc("Energy Point Log", self.revert_of)
	...
	def revert(self, reason, ignore_permissions=False):
		...
		if self.type != "Auto":
			capkpi.throw(_("This document cannot be reverted"))
...
def get_notification_message(doc):
	...
	if doc.type == "Auto":
		owner_name = capkpi.bold("You")
		if points == 1:
			message = _("{0} gained {1} point for {2} {3}")
		else:
			message = _("{0} gained {1} points for {2} {3}")
		message = message.format(owner_name, capkpi.bold(points), doc.rule, get_title_html(title))
		...
		message = message.format(capkpi.bold(owner_name), get_title_html(title), capkpi.bold(points))
		...
		message = message.format(capkpi.bold(owner_name), get_title_html(title))

	return message


def get_alert_dict(doc):
	alert_dict = capkpi._dict()
	owner_name = get_fullname(doc.owner)
	if doc.reference_doctype:
		doc_link = get_link_to_form(doc.reference_doctype, doc.reference_name)
	points = doc.points
	bold_points = capkpi.bold(doc.points)
	if doc.type == "Auto":
		if points == 1:
...
def create_energy_points_log(ref_doctype, ref_name, doc, apply_only_once=False):
	...
	new_log = capkpi.new_doc("Energy Point Log")
...
def check_if_log_exists(ref_doctype, ref_name, rule, user=None):
	...
	return capkpi.db.exists("Energy Point Log", filters)
...
def review(doc, points, to_user, reason, review_type="Appreciation"):
	current_review_points = get_energy_points(capkpi.session.user).review_points
	doc = doc.as_dict() if hasattr(doc, "as_dict") else capkpi._dict(json.loads(doc))
	points = abs(cint(points))
	if current_review_points < points:
		capkpi.msgprint(_("You do not have enough review points"))
		return
...
def send_summary(timespan):
	...
	if not is_email_notifications_enabled_for_type(capkpi.session.user, "Energy Point"):
		return

	from_date = capkpi.utils.add_to_date(None, weeks=-1)
	if timespan == "Monthly":
		from_date = capkpi.utils.add_to_date(None, months=-1)
