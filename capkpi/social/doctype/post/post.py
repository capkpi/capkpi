...
class Post(Document):
	def on_update(self):
		if self.is_globally_pinned:
			capkpi.publish_realtime("global_pin", after_commit=True)

	def after_insert(self):
		capkpi.publish_realtime("new_post", self.owner, after_commit=True)


@capkpi.whitelist()
def toggle_like(post_name, user=None):
	liked_by = capkpi.db.get_value("Post", post_name, "liked_by")
	...
	liked_by = "\n".join(liked_by)
	capkpi.db.set_value("Post", post_name, "liked_by", liked_by)
	capkpi.publish_realtime("update_liked_by" + post_name, liked_by, after_commit=True)
...
def frequently_visited_links():
	return capkpi.get_all(
		...
		filters={"user": capkpi.session.user},
		group_by="route",
		order_by="count desc",
		limit=5,
	)
...
def get_link_info(url):
	cached_link_info = capkpi.cache().hget("link_info", url)
	if cached_link_info:
	...
	try:
		...
		capkpi.cache().hset("link_info", url, {})
		return {}
	...
	capkpi.cache().hset("link_info", url, meta_obj)
...
def delete_post(post_name):
	post = capkpi.get_doc("Post", post_name)
	post.delete()
	capkpi.publish_realtime("delete_post" + post_name, after_commit=True)


def get_unseen_post_count():
	post_count = capkpi.db.count("Post")
...
def get_posts(filters=None, limit_start=0):
	filters = capkpi.utils.get_safe_filters(filters)
...
def get_viewed_posts(only_count=False):
	view_logs = capkpi.db.get_all(
		"View Log",
		filters={"reference_doctype": "Post", "viewed_by": capkpi.session.user},
