...
import capkpi
from capkpi.integrations.doctype.webhook.webhook import (
	enqueue_webhook,
	get_webhook_data,
	get_webhook_headers,
)
...
class TestWebhook(unittest.TestCase):
	...
	def setUpClass(cls):
		...
		capkpi.db.sql("DELETE FROM tabWebhook")
		# Delete existing logs if any
		capkpi.db.sql("DELETE FROM `tabWebhook Request Log`")
	...
	def setUp(self):
		...
		if capkpi.db.exists("Webhook", webhook_fields):
			self.webhook = capkpi.get_doc("Webhook", webhook_fields)
		else:
			self.webhook = capkpi.new_doc("Webhook")
			self.webhook.update(webhook_fields)

		# create a User document
		self.user = capkpi.new_doc("User")
		self.user.first_name = capkpi.mock("name")
		self.user.email = capkpi.mock("email")
		self.user.save()

		# Create another test user specific to this test
		self.test_user = capkpi.new_doc("User")
	...
	def test_webhook_trigger_with_enabled_webhooks(self):
		...
		capkpi.flags.webhooks = None

		# Insert the user to db
		self.test_user.insert()

		self.assertTrue("User" in capkpi.flags.webhooks)
		# only 1 hook (enabled) must be queued
		self.assertEqual(len(capkpi.flags.webhooks.get("User")), 1)
		self.assertTrue(self.test_user.email in capkpi.flags.webhooks_executed)
		self.assertEqual(
			capkpi.flags.webhooks_executed.get(self.test_user.email)[0], self.sample_webhooks[0].name
		)
	...
	def test_validate_doc_events(self):
		...
		self.assertRaises(capkpi.ValidationError, self.webhook.save)
	...
	def test_webhook_req_log_creation(self):
		if not capkpi.db.get_value("User", "user2@integration.webhooks.test.com"):
			user = capkpi.get_doc(
				{"doctype": "User", "email": "user2@integration.webhooks.test.com", "first_name": "user2"}
			).insert()
		else:
			user = capkpi.get_doc("User", "user2@integration.webhooks.test.com")

		webhook = capkpi.get_doc("Webhook", {"webhook_doctype": "User"})
		enqueue_webhook(user, webhook)

		self.assertTrue(capkpi.db.get_all("Webhook Request Log", pluck="name"))
