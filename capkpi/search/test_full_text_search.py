...
class TestFullTextSearch(unittest.TestCase):
	...
	def test_update_index(self):
		...
		self.index.update_index({"name": "sw/erp", "content": """AwesomeERP"""})

		res = self.index.search("CommonSearchTerm")
		self.assertTrue("sw/erp" not in res)

		res = self.index.search("AwesomeERP")
		self.assertEqual(res[0], "sw/erp")

		# Update new doc
		self.index.update_index({"name": "sw/capkpibooks", "content": """DesktopAccounting"""})

		res = self.index.search("DesktopAccounting")
		self.assertEqual(res[0], "sw/capkpibooks")
...
def get_index():
	return TestWrapper("test_capkpi_index")
...
def get_documents():
	...
	docs.append(
		{
			"name": "sw/erp",
			"content": """ERP is a free and open-source integrated Enterprise Resource Planning software developed by
			CapKPI Technologies Pvt. Ltd. and is built on MariaDB database system using a Python based server-side framework.
			ERP is a generic ERP software used by manufacturers, distributors and services companies. CommonSearchTerm""",
		}
	)
	...
	docs.append(
		{
			"name": "sw/capkpi",
			"content": """CapKPI Framework is a full-stack web framework, that includes everything you need to build and
