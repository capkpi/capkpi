...
from capkpi.website.utils import can_cache, delete_page_cache, extract_comment_tag, extract_title
...
def get_page_context(path):
	page_context = None
	if can_cache():
		page_context_cache = capkpi.cache().hget("page_context", path) or {}
		page_context = page_context_cache.get(capkpi.local.lang, None)

	if not page_context:
		page_context = make_page_context(path)
		if can_cache(page_context.no_cache):
			page_context_cache[capkpi.local.lang] = page_context
			capkpi.cache().hset("page_context", path, page_context_cache)
...
def make_page_context(path):
	...
	if not context:
		raise capkpi.PageDoesNotExistError
	...
	context.pathname = capkpi.local.path
...
def get_page_info_from_template(path):
	...
	for app in capkpi.get_installed_apps(capkpi_last=True):
		app_path = capkpi.get_app_path(app)
...
def get_page_context_from_doctype(path):
	...
	if page_info:
		return capkpi.get_doc(page_info.get("doctype"), page_info.get("name")).get_page_info()
...
def get_all_page_context_from_doctypes():
	...
	"""
	routes = capkpi.cache().get_value("website_generator_routes")
	if not routes:
		routes = get_page_info_from_doctypes()
		capkpi.cache().set_value("website_generator_routes", routes)
...
def evaluate_dynamic_routes(rules, path):
	...
	if hasattr(capkpi.local, "request") and capkpi.local.request.environ:
		urls = route_map.bind_to_environ(capkpi.local.request.environ)
		try:
			...
			if args:
				...
				capkpi.local.form_dict.update(args)
...
def get_pages(app=None):
	...
	def _build(app):
		...
		if app:
			...
			apps = capkpi.local.flags.web_pages_apps or capkpi.get_installed_apps()
	...
	return capkpi.cache().get_value("website_pages", lambda: _build(app))
...
def get_page_info(path, app, start, basepath=None, app_path=None, fname=None):
	...
	page_info.build_version = capkpi.utils.get_build_version()
...
def setup_source(page_info):
	...
	if page_info.template.endswith((".md", ".html")):
		...
		if page_info.template.endswith(".md"):
			source = capkpi.utils.md_to_html(source)
...
def get_base_template(path=None):
	...
	base_template_map = capkpi.get_hooks("base_template_map") or {}
...
def load_properties_from_controller(page_info):
	...
	module = capkpi.get_module(page_info.controller)
	if not module:
		return
...
def get_doctypes_with_web_view():
	...
	def _get():
		...
		doctypes = capkpi.get_hooks("website_generators")
		doctypes += [
			d.name
			for d in capkpi.get_all("DocType", "name, module", dict(has_web_view=1))
			if capkpi.local.module_app[capkpi.scrub(d.module)] in installed_apps
		]
		return doctypes

	return capkpi.cache().get_value("doctypes_with_web_view", _get)


def get_start_folders():
	return capkpi.local.flags.web_pages_folders or ("www", "templates/pages")
