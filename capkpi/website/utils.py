...
def can_cache(no_cache=False):
	if capkpi.conf.disable_website_cache or capkpi.conf.developer_mode:
		return False
	if getattr(capkpi.local, "no_cache", False):
		return False
	return not no_cache
...
def get_comment_list(doctype, name):
	comments = capkpi.get_all(
		...
		or_filters=[["owner", "=", capkpi.session.user], ["published", "=", 1]],
	)
	...
	communications = capkpi.get_all(
		...
		or_filters=[
			["recipients", "like", "%{0}%".format(capkpi.session.user)],
			["cc", "like", "%{0}%".format(capkpi.session.user)],
			["bcc", "like", "%{0}%".format(capkpi.session.user)],
		],
	)
...
def get_home_page():
	if capkpi.local.flags.home_page and not capkpi.flags.in_test:
		return capkpi.local.flags.home_page
	...
	def _get_home_page():
		...
		if capkpi.session.user != "Guest":
			# by role
			for role in capkpi.get_roles():
				home_page = capkpi.db.get_value("Role", role, "home_page")
				if home_page:
					break

			# portal default
			if not home_page:
				home_page = capkpi.db.get_single_value("Portal Settings", "default_portal_home")
		...
		# global
		if not home_page:
			home_page = capkpi.db.get_single_value("Website Settings", "home_page")

		if not home_page:
			home_page = "login" if capkpi.session.user == "Guest" else "me"
	...
	return capkpi.cache().hget("home_page", capkpi.session.user, _get_home_page)


def get_home_page_via_hooks():
	home_page = None

	home_page_method = capkpi.get_hooks("get_website_user_home_page")
	if home_page_method:
		home_page = capkpi.get_attr(home_page_method[-1])(capkpi.session.user)
	elif capkpi.get_hooks("website_user_home_page"):
		home_page = capkpi.get_hooks("website_user_home_page")[-1]

	if not home_page:
		role_home_page = capkpi.get_hooks("role_home_page")
		if role_home_page:
			for role in capkpi.get_roles():
	...
	if not home_page:
		home_page = capkpi.get_hooks("home_page")
		if home_page:
...
def is_signup_enabled():
	if getattr(capkpi.local, "is_signup_enabled", None) is None:
		capkpi.local.is_signup_enabled = True
		if capkpi.utils.cint(
			capkpi.db.get_value("Website Settings", "Website Settings", "disable_signup")
		):
			capkpi.local.is_signup_enabled = False

	return capkpi.local.is_signup_enabled


def is_signup_disabled():
	return capkpi.db.get_single_value("Website Settings", "disable_signup", True)
...
def get_toc(route, url_prefix=None, app=None):
	...
	return capkpi.get_template("templates/includes/full_index.html").render(
...
def get_full_index(route=None, app=None):
	...
	from capkpi.website.router import get_pages

	if not capkpi.local.flags.children_map:
		...
		children_map = capkpi.cache().get_value("website_full_index", _build)

		capkpi.local.flags.children_map = children_map

	return capkpi.local.flags.children_map
...
def add_missing_headers():
	...
	path = capkpi.get_app_path("erp", "docs")
	for basepath, folders, files in os.walk(path):
		for fname in files:
			if fname.endswith(".md"):
				with open(os.path.join(basepath, fname), "r") as f:
					content = capkpi.as_unicode(f.read())
