...
class PersonalDataDownloadRequest(Document):
	...
	def generate_file_and_send_mail(self, personal_data):
		...
		file_link = (
			capkpi.utils.get_url("/api/method/capkpi.utils.file_manager.download_file")
			+ "?"
...
def get_user_data(user):
	...
	hooks = capkpi.get_hooks("user_data_fields")
	data = {}
	for hook in hooks:
		d = data.get(hook.get("doctype"), [])
		d += capkpi.get_all(hook.get("doctype"), {hook.get("filter_by", "owner"): user}, ["*"])
		if d:
