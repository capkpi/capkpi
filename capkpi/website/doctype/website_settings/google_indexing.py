...
def authorize_access(reauthorize=None):
	...
	redirect_uri = (
		...
		+ "?cmd=capkpi.website.doctype.website_settings.google_indexing.google_callback"
	)
	...
	if not website_settings.indexing_authorization_code or reauthorize:
		...
		try:
			...
			capkpi.local.response["location"] = "/app/Form/{0}".format(quote("Website Settings"))

			capkpi.msgprint(_("Google Indexing has been configured."))
...
def google_callback(code=None):
	...
	capkpi.db.set_value("Website Settings", None, "indexing_authorization_code", code)
	capkpi.db.commit()
...
def get_google_indexing_object():
	...
	google_settings = capkpi.get_doc("Google Settings")
	account = capkpi.get_doc("Website Settings")
...
def publish_site(url, operation_type="URL_UPDATED"):
	...
	try:
		...
		capkpi.log_error(message=e, title="API Indexing Issue")
