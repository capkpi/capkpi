...
class PersonalDataDeletionRequest(Document):
	def __init__(self, *args, **kwargs):
		...
		self.user_data_fields = capkpi.get_hooks("user_data_fields")
	...
	def autoname(self):
		...
		domain = capkpi.local.site.replace("_", "-")
		...
		capkpi.utils.validate_email_address(self.email, throw=True)

	def after_insert(self):
		self.send_verification_mail()

	def generate_url_for_confirmation(self):
		params = {"email": self.email, "name": self.name, "host_name": capkpi.local.site}
		api = capkpi.utils.get_url(
			"/api/method/capkpi.website.doctype.personal_data_deletion_request"
	...
	def disable_user(self):
		user = capkpi.get_doc("User", self.email)
		user.enabled = False
		user.save()
	...
	def notify_system_managers(self):
		...
		capkpi.sendmail(
			...
			args={"user": self.email, "url": capkpi.utils.get_url(self.get_url())},
	...
	def validate_data_anonymization(self):
		...
		if self.status != "Pending Approval":
			capkpi.throw(_("This request has not yet been approved by the user."))
	...
	def generate_anonymization_dict(self, ref):
		...
		meta = capkpi.get_meta(ref["doctype"])
	...
	def redact_doc(self, doc, ref):
		...
		if ref.get("rename") and doc["name"] != self.anon:
			capkpi.rename_doc(ref["doctype"], doc["name"], self.anon, force=True, show_alert=False)
	...
	def _anonymize_data(self, email=None, anon=None, set_data=True, commit=False):
		...
		capkpi.rename_doc("User", email, anon, force=True, show_alert=False)
	...
	def __redact_partial_match_data(self, doctype):
		...
		for df in capkpi.get_meta(doctype["doctype"]).fields:
...
def confirm_deletion(email, name, host_name):
	...
	doc = capkpi.get_doc("Personal Data Deletion Request", name)
	host_name = capkpi.local.site
