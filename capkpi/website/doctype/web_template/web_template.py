...
from capkpi.modules.export_file import get_module_path, scrub_dt_dn, write_document_file
from capkpi.website.render import clear_cache


class WebTemplate(Document):
	def validate(self):
		if self.standard and not (capkpi.conf.developer_mode or capkpi.flags.in_patch):
			capkpi.throw(_("Enable developer mode to create a standard Web Template"))

		for field in self.fields:
			if not field.fieldname:
				field.fieldname = capkpi.scrub(field.label)
	...
	def on_trash(self):
		if capkpi.conf.developer_mode and self.standard:
	...
	def get_template_path(self):
		...
		file_name = capkpi.scrub(self.name) + ".html"
	...
	def render(self, values=None):
		if not values:
			values = {}
		values = capkpi.parse_json(values)
		values.update({"values": values})
		template = self.get_template(self.standard)

		return capkpi.render_template(template, values)
