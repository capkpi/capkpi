...
from capkpi.utils import cint, is_markdown, markdown
...
class HelpArticle(WebsiteGenerator):
	...
	def set_route(self):
		...
		if not self.route:
			self.route = "/".join(
				[capkpi.get_value("Help Category", self.category, "route"), self.scrub(self.title)]
			)
	...
	def update_category(self):
		...
		cat = capkpi.get_doc("Help Category", self.category)
	...
	def get_context(self, context):
		...
		context.category = capkpi.get_doc("Help Category", self.category)
...
def get_list_context(context=None):
	...
	category = capkpi.db.get_value("Help Category", {"route": capkpi.local.path})
	...
	list_context = capkpi._dict(
		...
		hide_filters=True,
		filters=filters,
		category=capkpi.local.form_dict.category,
		no_breadcrumbs=True,
	)

	if capkpi.local.form_dict.txt:
		list_context.blog_subtitle = _('Filtered by "{0}"').format(capkpi.local.form_dict.txt)
	#
	# list_context.update(capkpi.get_doc("Blog Settings", "Blog Settings").as_dict())
	return list_context
...
def get_sidebar_items():
	...
	return capkpi.cache().get_value("knowledge_base:category_sidebar", _get)
...
def clear_website_cache(path=None):
	capkpi.cache().delete_value("knowledge_base:category_sidebar")
	capkpi.cache().delete_value("knowledge_base:faq")
...
def add_feedback(article, helpful):
	...
	value = cint(capkpi.db.get_value("Help Article", article, field))
	capkpi.db.set_value("Help Article", article, field, value + 1, update_modified=False)
