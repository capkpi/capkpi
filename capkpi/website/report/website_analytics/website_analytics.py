# Copyright (c) 2013, CapKPI Technologies and contributors
...
import capkpi
from capkpi.query_builder.functions import Coalesce, Count
...
class WebsiteAnalytics(object):
	def __init__(self, filters=None):
		...
		if not self.filters.from_date:
			self.filters.from_date = capkpi.utils.add_days(self.filters.to_date, -7)

		if not self.filters.range:
			self.filters.range = "Daily"

		self.filters.to_date = capkpi.utils.add_days(self.filters.to_date, 1)
	...
	def get_data(self):
		WebPageView = capkpi.qb.DocType("Web Page View")
		count_all = Count("*").as_("count")
		case = capkpi.qb.terms.Case().when(WebPageView.is_unique == "1", "1")
		count_is_unique = Count(case).as_("unique_count")

		return (
			capkpi.qb.from_(WebPageView)
			...
			.orderby("count", Order=capkpi.qb.desc)
		).run()
	...
	def get_chart_data(self):
		...
		self.chart_data = capkpi.db.sql(query, values=values, as_dict=1)
	...
	def prepare_chart_data(self, data):
		...
		if self.filters.range == "Monthly":
			date_range = [capkpi.utils.add_days(dd, 1) for dd in date_range]
