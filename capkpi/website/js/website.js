...
capkpi.provide("website");
capkpi.provide("capkpi.awesome_bar_path");
window.cur_frm = null;
...
$.extend(capkpi, {
	...
	add_asset_to_head(link) {
		return new Promise(resolve => {
			if (capkpi._assets_loaded.includes(link)) return resolve();
			let el;
			...
			el.onload = () => {
				capkpi._assets_loaded.push(link);
				resolve();
			};
		});
	},
	...
	call: function(opts) {
		...
		return $.ajax({
			...
			data: opts.args,
			dataType: "json",
			headers: { "X-CapKPI-CSRF-Token": capkpi.csrf_token, "X-CapKPI-CMD": (opts.args && opts.args.cmd  || '') || '' },
			statusCode: opts.statusCode || {
				404: function() {
					capkpi.msgprint(__("Not found"));
				},
				403: function() {
					capkpi.msgprint(__("Not permitted"));
				},
				200: function(data) {
					if(opts.callback)
						opts.callback(data);
			...
			capkpi.process_response(opts, data);
		});
	},
	...
	process_response: function(opts, data) {
		...
		if (data._server_messages) {
			...
			if(opts.error_msg) {
				...
				capkpi.msgprint(server_messages);
			}
		}
	...
	send_message: function(opts, btn) {
		return capkpi.call({
			type: "POST",
			method: "capkpi.www.contact.send_message",
			btn: btn,
			args: opts,
	...
	has_permission: function(doctype, docname, perm_type, callback) {
		return capkpi.call({
			type: "GET",
			method: "capkpi.client.has_permission",
			no_spinner: true,
	...
	render_user: function() {
		if (capkpi.is_user_logged_in()) {
			$(".btn-login-area").toggle(false);
			$(".logged-in").toggle(true);
			$(".user-image").attr("src", capkpi.get_cookie("user_image"));

			$('.user-image-wrapper').html(capkpi.avatar(null, 'avatar-medium', null, null, null, true));
			$('.user-image-sidebar').html(capkpi.avatar(null, 'avatar-medium', null, null, null, true));
			$('.user-image-myaccount').html(capkpi.avatar(null, 'avatar-large', null, null, null, true));
		}
	},
	freeze_count: 0,
	freeze: function(msg) {
	...
	unfreeze: function() {
		if(!capkpi.freeze_count) return; // anything open?
		capkpi.freeze_count--;
		if(!capkpi.freeze_count) {
	...
	trigger_ready: function() {
		capkpi.ready_events.forEach(function(fn) {
			fn();
		});
	},
	...
	bind_filters: function() {
		...
		$(".filter").each(function() {
			...
			var val = capkpi.utils.get_url_arg(key).replace(/\+/g, " ");
	...
	bind_navbar_search: function() {
		capkpi.get_navbar_search().on("keypress", function(e) {
			var val = $(this).val();
			if(e.which===13 && val) {
				$(this).val("").blur();
				capkpi.do_search(val);
				return false;
			}
		});
	},
	do_search: function(val) {
		var path = (capkpi.awesome_bar_path && capkpi.awesome_bar_path[location.pathname]
			|| window.search_path || location.pathname);

		window.location.href = path + "?txt=" + encodeURIComponent(val);
	},
	set_search_path: function(path) {
		capkpi.awesome_bar_path[location.pathname] = path;
	},
	...
	is_user_logged_in: function() {
		return capkpi.get_cookie("user_id") !== "Guest" && capkpi.session.user !== "Guest";
	},
	...
	show_language_picker() {
		if (capkpi.session.user === 'Guest' && window.show_language_picker) {
			capkpi.call("capkpi.translate.get_all_languages", {
				with_language_name: true
			}).then(res => {
				let language_list = res.message;
				let language = capkpi.get_cookie('preferred_language');
				let language_codes = [];
				...
				language_switcher.change(() => {
					...
					capkpi.call("capkpi.translate.set_preferred_language_cookie", {
...
capkpi.setup_search = function (target, search_scope) {
	...
	$input.on('input', capkpi.utils.debounce(() => {
		if (!$input.val()) {
			clear_dropdown();
			return;
		}
		...
		capkpi.call({
			method: 'capkpi.search.web_search',
			args: {
...
window.valid_email = function(id) {
	...
	// copied regex from capkpi/utils.js validate_type
...
$(document).ready(function() {
	window.full_name = capkpi.get_cookie("full_name");
	var logged_in = capkpi.is_user_logged_in();
	$("#website-login").toggleClass("hide", logged_in ? true : false);
	$("#website-post-login").toggleClass("hide", logged_in ? false : true);
	$(".logged-in").toggleClass("hide", logged_in ? false : true);

	capkpi.bind_navbar_search();

	// switch to app link
	if(capkpi.get_cookie("system_user")==="yes" && logged_in) {
		capkpi.add_switch_to_desk();
	}

	capkpi.render_user();
	capkpi.setup_lazy_images();
...
$(document).on("page-change", function() {
	...
	$.extend(capkpi, capkpi.get_cookies());
	capkpi.session = {'user': capkpi.user_id};

	capkpi.datetime.refresh_when();
	capkpi.trigger_ready();
	capkpi.bind_filters();
	capkpi.highlight_code_blocks();
	capkpi.add_link_to_headings();
	capkpi.make_navbar_active();
	// scroll to hash
...
capkpi.ready(function() {
	capkpi.show_language_picker();
	capkpi.socketio.init(window.socketio_port);
});
