...
$.extend(capkpi.model, {
	...
	get_new_doc: function(
		...
		var doc = {
			docstatus: 0,
			doctype: doctype,
			name: capkpi.model.get_new_name(doctype),
			__islocal: 1,
			__unsaved: 1,
			owner: capkpi.session.user
		};
		capkpi.model.set_default_values(doc, parent_doc);
		...
		if (parent_doc) {
			...
			capkpi.provide("capkpi.model.docinfo." + doctype + "." + doc.name);
		}

		capkpi.model.add_to_locals(doc);

		if (with_mandatory_children) {
			capkpi.model.create_mandatory_children(doc);
		}
		...
		if (capkpi.route_options && capkpi.route_options.name_field) {
			var meta = capkpi.get_meta(doctype);
			// set title field / name as name
			if (meta.autoname && meta.autoname.indexOf("field:") !== -1) {
				doc[meta.autoname.substr(6)] = capkpi.route_options.name_field;
			} else if (meta.title_field) {
				doc[meta.title_field] = capkpi.route_options.name_field;
			}

			delete capkpi.route_options.name_field;
		}

		// set route options
		if (capkpi.route_options && !doc.parent) {
			$.each(capkpi.route_options, function(fieldname, value) {
				var df = capkpi.meta.has_field(doctype, fieldname);
				if (
					df &&
					in_list(
	...
	make_new_doc_and_get_name: function(doctype, with_mandatory_children) {
		return capkpi.model.get_new_doc(
			doctype,
			null,
			null,
			with_mandatory_children
	...
	get_new_name: function(doctype) {
		var cnt = capkpi.model.new_name_count;
		if (!cnt[doctype]) cnt[doctype] = 0;
		cnt[doctype]++;
		return capkpi.router.slug(`new-${doctype}-${cnt[doctype]}`);
	},

	set_default_values: function(doc, parent_doc) {
		var doctype = doc.doctype;
		var docfields = capkpi.meta.get_docfields(doctype);
		var updated = [];
		for (var fid = 0; fid < docfields.length; fid++) {
			var f = docfields[fid];
			if (
				!in_list(capkpi.model.no_value_type, f.fieldtype) &&
				doc[f.fieldname] == null
			) {
				if (f.no_default) continue;
				var v = capkpi.model.get_default_value(f, doc, parent_doc);
				if (v) {
	...
	create_mandatory_children: function(doc) {
		var meta = capkpi.get_meta(doc.doctype);
		if (meta && meta.istable) return;

		// create empty rows for mandatory table fields
		capkpi.meta.get_docfields(doc.doctype).forEach(function(df) {
			if (df.fieldtype === "Table" && df.reqd) {
				capkpi.model.add_child(doc, df.fieldname);
			}
		});
	},
	...
	get_default_value: function(df, doc, parent_doc) {
		var user_default = "";
		var user_permissions = capkpi.defaults.get_user_permissions();
		let allowed_records = [];
		...
		if (user_permissions) {
			({
				allowed_records,
				default_doc
			} = capkpi.perm.filter_allowed_docs_for_doctype(
		...
		if (df.fieldtype === "Link" && df.options !== "User") {
			...
			if (has_user_permissions && default_doc) {
				...
				if (!df.ignore_user_permissions) {
					var user_defaults = capkpi.defaults.get_user_defaults(df.options);
				...
				if (!user_default) {
					user_default = capkpi.defaults.get_user_default(df.fieldname);
				} else if (
					!user_default &&
					df.remember_last_selected_value &&
					capkpi.boot.user.last_selected_values
				) {
					user_default = capkpi.boot.user.last_selected_values[df.options];
				}
		...
		if (!value || df["default"]) {
			...
			if (
				...
				value = capkpi.session.user;
			} else if (default_val == "user_fullname") {
				value = capkpi.session.user_fullname;
			} else if (default_val == "Today") {
				value = capkpi.datetime.get_today();
			} else if ((default_val || "").toLowerCase() === "now") {
				value = capkpi.datetime.now_datetime();
			} else if (default_val[0] === ":") {
				var boot_doc = capkpi.model.get_default_from_boot_docs(
					df,
					doc,
					parent_doc
				);
			...
			value = capkpi.datetime.now_time();
		}
	...
	get_default_from_boot_docs: function(df, doc, parent_doc) {
		...
		if (capkpi.get_list(df["default"]).length > 0) {
			...
			var ref_value = parent_doc
				...
				: capkpi.defaults.get_user_default(ref_fieldname);
			var ref_doc = ref_value
				? capkpi.get_doc(df["default"], ref_value)
				: null;
	...
	add_child: function(parent_doc, doctype, parentfield, idx) {
		...
		if (arguments.length === 2) {
			parentfield = doctype;
			doctype = capkpi.meta.get_field(parent_doc.doctype, parentfield)
				.options;
		}
		...
		var child = capkpi.model.get_new_doc(doctype, parent_doc, parentfield);
		child.idx = idx;
	...
	copy_doc: function(doc, from_amend, parent_doc, parentfield) {
		...
		var newdoc = capkpi.model.get_new_doc(
			doc.doctype,
			parent_doc,
			parentfield
		);
		...
		for (var key in doc) {
			...
			var df = capkpi.meta.get_docfield(doc.doctype, key);
	...
	open_mapped_doc: function(opts) {
		...
		return capkpi.call({
			type: "POST",
			method: "capkpi.model.mapper.make_mapped_doc",
			args: {
				method: opts.method,
...
capkpi.create_routes = {};
capkpi.new_doc = function(doctype, opts, init_callback) {
	if (doctype === "File") {
	...
	return new Promise(resolve => {
		if (opts && $.isPlainObject(opts)) {
			capkpi.route_options = opts;
		}
		capkpi.model.with_doctype(doctype, function() {
			if (capkpi.create_routes[doctype]) {
				capkpi
					.set_route(capkpi.create_routes[doctype])
					.then(() => resolve());
			} else {
				capkpi.ui.form
