capkpi.provide('capkpi.model.user_settings');

$.extend(capkpi.model.user_settings, {
	get: function(doctype) {
		return capkpi.call('capkpi.model.utils.user_settings.get', { doctype })
			.then(r => JSON.parse(r.message || '{}'));
	},
	save: function(doctype, key, value) {
		if (capkpi.session.user === 'Guest') return Promise.resolve();

		const old_user_settings = capkpi.model.user_settings[doctype] || {};
	...
	remove: function(doctype, key) {
		var user_settings = capkpi.model.user_settings[doctype] || {};
	...
	update: function(doctype, user_settings) {
		if (capkpi.session.user === 'Guest') return Promise.resolve();
		return capkpi.call({
			method: 'capkpi.model.utils.user_settings.save',
			args: {
				doctype: doctype,
				user_settings: user_settings
			},
			callback: function(r) {
				capkpi.model.user_settings[doctype] = r.message;
			}
		});
	}
});

capkpi.get_user_settings = function(doctype, key) {
	var settings = capkpi.model.user_settings[doctype] || {};
	if(key) {
