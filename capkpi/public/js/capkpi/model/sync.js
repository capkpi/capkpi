...
$.extend(capkpi.model, {
	...
	sync: function(r) {
		/* docs:
			...
			from incoming request and set in `locals` and `capkpi.model.docinfo`
		*/
		var isPlain;
		...
		if(r.docs) {
			...
			for(var i=0, l=r.docs.length; i<l; i++) {
				...
				if (locals[d.doctype] && locals[d.doctype][d.name]) {
					// update values
					capkpi.model.update_in_locals(d);
				} else {
					capkpi.model.add_to_locals(d);
				}
				...
				if(d.localname) {
					capkpi.model.new_names[d.localname] = d.name;
					$(document).trigger('rename', [d.doctype, d.localname, d.name]);
					delete locals[d.doctype][d.localname];

					// update docinfo to new dict keys
					if(i===0) {
						capkpi.model.docinfo[d.doctype][d.name] = capkpi.model.docinfo[d.doctype][d.localname];
						capkpi.model.docinfo[d.doctype][d.localname] = undefined;
					}
				}
			}
		...
		if(r.docinfo) {
			...
			if(doc) {
				if(!capkpi.model.docinfo[doc.doctype])
					capkpi.model.docinfo[doc.doctype] = {};
				capkpi.model.docinfo[doc.doctype][doc.name] = r.docinfo;
			}
		}
	...
	add_to_locals: function(doc) {
		...
		if(!doc.name && doc.__islocal) { // get name (local if required)
			if(!doc.parentfield) capkpi.model.clear_doc(doc);

			doc.name = capkpi.model.get_new_name(doc.doctype);

			if(!doc.parentfield) capkpi.provide("capkpi.model.docinfo." + doc.doctype + "." + doc.name);
		}
	...
	update_in_locals: function(doc) {
		...
		for (let fieldname in doc) {
			let df = capkpi.meta.get_field(doc.doctype, fieldname);
			if (df && capkpi.model.table_fields.includes(df.fieldtype)) {
				// table
				...
				for (let i=0; i < doc[fieldname].length; i++ ) {
					...
					if (local_d) {
						...
						if (!d.name) {
							...
							d.name = capkpi.model.get_new_name(doc.doctype);
						}
