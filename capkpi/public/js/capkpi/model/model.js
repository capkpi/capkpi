...
$.extend(capkpi.model, {
	...
	init: function() {
		...
		capkpi.realtime.on("doc_update", function(data) {
			// set list dirty
			capkpi.views.ListView.trigger_list_update(data);
			var doc = locals[data.doctype] && locals[data.doctype][data.name];

			if(doc) {
				// current document is dirty, show message if its not me
				if(capkpi.get_route()[0]==="Form" && cur_frm.doc.doctype===doc.doctype && cur_frm.doc.name===doc.name) {
					if(!capkpi.ui.form.is_saving && data.modified!=cur_frm.doc.modified) {
					...
					if(!doc.__unsaved) {
						...
						capkpi.model.remove_from_locals(doc.doctype, doc.name);
					} else {
		...
		capkpi.realtime.on("list_update", function(data) {
			capkpi.views.ListView.trigger_list_update(data);
		});
	...
	is_value_type: function(fieldtype) {
		...
		return capkpi.model.no_value_type.indexOf(fieldtype)===-1;
	},

	is_non_std_field: function(fieldname) {
		return !capkpi.model.std_fields_list.includes(fieldname);
	},

	get_std_field: function(fieldname, ignore=false) {
		var docfield = $.map([].concat(capkpi.model.std_fields).concat(capkpi.model.std_fields_table),
			function(d) {
		...
		if (!docfield.length) {
			...
			if (ignore) {
				...
				capkpi.msgprint(__("Unknown Column: {0}", [fieldname]));
			}
		}
		return docfield[0];
	},
	...
	set_in_localstorage: function(doctype, docs) {
		try {
			...
			capkpi.model.clear_local_storage();
			localStorage["_doctype:" + doctype] = JSON.stringify(docs);
		}
	},
	...
	with_doctype: function(doctype, callback, async) {
		if(locals.DocType[doctype]) {
			...
			let cached_docs = capkpi.model.get_from_localstorage(doctype);
			...
			return capkpi.call({
				method:'capkpi.desk.form.load.getdoctype',
				type: "GET",
				args: {
					doctype: doctype,
					with_parent: 1,
				...
				callback: function(r) {
					if(r.exc) {
						capkpi.msgprint(__("Unable to load: {0}", [__(doctype)]));
						throw "No doctype";
					}
					if(r.message=="use_cache") {
						capkpi.model.sync(cached_doc);
					} else {
						capkpi.model.set_in_localstorage(doctype, r.docs)
					}
					capkpi.model.init_doctype(doctype);

					if(r.user_settings) {
						// remember filters and other settings from last view
						capkpi.model.user_settings[doctype] = JSON.parse(r.user_settings);
						capkpi.model.user_settings[doctype].updated_on = moment().toString();
					}
					callback && callback(r);
				}
			});
	...
	init_doctype: function(doctype) {
		...
		if(meta.__templates) {
			$.extend(capkpi.templates, meta.__templates);
		}
	},
	...
	with_doc: function(doctype, name, callback) {
		return new Promise(resolve => {
			...
			if(locals[doctype] && locals[doctype][name] && capkpi.model.get_docinfo(doctype, name)) {
				callback && callback(name);
				resolve(capkpi.get_doc(doctype, name));
			} else {
				return capkpi.call({
					method: 'capkpi.desk.form.load.getdoc',
					type: "GET",
					args: {
						doctype: doctype,
						name: name
	...
	get_docinfo: function(doctype, name) {
		return capkpi.model.docinfo[doctype] && capkpi.model.docinfo[doctype][name] || null;
	},

	set_docinfo: function(doctype, name, key, value) {
		if (capkpi.model.docinfo[doctype] && capkpi.model.docinfo[doctype][name]) {
			capkpi.model.docinfo[doctype][name][key] = value;
		}
	},

	get_shared: function(doctype, name) {
		return capkpi.model.get_docinfo(doctype, name).shared;
	},

	get_server_module_name: function(doctype) {
		var dt = capkpi.model.scrub(doctype);
		var module = capkpi.model.scrub(locals.DocType[doctype].module);
		var app = capkpi.boot.module_app[module];
	...
	can_create: function(doctype) {
		return capkpi.boot.user.can_create.indexOf(doctype)!==-1;
	},

	can_select: function(doctype) {
		if (capkpi.boot.user) {
			return capkpi.boot.user.can_select.indexOf(doctype)!==-1;
		}
	},

	can_read: function(doctype) {
		if (capkpi.boot.user) {
			return capkpi.boot.user.can_read.indexOf(doctype)!==-1;
		}
	},

	can_write: function(doctype) {
		return capkpi.boot.user.can_write.indexOf(doctype)!==-1;
	},

	can_get_report: function(doctype) {
		return capkpi.boot.user.can_get_report.indexOf(doctype)!==-1;
	},

	can_delete: function(doctype) {
		if(!doctype) return false;
		return capkpi.boot.user.can_delete.indexOf(doctype)!==-1;
	},

	can_cancel: function(doctype) {
		if(!doctype) return false;
		return capkpi.boot.user.can_cancel.indexOf(doctype)!==-1;
	},

	has_workflow: function(doctype) {
		return capkpi.get_list('Workflow', {'document_type': doctype,
			'is_active': 1}).length;
	},
	...
	is_single: function(doctype) {
		if(!doctype) return false;
		return capkpi.boot.single_types.indexOf(doctype) != -1;
	},

	is_tree: function(doctype) {
		if (!doctype) return false;
		return capkpi.boot.treeviews.indexOf(doctype) != -1;
	},
	...
	can_import: function(doctype, frm) {
		...
		if(capkpi.user_roles.includes("System Manager")) return true;

		if(frm) return frm.perm[0].import===1;
		return capkpi.boot.user.can_import.indexOf(doctype)!==-1;
	},
	...
	can_export: function(doctype, frm) {
		...
		return capkpi.boot.user.can_export.indexOf(doctype)!==-1;
	},

	can_print: function(doctype, frm) {
		if(frm) return frm.perm[0].print===1;
		return capkpi.boot.user.can_print.indexOf(doctype)!==-1;
	},

	can_email: function(doctype, frm) {
		if(frm) return frm.perm[0].email===1;
		return capkpi.boot.user.can_email.indexOf(doctype)!==-1;
	},
	...
	can_share: function(doctype, frm) {
		...
		return capkpi.boot.user.can_share.indexOf(doctype)!==-1;
	},
	...
	can_set_user_permissions: function(doctype, frm) {
		...
		return capkpi.boot.user.can_set_user_permissions.indexOf(doctype)!==-1;
	},

	has_value: function(dt, dn, fn) {
		// return true if property has value
		var val = locals[dt] && locals[dt][dn] && locals[dt][dn][fn];
		var df = capkpi.meta.get_docfield(dt, fn, dn);

		if(capkpi.model.table_fields.includes(df.fieldtype)) {
			var ret = false;
	...
	get_list: function(doctype, filters) {
		...
		if($.isEmptyObject(docsdict))
			return [];
		return capkpi.utils.filter_dict(docsdict, filters);
	},
	...
	get_value: function(doctype, filters, fieldname, callback) {
		if(callback) {
			...
			if(typeof filters==="string" && locals[doctype] && locals[doctype][filters]) {
				...
				var l = capkpi.get_list(doctype, filters);
	...
	set_value: function(doctype, docname, fieldname, value, fieldtype) {
		...
		$.each(to_update, (key, value) => {
			if (doc && doc[key] !== value) {
				...
				doc[key] = value;
				tasks.push(() => capkpi.model.trigger(key, value, doc));
			} else {
	...
	on: function(doctype, fieldname, fn) {
		...
		*/
		/* example: capkpi.model.on("Customer", "age", function(fieldname, value, doc) {
		  if(doc.age < 16) {
		   	capkpi.msgprint("Warning, Customer must atleast be 16 years old.");
		    raise "CustomerAgeError";
		  }
		}) */
		capkpi.provide("capkpi.model.events." + doctype);
		if(!capkpi.model.events[doctype][fieldname]) {
			capkpi.model.events[doctype][fieldname] = [];
		}
		capkpi.model.events[doctype][fieldname].push(fn);
	},
		...
		function enqueue_events(events) {
			...
			for (const fn of events) {
				...
				tasks.push(() => {
					...
					// or use the default promise capkpi.after_ajax
					if (return_value && return_value.then) {
						return return_value;
					} else {
						return capkpi.after_server_call();
					}
				});
			}
		};

		if(capkpi.model.events[doc.doctype]) {
			enqueue_events(capkpi.model.events[doc.doctype][fieldname]);
			enqueue_events(capkpi.model.events[doc.doctype]['*']);
		}
	...
	get_doc: function(doctype, name) {
		...
		if($.isPlainObject(name)) {
			var doc = capkpi.get_list(doctype, name);
	...
	get_children: function(doctype, parent, parentfield, filters) {
		if($.isPlainObject(doctype)) {
			...
			var doc = capkpi.get_doc(doctype, parent);
		}

		var children = doc[parentfield] || [];
		if(filters) {
			return capkpi.utils.filter_dict(children, filters);
		} else {
			return children;
		}
	},
	...
	remove_from_locals: function(doctype, name) {
		...
		if(capkpi.views.formview[doctype]) {
			delete capkpi.views.formview[doctype].frm.opendocs[name];
		}
	},
	...
	get_no_copy_list: function(doctype) {
		...
		var docfields = capkpi.get_doc("DocType", doctype).fields || [];
	...
	delete_doc: function(doctype, docname, callback) {
		let title = docname;
		const title_field = capkpi.get_meta(doctype).title_field;
		if (capkpi.get_meta(doctype).autoname == "hash" && title_field) {
			const value = capkpi.model.get_value(doctype, docname, title_field);
			if (value) {
				title = `${value} (${docname})`;
			}
		}
		capkpi.confirm(__("Permanently delete {0}?", [title.bold()]), function() {
			return capkpi.call({
				method: 'capkpi.client.delete',
				args: {
					doctype: doctype,
					name: docname
				},
				callback: function(r, rt) {
					if(!r.exc) {
						capkpi.utils.play_sound("delete");
						capkpi.model.clear_doc(doctype, docname);
	...
	round_floats_in: function(doc, fieldnames) {
		if(!fieldnames) {
			fieldnames = capkpi.meta.get_fieldnames(doc.doctype, doc.parent,
	...
	validate_missing: function(doc, fieldname) {
		if(!doc[fieldname]) {
			capkpi.throw(__("Please specify") + ": " +
				__(capkpi.meta.get_label(doc.doctype, fieldname, doc.parent || doc.name)));
		}
	},
	...
	is_numeric_field: function(fieldtype) {
		...
		return capkpi.model.numeric_fieldtypes.includes(fieldtype);
	}
});

// legacy
capkpi.get_doc = capkpi.model.get_doc;
capkpi.get_children = capkpi.model.get_children;
capkpi.get_list = capkpi.model.get_list;
