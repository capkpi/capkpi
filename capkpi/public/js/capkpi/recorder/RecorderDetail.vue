...
export default {
	...
	methods: {
		...
		refresh: function() {
			capkpi.call("capkpi.recorder.get").then( r => this.requests = r.message);
		},
		update: function(message) {
			this.requests.push(JSON.parse(message));
		},
		clear: function() {
			capkpi.call("capkpi.recorder.delete").then(r => this.refresh());
		},
		start: function() {
			capkpi.call("capkpi.recorder.start").then(r => this.fetch_status());
		},
		stop: function() {
			capkpi.call("capkpi.recorder.stop").then(r => this.fetch_status());
		},
		fetch_status: function() {
			capkpi.call("capkpi.recorder.status").then(r => this.update_status(r.message));
		},
		update_status: function(result) {
			...
			if(this.status.status == "Active") {
				capkpi.realtime.on("recorder-dump-event", this.update);
			} else {
				capkpi.realtime.off("recorder-dump-event", this.update);
			}
		...
		export_data: function() {
			...
			capkpi.call("capkpi.recorder.export_data")
				.then((r) => {
