...
window.format_number = function (v, format, decimals) {
	if (!format) {
		...
		if (decimals == null) decimals = cint(capkpi.defaults.get_default("float_precision")) || 3;
	}
...
function format_currency(v, currency, decimals) {
	...
	if(decimals === undefined) {
		decimals = capkpi.boot.sysdefaults.currency_precision || null;
	}
...
function get_currency_symbol(currency) {
	if (capkpi.boot) {
		if (capkpi.boot.sysdefaults && capkpi.boot.sysdefaults.hide_currency_symbol == "Yes")
			return null;

		if (!currency)
			currency = capkpi.boot.sysdefaults.currency;

		return capkpi.model.get_value(":Currency", currency, "symbol") || currency;
	} else {
		// load in template
		return capkpi.currency_symbols[currency];
	}
}

function get_number_format(currency) {
	return (capkpi.boot && capkpi.boot.sysdefaults && capkpi.boot.sysdefaults.number_format) || "#,###.##";
}

function get_number_format_info(format) {
	var info = capkpi.number_format_info[format];
...
function precision(fieldname, doc) {
	if (cur_frm) {
		...
		var df = capkpi.meta.get_docfield(doc.doctype, fieldname, doc.parent || doc.name);
		if (!df) console.log(fieldname + ": could not find docfield in method precision()");
		return capkpi.meta.get_field_precision(df, doc);
	} else {
		return capkpi.boot.sysdefaults.float_precision
	}
}
...
function round_based_on_smallest_currency_fraction(value, currency, precision) {
	var smallest_currency_fraction_value = flt(capkpi.model.get_value(":Currency",
