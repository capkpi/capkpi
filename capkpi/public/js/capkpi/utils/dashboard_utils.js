capkpi.dashboard_utils = {
	render_chart_filters: function(filters, button_class, container, append) {
		filters.forEach(filter => {
			...
			if (filter.icon) {
				icon_html = capkpi.utils.icon(filter.icon);
			}
	...
	get_filters_for_chart_type: function(chart) {
		if (chart.chart_type === 'Custom' && chart.source) {
			const method = 'capkpi.desk.doctype.dashboard_chart_source.dashboard_chart_source.get_config';
			return capkpi.xcall(method, {name: chart.source}).then(config => {
				capkpi.dom.eval(config);
				return capkpi.dashboards.chart_sources[chart.source].filters;
			});
		} else if (chart.chart_type === 'Report' && chart.report_name) {
			return capkpi.report_utils.get_report_filters(chart.report_name).then(filters => {
				return filters;
			});
		} else {
	...
	get_dashboard_settings() {
		return capkpi.db.get_list('Dashboard Settings', {
			filters: {
	...
	create_dashboard_settings() {
		return capkpi.xcall(
			'capkpi.desk.doctype.dashboard_settings.dashboard_settings.create_dashboard_settings',
			{
	...
	get_years_since_creation(creation) {
		...
		let current_year = this.get_year(capkpi.datetime.now_date());
		let years_list = [];
		...
		let fields = [
			{
				...
				options:
					`<div>
						...
						<p>Ex:
							<code>capkpi.defaults.get_user_default("Company")</code>
						</p>
					</div>`
			}
		];
		...
		if ($.isArray(dynamic_filters)) {
			dynamic_filters.forEach(f => {
				try {
					...
					capkpi.throw(__("Invalid expression set in filter {0} ({1})", [f[1], f[0]]));
				}
			});
			filters = [...filters, ...dynamic_filters];
		} else {
			for (let key of Object.keys(dynamic_filters)) {
				try {
					...
					capkpi.throw(__("Invalid expression set in filter {0}", [key]));
				}
			}
	...
	get_add_to_dashboard_dialog(docname, doctype, method) {
		...
		const dialog = new capkpi.ui.Dialog({
			...
			primary_action: (values) => {
				values.name = docname;
				values.set_standard = capkpi.boot.developer_mode;
				capkpi.xcall(
					method,
					{args: values}
				).then(()=> {
