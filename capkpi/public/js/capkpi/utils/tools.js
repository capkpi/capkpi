...
capkpi.provide("capkpi.tools");

capkpi.tools.downloadify = function(data, roles, title) {
	...
	var csv_data = capkpi.tools.to_csv(data);
...
capkpi.markdown = function(txt) {
	if(!capkpi.md2html) {
		capkpi.md2html = new showdown.Converter();
	}
	...
	return capkpi.md2html.makeHtml(txt);
};

capkpi.tools.to_csv = function(data) {
	var res = [];
