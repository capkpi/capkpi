function prettyDate(date, mini) {
	...
	if (typeof (date) == "string") {
		date = capkpi.datetime.convert_to_user_tz(date);
...
window.comment_when = function(datetime, mini) {
	var timestamp = capkpi.datetime.str_to_user ?
		capkpi.datetime.str_to_user(datetime) : datetime;
	return '<span class="capkpi-timestamp '
		+ (mini ? " mini" : "") + '" data-timestamp="' + datetime
		+ '" title="' + timestamp + '">'
		+ prettyDate(datetime, mini) + '</span>';
};
capkpi.datetime.comment_when = comment_when;
capkpi.datetime.prettyDate = prettyDate;

capkpi.datetime.refresh_when = function() {
	if (jQuery) {
		$(".capkpi-timestamp").each(function() {
