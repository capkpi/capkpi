...
export default class OnboardingWidget extends Widget {
	...
	add_step(step, index) {
		...
		let $step = $(`<a class="onboarding-step ${status}">
				<div class="step-title">
					...
					<div class="step-index step-skipped">${capkpi.utils.icon('tick', 'xs')}</div>
					<div class="step-index step-complete">${capkpi.utils.icon('tick', 'xs')}</div>
	...
	show_step(step) {
		...
		let actions = {
			...
			"Create Entry": (step) => {
				if (step.is_complete) {
					capkpi.set_route(`/app/List/${step.reference_document}`);
				} else {
		...
		const toggle_content = () => {
			...
			this.step_body.html(
				step.description ?
					capkpi.markdown(step.description)
					: `<h1>${step.title}</h1>`
			);
	...
	go_to_page(step) {
		...
		capkpi.set_route(step.path).then(() => {
			...
			let msg_dialog = capkpi.msgprint({
				message: message,
				title: title,
				primary_action: {
					action: () => {
	...
	open_report(step) {
		let route = capkpi.utils.generate_route({
		...
		let current_route = capkpi.get_route();

		capkpi.set_route(route).then(() => {
			let msg_dialog = capkpi.msgprint({
				message: __(step.report_description),
				title: __(step.reference_report),
				primary_action: {
					action: () => {
						capkpi.set_route(current_route).then(() => {
							this.mark_complete(step);
						});
						msg_dialog.hide();
					},
			...
			capkpi.msg_dialog.custom_onhide = () => this.mark_complete(step);
		});
	}
	...
	show_form_tour(step) {
		let route;
		if (step.is_single) {
			route = capkpi.router.slug(step.reference_document);
		} else {
			route = `${capkpi.router.slug(step.reference_document)}/new`;
		}

		let current_route = capkpi.get_route();

		capkpi.route_hooks = {};
		capkpi.route_hooks.after_load = (frm) => {
			frm.show_tour(() => {
				let msg_dialog = capkpi.msgprint({
	...
	update_settings(step) {
		...
		capkpi.route_hooks.after_save = (frm) => {
			let success = false;
			let args = {};
			...
			if (success) {
				...
				args.secondary_action = {
					action: () => capkpi.set_route(current_route),
					label: __("Go Back"),
				};
			...
			capkpi.msgprint(args);
			capkpi.msg_dialog.custom_onhide = () => custom_onhide();
		};

		capkpi.set_route("Form", step.reference_document);
	}
	...
	create_entry(step) {
		...
		if (step.is_submittable) {
			capkpi.route_hooks.after_save = () => {
				capkpi.msgprint({
					message: __("Submit this document to complete this step."),
					title: __("Great")
				});
			};
			capkpi.route_hooks.after_submit = callback;
		} else {
			capkpi.route_hooks.after_save = callback;
		}

		capkpi.set_route('Form', step.reference_document, 'new');
	}

	show_quick_entry(step) {
		let current_route = capkpi.get_route_str();
		capkpi.ui.form.make_quick_entry(
			step.reference_document,
			() => {
				if (capkpi.get_route_str() != current_route) {
					let success_dialog = capkpi.msgprint({
						message: __("Let's take you back to onboarding"),
	...
	show_video(step) {
		capkpi.help.show_video(step.video_url, step.title);
		this.mark_complete(step);
	}
	...
	update_step_status(step, status, value, callback) {
		...
		capkpi
			.call("capkpi.desk.desktop.update_onboarding_step", {
				name: step.name,
				field: status,
				value: value,
			})
	...
	show_success() {
		...
		let success_state_image =
			...
			"/assets/capkpi/images/ui-states/success-color.png";
		let documentation = "";
		if (this.docs_url) {
			documentation = __(
	...
	is_dismissed() {
		...
		if (Object.keys(dismissed).includes(this.label)) {
			...
			let today = new Date();
			let diff = capkpi.datetime.get_hour_diff(today, last_hidden);
			return diff < 24;
		}
		return false;
	}
	...
	set_actions() {
		...
		dismiss.on("click", () => {
			...
			dismissed[this.label] = capkpi.datetime.now_datetime();
