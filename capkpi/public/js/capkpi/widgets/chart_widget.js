...
export default class ChartWidget extends Widget {
	...
	render_time_series_filters() {
		...
		capkpi.dashboard_utils.render_chart_filters(
			filters,
			"chart-actions",
			this.action_area,
			0
	...
	get_time_series_filters() {
		...
		if (this.chart_doc.type == 'Heatmap') {
			filters = [{
				...
				options: capkpi.dashboard_utils.get_years_since_creation(capkpi.boot.user.creation),
	...
	render_date_range_field() {
		if (
			...
			this.date_range_field = capkpi.ui.form.make_control({
				df: {
					fieldtype: 'DateRange',
					fieldname: 'from_date',
	...
	get_report_chart_data(result) {
		if (result.chart && this.chart_doc.use_report_chart) {
			...
			let columns = result.columns.map(col => {
				return capkpi.report_utils.prepare_field_from_column(col);
			});

			let data = capkpi.report_utils.make_chart_options(
				columns,
				result,
				chart_fields
			).data;
	...
	prepare_chart_actions() {
		...
		if (this.chart_doc.document_type) {
			actions.push({
				...
				action: "action-list",
				handler: () => {
					capkpi.set_route("List", this.chart_doc.document_type);
				}
			});
	...
	setup_filter_dialog(fields) {
		...
		if (this.chart_doc.chart_type == 'Report') {
			...
			capkpi.query_reports[this.chart_doc.report_name].onload
					&& capkpi.query_reports[this.chart_doc.report_name].onload(capkpi.query_report);
		}
	...
	save_chart_config_for_user(config, reset=0) {
		...
		capkpi.xcall('capkpi.desk.doctype.dashboard_settings.dashboard_settings.save_chart_config', {
			'reset': reset,
	...
	create_filter_group_and_add_filters() {
		this.filter_group = new capkpi.ui.FilterGroup({
		...
		this.filters &&
			capkpi.model.with_doctype(this.chart_doc.document_type, () => {
	...
	fetch(filters, refresh = false, args) {
		...
		return capkpi.xcall(method, args);
	}
	...
	async get_source_doctype() {
		...
		if (this.chart_doc.chart_type == "Report" && this.chart_doc.report_name) {
			return await capkpi.db.get_value("Report", this.chart_doc.report_name, "ref_doctype").then(r => r.message.ref_doctype);
		}
	}
	...
	async render() {
		let setup_dashboard_chart = () => {
			...
			if (!this.dashboard_chart) {
				this.dashboard_chart = capkpi.utils.make_chart(this.chart_wrapper[0], chart_args);
			} else {
		...
		if (!this.data || !this.data.labels || !Object.keys(this.data).length) {
			...
			if (this.chart_doc.document_type) {
				capkpi.model.with_doctype(this.chart_doc.document_type, setup_dashboard_chart);
			} else {
	...
	get_chart_args() {
		...
		if (this.chart_doc.document_type) {
			let doctype_meta = capkpi.get_meta(this.chart_doc.document_type);
		...
		chart_args.tooltipOptions = {
			formatTooltipY: value => capkpi.format(value, { fieldtype, options }, { always_show_decimals: true, inline: true })
		};
	...
	update_chart_object() {
		capkpi.db.get_doc("Dashboard Chart", this.chart_doc.name).then(doc => {
			this.chart_doc = doc;
	...
	prepare_chart_object() {
		if (this.chart_doc.type == 'Heatmap' && !this.chart_doc.heatmap_year) {
			this.chart_doc.heatmap_year = capkpi.dashboard_utils.get_year(capkpi.datetime.now_date());
		}
	...
	set_chart_filters() {
		...
		let chart_saved_filters = capkpi.dashboard_utils.get_all_filters(this.chart_doc);

		if (this.chart_doc.chart_type == 'Report') {
			return capkpi.dashboard_utils
				.get_filters_for_chart_type(this.chart_doc).then(filters => {
					...
					this.filters =
					capkpi.utils.parse_array(user_saved_filters) || capkpi.utils.parse_array(this.filters)
						|| capkpi.utils.parse_array(chart_saved_filters);
				});
		} else {
			this.filters =
				capkpi.utils.parse_array(user_saved_filters) || capkpi.utils.parse_array(this.filters)
			...
			.then(chart_doc => {
				if (chart_doc) {
					...
					if (this.chart_doc.chart_type == "Custom") {
						// custom source
						if (
							capkpi.dashboards.chart_sources[this.chart_doc.source]
						) {
							this.settings =
								capkpi.dashboards.chart_sources[
									this.chart_doc.source
								];
							return Promise.resolve();
						} else {
							const method =
								"capkpi.desk.doctype.dashboard_chart_source.dashboard_chart_source.get_config";
							return capkpi
								.xcall(method, { name: this.chart_doc.source })
								.then(config => {
									capkpi.dom.eval(config);
									this.settings =
										capkpi.dashboards.chart_sources[
											this.chart_doc.source
										];
						...
						this.settings = {
							method: "capkpi.desk.doctype.dashboard_chart.dashboard_chart.get"
						};
						return Promise.resolve();
					}
				}
