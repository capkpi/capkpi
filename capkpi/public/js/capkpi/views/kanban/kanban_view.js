...
capkpi.views.KanbanView = class KanbanView extends capkpi.views.ListView {
	static load_last_view() {
		const route = capkpi.get_route();
		if (route.length === 3) {
			const doctype = route[1];
			const user_settings = capkpi.get_user_settings(doctype)['Kanban'] || {};
			if (!user_settings.last_kanban_board) {
				...
				capkpi.set_route('List', doctype, 'List');
				return true;
			}
	...
	setup_defaults() {
		return super.setup_defaults()
			.then(() => {
				this.board_name = capkpi.get_route()[3];
	...
	get_board() {
		return capkpi.db.get_doc('Kanban Board', this.board_name)
			.then(board => {
				this.board = board;
	...
	save_kanban_board_filters() {
		...
		capkpi.call({
			method: 'capkpi.desk.doctype.kanban_board.kanban_board.save_filters',
			args: {
	...
	render() {
		...
		this.kanban = new capkpi.views.KanbanBoard({
	...
	get_card_meta() {
		var meta = capkpi.get_meta(this.doctype);
		var doc = capkpi.model.get_new_doc(this.doctype);
		var title_field = null;
		var quick_entry = false;

		if (this.meta.title_field) {
			title_field = capkpi.meta.get_field(this.doctype, this.meta.title_field);
		}
		...
		if (mandatory.some(df => capkpi.model.table_fields.includes(df.fieldtype)) || mandatory.length > 1) {
			quick_entry = true;
		}

		if (!title_field) {
			title_field = capkpi.meta.get_field(this.doctype, 'name');
		}
	...
	get required_libs() {
		return [
			'assets/capkpi/js/lib/fluxify.min.js',
			'assets/capkpi/js/capkpi/views/kanban/kanban_board.js'
		];
	}
};


capkpi.views.KanbanView.get_kanbans = function (doctype) {
	let kanbans = [];
		...
		.then((kanban_boards) => {
			if (kanban_boards) {
				kanban_boards.forEach(board => {
					let route = `/app/${capkpi.router.slug(board.reference_doctype)}/view/kanban/${board.name}`;
	...
	function get_kanban_boards() {
		return capkpi.call('capkpi.desk.doctype.kanban_board.kanban_board.get_kanban_boards', { doctype })
			.then(r => r.message);
	}
};


capkpi.views.KanbanView.show_kanban_dialog = function (doctype) {
	let dialog = new_kanban_dialog();
	dialog.show();

	function make_kanban_board(board_name, field_name, project) {
		return capkpi.call({
			method: 'capkpi.desk.doctype.kanban_board.kanban_board.quick_kanban_board',
			args: {
				doctype,
				board_name,
				field_name,
			...
			callback: function (r) {
				var kb = r.message;
				if (kb.filters) {
					capkpi.provide('capkpi.kanban_filters');
					capkpi.kanban_filters[kb.kanban_board_name] = kb.filters;
				}
				capkpi.set_route('List', doctype, 'Kanban', kb.kanban_board_name);
			}
		});
	}
	...
	function new_kanban_dialog() {
		...
		const select_fields = capkpi.get_meta(doctype).fields.filter((df) => {
		...
		let primary_action = () => {
			if (to_save) {
				...
				make_kanban_board(values.board_name, values.field_name, values.project).then(
					() => dialog.hide(),
					(err) => capkpi.msgprint(err)
				);
			} else {
				capkpi.set_route("Form", "Customize Form", {"doc_type": doctype});
			}
		};

		return new capkpi.ui.Dialog({
			title: dialog_title,
