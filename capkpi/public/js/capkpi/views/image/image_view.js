/**
 * capkpi.views.ImageView
 */
capkpi.provide("capkpi.views");

capkpi.views.ImageView = class ImageView extends capkpi.views.ListView {
	get view_name() {
		return "Image";
	}
	...
	item_html(item) {
		...
		const escaped_title = capkpi.utils.escape_html(title);
	...
	get_attached_images() {
		return capkpi
			.call({
				method: "capkpi.core.doctype.file.file.get_attached_images",
				args: {
	...
	setup_gallery() {
		var me = this;
		this.gallery = new capkpi.views.GalleryView({
			doctype: this.doctype,
			items: this.items,
			wrapper: this.$result,
...
capkpi.views.GalleryView = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		var me = this;
	...
	prepare: function() {
		...
		if (this.pswp_root.length === 0) {
			var pswp = capkpi.render_template("photoswipe_dom");
	...
	load_lib: function() {
		return new Promise(resolve => {
			var asset_dir = "assets/capkpi/js/lib/photoswipe/";
			capkpi.require(
				[
