...
capkpi.views.FileView = class FileView extends capkpi.views.ListView {
	static load_last_view() {
		const route = capkpi.get_route();
		if (route.length === 2) {
			const view_user_settings = capkpi.get_user_settings("File", "File");
			capkpi.set_route(
				"List",
				"File",
				view_user_settings.last_folder || capkpi.boot.home_folder
			);
			return true;
		}
	...
	setup_view() {
		...
		this.page.add_button(__("Toggle Grid View"), () => {
			capkpi.views.FileView.grid_view = !capkpi.views.FileView.grid_view;
			this.refresh();
		});
	}
	...
	get_args() {
		...
		if (capkpi.views.FileView.grid_view) {
			Object.assign(args, {
		...
		const items = [
			{
				label: __("Home"),
				action: () => {
					capkpi.set_route("List", "File", "Home");
				}
			},
			{
				label: __("New Folder"),
				action: () => {
					capkpi.prompt(
						...
						values => {
							...
							capkpi.call({
								method:
									"capkpi.core.doctype.file.file.create_new_folder",
								args: data
							});
						},
			...
			},
			{
				...
				action: () => {
					new capkpi.ui.FileUploader({
						...
						on_success: file => {
							capkpi.show_alert(__("Unzipping files..."));
							capkpi
								.call(
									"capkpi.core.doctype.file.file.unzip_file",
									{
										name: file.name
									}
								)
	...
	add_file_action_buttons() {
		...
		this.$paste_btn = this.page
			.add_button(__("Paste"), () =>
				capkpi.file_manager.paste(this.current_folder)
			)
			.hide();
	}
	...
	set_fields() {
		this.fields = this.meta.fields
			.filter(
				df => capkpi.model.is_value_type(df.fieldtype) && !df.hidden
			)
			.map(df => df.fieldname)
	...
	prepare_datum(d) {
		...
		if (d.is_folder) {
			...
			type = "folder";
		} else if (capkpi.utils.is_image_file(d.file_name)) {
			icon_class = "image";
			type = "image";
		} else {
			icon_class = "file";
	...
	before_render() {
		...
		capkpi.model.user_settings.save(
			"File",
			"grid_view",
			capkpi.views.FileView.grid_view
		);
	...
	get_breadcrumbs_html() {
		const route = capkpi.router.parse();
	...
	get_header_html() {
		...
		let header_selector_html = !capkpi.views.FileView.grid_view
			? `<input class="level-item list-check-all hidden-xs" type="checkbox" title="${__(
				"Select All"
			)}">`
			: "";

		let header_columns_html = !capkpi.views.FileView.grid_view
	...
	get_creation_date(file) {
		...
		let created_on;
		if (date === capkpi.datetime.now_date()) {
			created_on = comment_when(file.creation);
		} else {
			created_on = capkpi.datetime.str_to_user(date);
		}
		return created_on;
	}
	...
	get_left_html(file) {
		...
		const file_size = file.file_size
			? capkpi.form.formatters.FileSize(file.file_size)
			: "";
		...
		this.$result.on("drop", e => {
			...
			if (dataTransfer.files && dataTransfer.files.length > 0) {
				...
				if ($el.parents(".folders").length !== 0) {
					...
					capkpi.file_manager.paste(folder_name);
					capkpi.show_alert(
	...
	toggle_cut_paste_buttons() {
		const hide_paste_btn =
			!capkpi.file_manager.can_paste ||
			capkpi.file_manager.old_folder === this.current_folder;
...
capkpi.views.FileView.grid_view =
	capkpi.get_user_settings("File").grid_view || false;
