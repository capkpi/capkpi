...
capkpi.provide("capkpi.views.calendar");
capkpi.provide("capkpi.views.calendars");

capkpi.views.CalendarView = class CalendarView extends capkpi.views.ListView {
	static load_last_view() {
		const route = capkpi.get_route();
		if (route.length === 3) {
			const doctype = route[1];
			const user_settings = capkpi.get_user_settings(doctype)['Calendar'] || {};
	...
	setup_defaults() {
		return super.setup_defaults()
			.then(() => {
				...
				this.calendar_settings = capkpi.views.calendar[this.doctype] || {};
				this.calendar_name = capkpi.get_route()[3];
			});
	}
	...
	render() {
		...
		this.load_lib
			...
			.then(options => {
				this.calendar = new capkpi.views.Calendar(options);
			});
	}
	...
	get_calendar_preferences() {
		...
		return new Promise(resolve => {
			if (calendar_name === 'default') {
				Object.assign(options, capkpi.views.calendar[this.doctype]);
				resolve(options);
			} else {
				capkpi.model.with_doc('Calendar View', calendar_name, () => {
					const doc = capkpi.get_doc('Calendar View', calendar_name);
					if (!doc) {
						capkpi.show_alert(__("{0} is not a valid Calendar. Redirecting to default Calendar.", [calendar_name.bold()]));
						capkpi.set_route("List", this.doctype, "Calendar", "default");
						return;
					}
	...
	get required_libs() {
		let assets = [
			'assets/capkpi/js/lib/fullcalendar/fullcalendar.min.css',
			'assets/capkpi/js/lib/fullcalendar/fullcalendar.min.js',
		];
		let user_language = capkpi.boot.user.language;
		if (user_language && user_language !== 'en') {
			assets.push('assets/capkpi/js/lib/fullcalendar/locale-all.js');
		}
		return assets;
	}
};

capkpi.views.Calendar = Class.extend({
	...
	make_page: function() {
		...
		$.each(capkpi.boot.calendars, function(i, doctype) {
			if(capkpi.model.can_read(doctype)) {
				me.page.add_menu_item(__(doctype), function() {
					capkpi.set_route("List", doctype, "Calendar");
				});
			}
		});
	...
	make: function() {
		...
		this.footnote_area = capkpi.utils.set_footnote(this.footnote_area, this.$wrapper,
	...
	set_css: function() {
		...
		this.$wrapper.find('.fc-prev-button span')
			.attr('class', '').html(capkpi.utils.icon('left'));
		this.$wrapper.find('.fc-next-button span')
			.attr('class', '').html(capkpi.utils.icon('right'));

		this.$wrapper.find('.fc-today-button')
			.prepend(capkpi.utils.icon('today'));
	...
	get_system_datetime: function(date) {
		date._offset = (moment(date).tz(capkpi.sys_defaults.time_zone)._offset);
		return capkpi.datetime.convert_to_system_tz(date);
	},
	setup_options: function(defaults) {
		var me = this;
		defaults.meridiem = 'false';
		this.cal_options = {
			locale: capkpi.boot.user.language || "en",
			header: {
			...
			events: function(start, end, timezone, callback) {
				return capkpi.call({
					method: me.get_events_method || "capkpi.desk.calendar.get_events",
					type: "GET",
			...
			eventClick: function(event) {
				...
				if(capkpi.model.can_read(doctype)) {
					capkpi.set_route("Form", doctype, event.name);
				}
			},
			...
			select: function(startDate, endDate, jsEvent, view) {
				...
				var event = capkpi.model.get_new_doc(me.doctype);
				...
				capkpi.set_route("Form", me.doctype, event.name);
			},
	...
	prepare_events: function(events) {
		...
		return (events || []).map(d => {
			d.id = d.name;
			d.editable = capkpi.model.can_write(d.doctype || me.doctype);
			...
			// convert to user tz
			d.start = capkpi.datetime.convert_to_user_tz(d.start);
			d.end = capkpi.datetime.convert_to_user_tz(d.end);

			// show event on single day if start or end date is invalid
			if (!capkpi.datetime.validate(d.start) && d.end) {
				d.start = capkpi.datetime.add_days(d.end, -1);
			}

			if (d.start && !capkpi.datetime.validate(d.end)) {
				d.end = capkpi.datetime.add_days(d.start, 1);
			}

			me.fix_end_date_for_event_render(d);
			me.prepare_colors(d);

			d.title = capkpi.utils.html2text(d.title);
	...
	prepare_colors: function(d) {
		...
		if(this.get_css_class) {
			...
			if (color_name.startsWith("#")) {
				color_name = capkpi.ui.color.validate_hex(color_name) ?
					color_name : 'blue';
			}

			d.backgroundColor = capkpi.ui.color.get(color_name, 'extra-light');
			d.textColor = capkpi.ui.color.get(color_name, 'dark');
		} else {
			color = d.color;
			if (!capkpi.ui.color.validate_hex(color) || !color) {
				color = capkpi.ui.color.get('blue', 'extra-light');
			}
			d.backgroundColor = color;
			d.textColor = capkpi.ui.color.get_contrast_color(color);
		}
		return d;
	},
	update_event: function(event, revertFunc) {
		var me = this;
		capkpi.model.remove_from_locals(me.doctype, event.name);
		return capkpi.call({
			method: me.update_event_method || "capkpi.desk.calendar.update_event",
			args: me.get_update_args(event),
			callback: function(r) {
				if(r.exc) {
					capkpi.show_alert(__("Unable to update event"));
					revertFunc();
				}
			},
			error: function() {
