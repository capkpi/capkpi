...
capkpi.breadcrumbs = {
	...
	add(module, doctype, type) {
		...
		this.all[capkpi.breadcrumbs.current_page()] = obj;
		this.update();
	},

	current_page() {
		return capkpi.get_route_str();
	},

	update() {
		var breadcrumbs = this.all[capkpi.breadcrumbs.current_page()];
	...
	set_workspace_breadcrumb(breadcrumbs) {
		...
		if (breadcrumbs.workspace) {
			if (!breadcrumbs.module_info.blocked && capkpi.visible_modules.includes(breadcrumbs.module_info.module)) {
				$(`<li><a href="/app/${capkpi.router.slug(breadcrumbs.workspace)}">${__(breadcrumbs.workspace)}</a></li>`)
	...
	set_workspace(breadcrumbs) {
		...
		if (breadcrumbs.module) {
			...
			breadcrumbs.module_info = capkpi.get_module(breadcrumbs.module);

			// set workspace
			if (breadcrumbs.module_info && capkpi.boot.module_page_map[breadcrumbs.module]) {
				breadcrumbs.workspace = capkpi.boot.module_page_map[breadcrumbs.module];
			}
		}
	},
	...
	set_list_breadcrumb(breadcrumbs) {
		...
		const doctype_meta = capkpi.get_doc('DocType', doctype);
		if ((doctype==="User" && !capkpi.user.has_role('System Manager'))
			|| (doctype_meta && doctype_meta.issingle)) {
			// no user listview for non-system managers and single doctypes
		} else {
			let route;
			const doctype_route = capkpi.router.slug(capkpi.router.doctype_layout || doctype);
			if (capkpi.boot.treeviews.indexOf(doctype) !== -1) {
				let view = capkpi.model.user_settings[doctype].last_view || 'Tree';
	...
	set_form_breadcrumb(breadcrumbs, view) {
		const doctype = breadcrumbs.doctype;
		const docname = capkpi.get_route().slice(2).join("/");
		let form_route = `/app/${capkpi.router.slug(doctype)}/${docname}`;
		...
		if (view === "form") {
			...
			last_crumb.click((event) => {
				...
				capkpi.utils.copy_to_clipboard(last_crumb.text());
			});
		}
	...
	set_dashboard_breadcrumb(breadcrumbs) {
		const doctype = breadcrumbs.doctype;
		const docname = capkpi.get_route()[1];
		let dashboard_route = `/app/${capkpi.router.slug(doctype)}/${docname}`;
		$(`<li><a href="${dashboard_route}">${__(docname)}</a></li>`)
			.appendTo(this.$breadcrumbs);
	},

	setup_modules() {
		if (!capkpi.visible_modules) {
			capkpi.visible_modules = $.map(capkpi.boot.allowed_workspaces, (m) => {
				return m.module;
			});
		}
	},
	...
	rename(doctype, old_name, new_name) {
		...
		delete capkpi.breadcrumbs.all[old_route_str];
		this.update();
	},
