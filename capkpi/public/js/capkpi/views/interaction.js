...
capkpi.provide("capkpi.interaction_settings");

capkpi.views.InteractionComposer = class InteractionComposer {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	get_event_categories() {
		let me = this;
		capkpi.model.with_doctype('Event', () => {
			let categories = capkpi.meta.get_docfield("Event", "event_category").options.split("\n");
	...
	create_interaction(btn, form_values, selected_attachments) {
		...
		if (!("owner" in interaction_values)){
			interaction_values["owner"] = capkpi.session.user;
		}
		if (!("assigned_by" in interaction_values) && interaction_values["doctype"] == "ToDo") {
			interaction_values["assigned_by"] = capkpi.session.user;
		}
		return capkpi.call({
			...
			callback: function(r) {
				if(!r.exc) {
					...
					capkpi.msgprint(__("There were errors while creating the document. Please try again."));
				}
			}
		});
	...
	add_attachments(doc, attachments) {
		capkpi.call({
			method:'capkpi.utils.file_manager.add_attachments',
			args: {
				doctype: doc.doctype,
				name: doc.name,
