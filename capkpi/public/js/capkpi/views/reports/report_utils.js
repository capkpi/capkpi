capkpi.provide('capkpi.report_utils');
...
capkpi.report_utils = {
	make_chart_options: function(columns, raw_data, { y_fields, x_field, chart_type, colors, height }) {
		...
		let datasets = y_fields.map(y_field => ({
			name: capkpi.model.unscrub(y_field),
		...
		function get_options(fields) {
			return fields.map((field) => {
				if (field.fieldname) {
					...
					field = capkpi.report_utils.prepare_field_from_column(field);
					return {label: field.label, value: field.fieldname};
				}
			});
		}
	...
	get_report_filters: function(report_name) {
		...
		if (capkpi.query_reports[report_name]) {
			let filters = capkpi.query_reports[report_name].filters;
			return Promise.resolve(filters);
		}

		return capkpi.xcall(
			'capkpi.desk.query_report.get_script',
			{
				report_name: report_name
			}
		).then(r => {
			capkpi.dom.eval(r.script || '');
			return capkpi.after_ajax(() => {
				return capkpi.query_reports[report_name] && capkpi.query_reports[report_name].filters;
			})
		});
	},
