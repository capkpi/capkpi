...
capkpi.provide('capkpi.widget.utils');
capkpi.provide('capkpi.views');
capkpi.provide('capkpi.query_reports');

capkpi.standard_pages['query-report'] = function() {
	var wrapper = capkpi.container.add_page('query-report');
	...
	capkpi.query_report = new capkpi.views.QueryReport({
		parent: wrapper,
	});

	$(wrapper).bind('show', function() {
		capkpi.query_report.show();
	});
};

capkpi.views.QueryReport = class QueryReport extends capkpi.views.BaseList {
	show() {
	...
	init() {
		...
		this.init_promise = capkpi.run_serially(tasks);
		return this.init_promise;
	}
	...
	setup_defaults() {
		...
		this.refresh = capkpi.utils.throttle(this.refresh, 300);
	...
	get_no_result_message() {
		return `<div class="msg-box no-border">
			<div>
				<img src="/assets/capkpi/images/ui-states/list-empty-state.svg" alt="Generic Empty State" class="null-state">
			</div>
	...
	setup_events() {
		capkpi.realtime.on("report_generated", (data) => {
			...
			if(data.report_name) {
				...
				if(data.name == this.prepared_report_doc_name) {
					...
					capkpi.show_alert({message: alert_message, indicator: 'orange'});
				}
			}
		});
	...
	load() {
		if (capkpi.get_route().length < 2) {
			this.toggle_nothing_to_show(true);
			return;
		}

		let route_options = {};
		route_options = Object.assign(route_options, capkpi.route_options);

		if (this.report_name !== capkpi.get_route()[1]) {
			// different report
			this.load_report(route_options);
		}
		else if (capkpi.has_route_options()) {
	...
	add_card_to_dashboard() {
		let field_options = capkpi.report_utils.get_field_options_from_report(this.columns, this.raw_data);
		const dashboard_field = capkpi.dashboard_utils.get_dashboard_link_field();
		const set_standard = capkpi.boot.developer_mode;
	...
	add_chart_to_dashboard() {
		if (this.chart_fields || this.chart_options) {
			...
			capkpi.msgprint(__('Please Set Chart'));
		}
	}
	...
	create_number_card(values, dashboard_name, card_name, set_standard) {
		...
		this.add_to_dashboard(
			'capkpi.desk.doctype.number_card.number_card.create_report_number_card',
			args,
			dashboard_name,
			card_name,
			'Number Card'
	...
	create_dashboard_chart(chart_args, dashboard_name, chart_name, set_standard) {
		let args = {
			...
			'chart_type': 'Report',
			'report_name': this.report_name,
			'type': chart_args.chart_type || capkpi.model.unscrub(chart_args.type),
		...
		this.add_to_dashboard(
			'capkpi.desk.doctype.dashboard_chart.dashboard_chart.create_report_chart',
			args,
			dashboard_name,
			chart_name,
			'Dashboard Chart'
	...
	add_to_dashboard(method, args, dashboard_name, name, doctype) {
		capkpi.xcall(
			...
			capkpi.msgprint(message, __("New {0} Created", [__(doctype)]));
		});
	}
	...
	get_report_doc() {
		return capkpi.model.with_doc('Report', this.report_name)
			.then(doc => {
				this.report_doc = doc;
			})
			.then(() => capkpi.model.with_doctype(this.report_doc.ref_doctype));
	}

	get_report_settings() {
		return new Promise((resolve, reject) => {
			if (capkpi.query_reports[this.report_name]) {
				this.report_settings = capkpi.query_reports[this.report_name];
				resolve();
			} else {
				capkpi.xcall('capkpi.desk.query_report.get_script', {
					report_name: this.report_name
				}).then(settings => {
					capkpi.dom.eval(settings.script || '');
					capkpi.after_ajax(() => {
						...
						capkpi.query_reports[this.report_name] = this.report_settings;
	...
	get_local_report_settings() {
		let report_script_name = this.report_doc.report_type === 'Custom Report'
			...
			: this.report_name;
		return capkpi.query_reports[report_script_name] || {};
	}
	...
	setup_progress_bar() {
		...
		this.interval = setInterval(function()  {
			seconds_elapsed += 1;
			capkpi.show_progress(__('Preparing Report'), seconds_elapsed, execution_time);
		}, 1000);
	}
		...
		if (doc) {
			if (typeof expression === 'boolean') {
				...
				try {
					...
					capkpi.throw(__('Invalid "depends_on" expression set in filter {0}', [filter_label]));
				}
			} else {
	...
	set_route_filters(route_options) {
		if (!route_options) route_options = capkpi.route_options;
		...
		if (route_options) {
			...
			return capkpi.run_serially(promises);
		}
	}
	...
	refresh() {
		...
		return new Promise(resolve => {
			this.last_ajax = capkpi.call({
				method: 'capkpi.desk.query_report.run',
				type: 'GET',
				args: {
					report_name: this.report_name,
					filters: filters,
			...
			if (data.result && data.result.length) {
				...
				if (this.chart_options) {
					...
					if (this.chart_fields) {
						this.chart_options =
							capkpi.report_utils.make_chart_options(
								this.columns,
								this.raw_data,
								this.chart_fields
							);
	...
	render_summary(data) {
		data.forEach((summary) => {
			capkpi.utils.build_summary_item(summary).appendTo(this.$summary);
		})
	...
	get_query_params() {
		const query_string = capkpi.utils.get_query_string(capkpi.get_route_str());
		const query_params = capkpi.utils.get_query_params(query_string);
		return query_params;
	}
	...
	add_prepared_report_buttons(doc) {
			...
			this.page.add_inner_button(__("Download Report"), function (){
				window.open(
					capkpi.urllib.get_full_url(
						"/api/method/capkpi.core.doctype.prepared_report.prepared_report.download_attachment?"
						+"dn="+encodeURIComponent(doc.name)));
			});

			const part1 = __('This report was generated {0}.', [capkpi.datetime.comment_when(doc.report_end_time)]);
		...
		this.primary_action_map = {
			...
			"Edit": {
				label: __("Edit"),
				click: () => {
					capkpi.set_route(capkpi.get_route());
				}
			},
			"Rebuild": {
				label:	__("Rebuild"),
	...
	show_warning_or_generate_report() {
		capkpi.xcall(
			'capkpi.core.doctype.prepared_report.prepared_report.get_reports_in_queued_state',
			{
			...
			if (reports.length) {
				...
				this.prepared_report_dialog = capkpi.warn(
				...
				capkpi.utils.bind_actions_with_object(this.prepared_report_dialog.wrapper, this);
			} else {
				this.generate_background_report();
			}
		});
	...
	delete_old_queued_reports() {
		...
		capkpi.xcall(
			'capkpi.core.doctype.prepared_report.prepared_report.delete_prepared_reports',
			{
	...
	generate_background_report() {
		...
		if (!missing_mandatory.length) {
			let filters = this.get_filter_values(true);
			return new Promise(resolve => capkpi.call({
				method: 'capkpi.desk.query_report.background_enqueue_run',
				type: 'GET',
				args: {
					report_name: this.report_name,
					filters: filters
				...
				capkpi.show_alert({message: alert_message, indicator: 'orange'}, 10);
				this.toggle_nothing_to_show(true);
			});
		}
	}
	...
	render_datatable() {
		...
		if (this.datatable && this.datatable.options
			...
			let datatable_options = {
				...
				direction: capkpi.utils.is_rtl() ? 'rtl' : 'ltr',
				hooks: {
					columnTotal: capkpi.utils.report_column_total
				}
			};
	...
	get_chart_options(data) {
		...
		if (options.fieldtype) {
			options.tooltipOptions = {
				formatTooltipY: d => capkpi.format(d, {
	...
	render_chart(options) {
		this.$chart.empty();
		this.$chart.show();
		this.chart = new capkpi.Chart(this.$chart[0], options);
	}
	...
	open_create_chart_dialog() {
		...
		function preview_chart() {
			...
			if (values.x_field && values.y_fields.length) {
				let options = capkpi.report_utils.make_chart_options(me.columns, me.raw_data, values);
				me.chart_fields = values;
				wrapper.empty();
				new capkpi.Chart(wrapper[0], options);
	...
	prepare_columns(columns) {
		return columns.map(column => {
			column = capkpi.report_utils.prepare_field_from_column(column);
			...
			const format_cell = (value, row, column, data) => {
				...
				return capkpi.format(value, column,
			...
			if (column.fieldtype === 'Date') {
				compareFn = (cell, keyword) => {
					...
					const keywordValue = capkpi.datetime.user_to_obj(keyword);
					const cellValue = capkpi.datetime.str_to_obj(cell.content);
	...
	set_breadcrumbs() {
		...
		const ref_doctype = capkpi.get_meta(this.report_doc.ref_doctype);
		capkpi.breadcrumbs.add(ref_doctype.module);
	}

	make_access_log (method, file_format) {
		capkpi.call("capkpi.core.doctype.access_log.access_log.make_access_log",
			{
	...
	pdf_report(print_settings) {
		const base_url = capkpi.urllib.get_base_url();
		const print_css = capkpi.boot.print_css;
		const landscape = print_settings.orientation == 'Landscape';
		...
		const content = capkpi.render_template(template, {
			title: __(this.report_name),
			subtitle: filters_html,
		...
		const html = capkpi.render_template('print_template', {
			title: __(this.report_name),
			content: content,
			base_url: base_url,
			print_css: print_css,
			...
			landscape: landscape,
			columns: columns,
			lang: capkpi.boot.lang,
			layout_direction: capkpi.utils.is_rtl() ? "rtl" : "ltr"
		});

		capkpi.render_pdf(html, print_settings);
	}
	...
	get_filters_html_for_print() {
		...
		return Object.keys(applied_filters)
			.map(fieldname => {
				const label = capkpi.query_report.get_filter(fieldname).df.label;
	...
	export_report() {
		...
		this.export_dialog = capkpi.prompt(export_dialog_fields, ({ file_format, include_indentation }) => {
			...
			if (file_format === 'CSV') {
				...
				capkpi.tools.downloadify(out, null, this.report_name);
			} else {
				let filters = this.get_filter_values(true);
				if (capkpi.urllib.get_dict("prepared_report_name")) {
					filters = Object.assign(capkpi.urllib.get_dict("prepared_report_name"), filters);
				}
				...
				const args = {
					cmd: 'capkpi.desk.query_report.export_query',
					report_name: this.report_name,
			...
			return row
				...
				.map((cell, i) => {
					if (cell.column.fieldtype === "Duration") {
						cell.content = capkpi.utils.get_formatted_duration(cell.content);
					}
	...
	get_menu_items() {
		let items = [
			...
			{
				label: __('Edit'),
				action: () => capkpi.set_route('Form', 'Report', this.report_name),
				condition: () => capkpi.user.is_report_manager(),
				standard: true
			},
			{
				label: __('Print'),
				action: () => {
					let dialog = capkpi.ui.get_print_settings(
						false,
				...
				condition: () => capkpi.model.can_print(this.report_doc.ref_doctype),
				standard: true
			},
			{
				label: __('PDF'),
			...
			},
			{
				...
				condition: () => capkpi.model.can_export(this.report_doc.ref_doctype),
				standard: true
			},
			{
				label: __('Setup Auto Email'),
				action: () => capkpi.set_route('List', 'Auto Email Report', {'report' : this.report_name}),
				standard: true
			},
			{
				label: __('Add Column'),
				action: () => {
					let d = new capkpi.ui.Dialog({
						...
						fields: [
							{
								...
								change: () => {
									...
									capkpi.model.with_doctype(doctype, () => {
										let options = capkpi.meta.get_docfields(doctype)
											.filter(capkpi.model.is_value_type)
						...
						primary_action: (values) => {
							...
							let df = capkpi.meta.get_docfield(values.doctype, values.field);
							...
							capkpi.call({
								method: 'capkpi.desk.query_report.get_data_for_custom_field',
								args: {
									doctype: values.doctype,
									fieldname: fieldname,
									field: values.field
			...
			},
			{
				...
				action: () => capkpi.set_route('List', 'User Permission', {
					doctype: 'Report',
					name: this.report_name
				}),
				condition: () => capkpi.model.can_set_user_permissions('Report'),
				standard: true
			}
		];

		if (capkpi.user.is_report_manager()) {
			items.push({
				label: __('Save'),
				action: () => {
					let d = new capkpi.ui.Dialog({
						...
						primary_action: (values) => {
							capkpi.call({
								method: "capkpi.desk.query_report.save_report",
								args: {
								...
								callback: function(r) {
									this.show_save = false;
									d.hide();
									capkpi.set_route('query-report', r.message);
								}
							});
						}
					});
	...
	get_fieldname_for_column(fieldname, doctype) {
		...
		if (existing_fieldnames.includes(fieldname)) {
			fieldname = capkpi.model.scrub(fieldname + " " + doctype);
		}
...
Object.defineProperty(capkpi, 'query_report_filters_by_name', {
	get() {
		console.warn('[Query Report] capkpi.query_report_filters_by_name is deprecated. Please use the new api: capkpi.query_report.get_filter_value(fieldname) and capkpi.query_report.set_filter_value(fieldname, value)');
		return null;
	}
});
