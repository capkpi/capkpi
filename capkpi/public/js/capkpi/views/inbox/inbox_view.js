/**
 * capkpi.views.InboxView
 */

capkpi.provide("capkpi.views");

capkpi.views.InboxView = class InboxView extends capkpi.views.ListView {
	static load_last_view() {
		const route = capkpi.get_route();
		if (!route[3] && capkpi.boot.email_accounts.length) {
			let email_account;
			if (capkpi.boot.email_accounts[0].email_id == "All Accounts") {
				email_account = "All Accounts";
			} else {
				email_account = capkpi.boot.email_accounts[0].email_account;
			}
			capkpi.set_route("List", "Communication", "Inbox", email_account);
			return true;
		} else if (!route[3] || (route[3] !== "All Accounts" && !is_valid(route[3]))) {
			capkpi.throw(__('No email account associated with the User. Please add an account under User > Email Inbox.'));
		}
		return false;

		function is_valid(email_account) {
			return capkpi.boot.email_accounts.find(d => d.email_account === email_account);
		}
	}
	...
	setup_defaults() {
		...
		this.email_account = capkpi.get_route()[3];
	...
	get_seen_class(doc) {
		const seen =
			Boolean(doc.seen) || JSON.parse(doc._seen || '[]').includes(capkpi.session.user)
				? ''
				: 'bold';
		return seen;
	}
	...
	get_meta_html(email) {
		...
		if (email.reference_doctype && email.reference_doctype !== this.doctype) {
			link = `<a class="text-muted grey"
				href="${capkpi.utils.get_form_link(email.reference_doctype, email.reference_name)}"
	...
	get_inbox_filters() {
		...
		if (email_account === "Sent") {
			...
			filters = default_filters.concat([
				...
				["Communication", "email_account", "in", capkpi.boot.all_accounts, true]
			]);
		} else {
			var op = "=";
			if (email_account == "All Accounts") {
				op = "in";
				email_account = capkpi.boot.all_accounts;
			}
	...
	get_no_result_message() {
		...
		} else if (!email_account && !capkpi.boot.email_accounts.length) {
		...
		const html = capkpi.model.can_create(args.doctype) ?
			`<p>${args.msg}</p>
			<p>
	...
	make_new_doc() {
		if (!this.email_account && !capkpi.boot.email_accounts.length) {
			capkpi.route_options = {
				'email_id': capkpi.session.user_email
			};
			capkpi.new_doc('Email Account');
		} else {
