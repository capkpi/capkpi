...
capkpi.last_edited_communication = {};
const separator_element = '<div>---</div>';

capkpi.views.CommunicationComposer = class {
	constructor(opts) {
		$.extend(this, opts);
		if (!this.doc) {
	...
	get_fields() {
		const fields = [
			...
			},
			{
				...
				fieldname: "content",
				onchange: capkpi.utils.debounce(
			...
			{ fieldtype: "Section Break" },
			{
				...
				default: capkpi.boot.user.send_me_a_copy
			},
			{
		...
		const email_accounts = capkpi.boot.email_accounts.filter(account => {
			return (
				!in_list(
	...
	toggle_more_options(show_options) {
		...
		const label = capkpi.utils.icon(show_options ? 'up-line': 'down');
	...
	setup_multiselect_queries() {
		['recipients', 'cc', 'bcc'].forEach(field => {
			this.dialog.fields_dict[field].get_data = () => {
				...
				capkpi.call({
					method: "capkpi.email.get_contact_list",
					args: {txt},
					callback: (r) => {
	...
	setup_email_template() {
		...
		this.dialog.fields_dict["email_template"].df.onchange = () => {
			...
			capkpi.call({
				method: 'capkpi.email.doctype.email_template.email_template.get_email_template',
				args: {
	...
	get_last_edited_communication(clear) {
		if (!capkpi.last_edited_communication[this.doctype]) {
			capkpi.last_edited_communication[this.doctype] = {};
		}

		if (clear || !capkpi.last_edited_communication[this.doctype][this.key]) {
			capkpi.last_edited_communication[this.doctype][this.key] = {};
		}

		return capkpi.last_edited_communication[this.doctype][this.key];
	}
	...
	async set_values() {
		...
		const subject = capkpi.utils.html2text(this.subject) || '';
	...
	setup_print_language() {
		...
		$(fields.language_sel.input)
			.empty()
			.add_options(capkpi.get_languages());
	...
	setup_print() {
		...
		if (this.frm) {
			const print_formats = capkpi.meta.get_print_formats(this.frm.meta.name);
	...
	setup_attach() {
		...
		$(`
			...
			<p class='add-more-attachments'>
				<button class='btn btn-xs btn-default'>
					${capkpi.utils.icon('small-add', 'xs')}&nbsp;
					${__("Add Attachment")}
				</button>
			</p>
		...
		attach
			...
			.on('click', () => new capkpi.ui.FileUploader(args));
			...
			if (files.length) {
				$.each(files, (i, f) => {
					...
					if (!attachment_rows.find(`[data-file-name="${f.name}"]`).length) {
						f.file_url = capkpi.urllib.get_full_url(f.file_url);
	...
	setup_email() {
		...
		$(fields.send_me_a_copy.input).on('click', () => {
			...
			capkpi.db.set_value('User', capkpi.session.user, 'send_me_a_copy', val);
			capkpi.boot.user.send_me_a_copy = val;
		});
	...
	send_email(btn, form_values, selected_attachments, print_html, print_format) {
		...
		if (!form_values.recipients) {
			capkpi.msgprint(__("Enter Email Recipient(s)"));
			return;
		}
		...
		if (this.frm && !capkpi.model.can_email(this.doc.doctype, this.frm)) {
			capkpi.msgprint(__("You are not allowed to send emails related to this document"));
			return;
		}
			...
			callback(r) {
				if (!r.exc) {
					...
					if (r.message["emails_not_sent_to"]) {
						capkpi.msgprint(__("Email not sent to {0} (unsubscribed / disabled)",
	...
	is_print_letterhead_checked() {
		if (this.frm && $(this.frm.wrapper).find('.form-print-wrapper').is(':visible')){
			...
			return (capkpi.model.get_doc(":Print Settings", "Print Settings") ||
	...
	async get_signature(sender_email) {
		let signature = capkpi.boot.user.email_signature;
		...
		if (!signature) {
			...
			const email_accounts = await capkpi.db.get_list("Email Account", {
				filters: filters,
		...
		if (!capkpi.utils.is_html(signature)) {
	...
	get_earlier_reply() {
		...
		const communication_date = capkpi.datetime.global_date_format(
