...
capkpi.re_route = {"#login": ""};
capkpi.route_titles = {};
capkpi.route_flags = {};
capkpi.route_history = [];
capkpi.view_factory = {};
capkpi.view_factories = [];
capkpi.route_options = null;
capkpi.route_hooks = {};

$(window).on('hashchange', function(e) {
	// v1 style routing, route is in hash
	if (window.location.hash && !capkpi.router.is_app_route(e.currentTarget.pathname)) {
		let sub_path = capkpi.router.get_sub_path(window.location.hash);
		capkpi.router.push_state(sub_path);
		return false;
	}
});
...
$('body').on('click', 'a', function(e) {
	...
	if (capkpi.router.is_app_route(e.currentTarget.pathname)) {
...
capkpi.router = {
	...
	setup() {
		...
		for (let doctype of capkpi.boot.user.can_read) {
			this.routes[this.slug(doctype)] = {doctype: doctype};
		}
		if (capkpi.boot.doctype_layouts) {
			for (let doctype_layout of capkpi.boot.doctype_layouts) {
	...
	convert_to_standard_route(route) {
		...
		if (capkpi.workspaces[route[0]]) {
			// workspace
			route = ['Workspaces', capkpi.workspaces[route[0]].name];
	...
	set_doctype_route(route) {
		...
		} else if (capkpi.model.is_single(doctype_route.doctype)) {
	...
	get_standard_route_for_list(route, doctype_route) {
		...
		if (route[2].toLowerCase()==='tree') {
			...
			standard_route = ['List', doctype_route.doctype, capkpi.utils.to_title_case(route[2])];
	...
	set_history() {
		capkpi.route_history.push(this.current_route);
	...
	render_page() {
		...
		const factory = capkpi.utils.to_title_case(route[0]);

		if (route[1] && capkpi.views[factory + "Factory"]) {
			route[0] = factory;
			// has a view generator, generate!
			if (!capkpi.view_factory[factory]) {
				capkpi.view_factory[factory] = new capkpi.views[factory + "Factory"]();
			}

			capkpi.view_factory[factory].show();
		} else {
			// show page
			const route_name = capkpi.utils.xss_sanitise(route[0]);
			if (capkpi.views.pageview) {
				capkpi.views.pageview.show(route_name);
			}
		}
	},

	re_route(sub_path) {
		if (capkpi.re_route[sub_path] !== undefined) {
			...
			const re_route_val = this.get_sub_path(capkpi.re_route[sub_path]);
			if (re_route_val === this.current_sub_path) {
				window.history.back();
			} else {
				capkpi.set_route(re_route_val);
			}
	...
	set_title(sub_path) {
		if (capkpi.route_titles[sub_path]) {
			capkpi.utils.set_title(capkpi.route_titles[sub_path]);
		}
	},

	set_route() {
		// set the route (push state) with given arguments
		// example 1: capkpi.set_route('a', 'b', 'c');
		// example 2: capkpi.set_route(['a', 'b', 'c']);
		// example 3: capkpi.set_route('a/b/c');
		...
		return new Promise(resolve => {
			...
			setTimeout(() => {
				capkpi.after_ajax && capkpi.after_ajax(() => {
					resolve();
				});
			}, 100);
		}).finally(() => capkpi.route_flags = {});
	},

	get_route_from_arguments(route) {
		if (route.length===1 && $.isArray(route[0])) {
			// called as capkpi.set_route(['a', 'b', 'c']);
			route = route[0];
		}
	...
	push_state(url) {
		...
		if (window.location.pathname !== url) {
			...
			const method = capkpi.route_flags.replace_route ? "replaceState" : "pushState";
	...
	set_route_options_from_url(route) {
		// set query parameters as capkpi.route_options
		...
		if (last_part.indexOf("?") < last_part.indexOf("=")) {
			...
			let query_params = capkpi.utils.get_query_params(parts[1]);
			capkpi.route_options = $.extend(capkpi.route_options || {}, query_params);
		}
	},
...
capkpi.get_route = () => capkpi.router.current_route;
capkpi.get_route_str = () => capkpi.router.current_route.join('/');
capkpi.set_route = function() {
	return capkpi.router.set_route.apply(capkpi.router, arguments);
};

capkpi.get_prev_route = function() {
	if (capkpi.route_history && capkpi.route_history.length > 1) {
		return capkpi.route_history[capkpi.route_history.length - 2];
	} else {
		return [];
	}
};

capkpi.set_re_route = function() {
	var tmp = capkpi.router.get_sub_path();
	capkpi.set_route.apply(null, arguments);
	capkpi.re_route[tmp] = capkpi.router.get_sub_path();
};

capkpi.has_route_options = function() {
	return Boolean(Object.keys(capkpi.route_options || {}).length);
};

capkpi.utils.make_event_emitter(capkpi.router);
