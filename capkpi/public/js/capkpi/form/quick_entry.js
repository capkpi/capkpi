...
capkpi.quick_edit = function(doctype, name) {
	capkpi.db.get_doc(doctype, name).then(doc => {
		capkpi.ui.form.make_quick_entry(doctype, null, null, doc);
	});
};

capkpi.ui.form.make_quick_entry = (doctype, after_insert, init_callback, doc, force) => {
	var trimmed_doctype = doctype.replace(/ /g, '');
	var controller_name = "QuickEntryForm";

	if(capkpi.ui.form[trimmed_doctype + "QuickEntryForm"]){
		controller_name = trimmed_doctype + "QuickEntryForm";
	}

	capkpi.quick_entry = new capkpi.ui.form[controller_name](doctype, after_insert, init_callback, doc, force);
	return capkpi.quick_entry.setup();
};

capkpi.ui.form.QuickEntryForm = Class.extend({
	...
	check_quick_entry_doc: function() {
		if (!this.doc) {
			this.doc = capkpi.model.get_new_doc(this.doctype, null, null, true);
		}
	},
	...
	render_dialog: function() {
		...
		this.dialog.onhide = () => capkpi.quick_entry = null;
		this.dialog.show();
	...
	insert: function() {
		...
		return new Promise(resolve => {
			me.update_doc();
			capkpi.call({
				method: "capkpi.client.save",
				args: {
					doc: me.dialog.doc
				},
				callback: function(r) {

					if (capkpi.model.is_submittable(me.doctype)) {
						capkpi.run_serially([
							() => me.dialog.working = true,
							() => {
	...
	open_doc: function(set_hooks) {
		...
		if (set_hooks && this.after_insert) {
			capkpi.route_options = capkpi.route_options || {};
			capkpi.route_options.after_save = (frm) => {
