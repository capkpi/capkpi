...
capkpi.provide('capkpi.success_action');

capkpi.ui.form.SuccessAction = class SuccessAction {
	constructor(form) {
		this.form = form;
		this.load_setting();
	}
	...
	load_setting() {
		this.setting = capkpi.boot.success_action
	...
	show_alert() {
		capkpi.db.get_list(this.form.doctype, {limit: 2})
			.then(result => {
	...
	get default_actions() {
		return {
			'new': {
				label: __('New'),
				action: (frm) => capkpi.new_doc(frm.doctype)
			},
			'print': {
				label: __('Print'),
			...
			'list': {
				label: __('View All'),
				action: (frm) => {
					capkpi.set_route('List', frm.doctype);
				}
			}
		};
	}
