capkpi.ui.get_print_settings = function(
	pdf,
	callback,
	letter_head,
	pick_columns
	...
	var default_letter_head =
		locals[":Company"] && capkpi.defaults.get_default("company")
			? locals[":Company"][capkpi.defaults.get_default("company")]["default_letter_head"]
			: "";
	...
	var columns = [
		...
		},
		{
			...
			options: Object.keys(capkpi.boot.letter_heads),
	...
	return capkpi.prompt(
		...
		function(data) {
			...
			if (data.letter_head) {
				data.letter_head =
					capkpi.boot.letter_heads[print_settings.letter_head];
			}
			callback(data);
		},
		__("Print Settings")
...
capkpi.ui.form.qz_connect = function() {
	return new Promise(function(resolve, reject) {
		capkpi.ui.form.qz_init().then(() => {
			if (qz.websocket.isActive()) {
				// if already active, resolve immediately
				// capkpi.show_alert({message: __('QZ Tray Connection Active!'), indicator: 'green'});
				resolve();
			} else {
				// try to connect once before firing the mimetype launcher
				capkpi.show_alert({
							...
							qz.websocket
								...
								.then(
									...
									() => {
										capkpi.throw(
											__(
												'Error connecting to QZ Tray Application...<br><br> You need to have QZ Tray application installed and running, to use the Raw Print feature.<br><br><a target="_blank" href="https://qz.io/download/">Click here to Download and install QZ Tray</a>.<br> <a target="_blank" href="https://capkpi.com/docs/user/manual/en/setting-up/print/raw-printing">Click here to learn more about Raw Printing</a>.'
											)
										);
										reject();
									}
...
capkpi.ui.form.qz_init = function() {
	...
	return new Promise(resolve => {
		if (typeof qz === "object" && typeof qz.version === "string") {
			...
			let qz_required_assets = [
				"/assets/capkpi/node_modules/js-sha256/build/sha256.min.js",
				"/assets/capkpi/node_modules/qz-tray/qz-tray.js"
			];
			capkpi.require(qz_required_assets, () => {
			...
			// note 'capkpi.require' does not have callback on fail. Hence, any failure cannot be communicated to the user.
		}
	});
};

capkpi.ui.form.qz_get_printer_list = function() {
	// returns the list of printers that are available to the QZ Tray
	return capkpi.ui.form
		.qz_connect()
		.then(function() {
...
capkpi.ui.form.qz_success = function() {
...
capkpi.ui.form.qz_fail = function(e) {
	// notify qz errors
	capkpi.show_alert(
		{
