...
capkpi.provide('capkpi.model.docinfo');
...
capkpi.ui.form.Controller = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
	}
});

capkpi.ui.form.Form = class CapKPIForm {
	constructor(doctype, parent, in_form, doctype_layout_name) {
		...
		this.sections = [];
		this.grids = [];
		this.cscript = new capkpi.ui.form.Controller({ frm: this });
		this.events = {};
		this.fetch_dict = {};
		this.parent = parent;
		this.doctype_layout = capkpi.get_doc('DocType Layout', doctype_layout_name);
	...
	setup_meta() {
		this.meta = capkpi.get_doc('DocType', this.doctype);

		if(this.meta.istable) {
			this.meta.in_dialog = 1;
		}

		this.perm = capkpi.perm.get_perm(this.doctype); // for create
	...
	setup() {
		this.fields = [];
		this.fields_dict = {};
		this.state_fieldname = capkpi.workflow.get_state_fieldname(this.doctype);
		...
		this.toolbar = new capkpi.ui.form.Toolbar({
			frm: this,
			page: this.page
		});
		...
		this.script_manager = new capkpi.ui.form.ScriptManager({
			frm: this
		});
		this.script_manager.setup();
		this.watch_model_updates();

		if (!this.meta.hide_toolbar && capkpi.boot.desk_settings.timeline) {
			this.footer = new capkpi.ui.form.Footer({
				frm: this,
	...
	setup_std_layout() {
		...
		// layout
		this.layout = new capkpi.ui.form.Layout({
			parent: this.body,
	...
	watch_model_updates() {
		...
		// on main doc
		capkpi.model.on(me.doctype, "*", function(fieldname, value, doc) {
			// set input
		...
		// on table fields
		var table_fields = capkpi.get_children("DocType", me.doctype, "fields", {
			fieldtype: ["in", capkpi.model.table_fields]
		});

		// using $.each to preserve df via closure
		$.each(table_fields, function(i, df) {
			capkpi.model.on(df.options, "*", function(fieldname, value, doc) {
	...
	setup_file_drop() {
		...
		this.$wrapper.on('dragenter dragover', false)
			.on('drop', function(e) {
				...
				if(me.doc.__islocal) {
					capkpi.msgprint(__("Please save before attaching."));
					throw "attach error";
				}
	...
	refresh(docname) {
		...
		if(this.docname) { // document to show
			...
			// set the doc
			this.doc = capkpi.get_doc(this.doctype, this.docname);

			// check permissions
			if (!this.has_read_permission()) {
				capkpi.show_not_permitted(__(this.doctype) + " " + __(this.docname));
				return;
			}
			...
			// read only (workflow)
			this.read_only = capkpi.workflow.is_read_only(this.doctype, this.docname);
	...
	setup_doctype_actions() {
		if (this.meta.actions) {
			for (let action of this.meta.actions) {
				capkpi.ui.form.on(this.doctype, 'refresh', () => {
					if (!this.is_new()) {
						if (!action.hidden) {
	...
	execute_action(action) {
		...
		if (action.action_type==='Server Action') {
			...
			capkpi.set_route(action.action);
		}
	}
	...
	initialize_new_doc() {
		...
		this.script_manager.trigger("before_load", this.doctype, this.docname)
			.then(() => {
				...
				capkpi.after_ajax(function() {
					me.trigger_link_fields();
				});

				capkpi.breadcrumbs.add(me.meta.module, me.doctype);
			});
	...
	render_form(switched) {
		if(!this.meta.istable) {
			...
			if (capkpi.boot.desk_settings.form_sidebar) {
				this.sidebar = new capkpi.ui.form.Sidebar({
					frm: this,
					page: this.page
				});
				this.sidebar.make();
	...
	run_after_load_hook() {
		if (capkpi.route_hooks.after_load) {
			let route_callback = capkpi.route_hooks.after_load;
			delete capkpi.route_hooks.after_load;
	...
	refresh_header(switched) {
		...
		if(!this.meta.in_dialog || this.in_form) {
			capkpi.utils.set_title(this.meta.issingle ? this.doctype : this.docname);
		}
		...
		var after_save = function(r) {
			if(!r.exc) {
				if (["Save", "Update", "Amend"].indexOf(save_action)!==-1) {
					capkpi.utils.play_sound("click");
				}

				me.script_manager.trigger("after_save");

				if (capkpi.route_hooks.after_save) {
					let route_callback = capkpi.route_hooks.after_save;
					delete capkpi.route_hooks.after_save;
		...
		if(save_action != "Update") {
			...
			capkpi.run_serially([
				...
				() => {
					...
					capkpi.ui.form.save(me, save_action, after_save, btn);
				}
			]).catch(fail);
		} else {
			capkpi.ui.form.save(me, save_action, after_save, btn);
	...
	savesubmit(btn, callback, on_error) {
		...
		return new Promise(resolve => {
			...
			capkpi.confirm(__("Permanently Submit {0}?", [this.docname]), function() {
				capkpi.validated = true;
					...
					me.save('Submit', function(r) {
						if(r.exc) {
							...
							me.script_manager.trigger("on_submit")
								.then(() => resolve(me))
								.then(() => {
									if (capkpi.route_hooks.after_submit) {
										let route_callback = capkpi.route_hooks.after_submit;
										delete capkpi.route_hooks.after_submit;
										route_callback(me);
									}
								});
						}
	...
	savecancel(btn, callback, on_error) {
		...
		capkpi.call({
			method: "capkpi.desk.form.linked_with.get_submitted_linked_docs",
			args: {
	...
	_cancel_all(r, btn, callback, on_error) {
		...
		for (let doctype of doctypes) {
			if (!me.ignore_doctypes_on_cancel_all.includes(doctype)) {
				let docnames = links
					...
					.map((link) => capkpi.utils.get_form_link(link.doctype, link.name, true))
					.join(", ");
		...
		let can_cancel = links.every((link) => capkpi.model.can_cancel(link.doctype));
		if (can_cancel) {
		...
		let d = new capkpi.ui.Dialog({
			...
			fields: [{
				fieldtype: "HTML",
				options: `<p class="capkpi-confirm-message">${confirm_message}</p>`
			}]
		}, () => me.handle_save_fail(btn, on_error));
		...
		if (can_cancel) {
			d.set_primary_action("Cancel All", () => {
				d.hide();
				capkpi.call({
					method: "capkpi.desk.form.linked_with.cancel_all_linked_docs",
					args: {
						docs: links,
	...
	_cancel(btn, callback, on_error, skip_confirm) {
		...
		const cancel_doc = () => {
			...
			me.script_manager.trigger("before_cancel").then(() => {
				...
				capkpi.ui.form.save(me, "cancel", after_cancel, btn);
			});
		}
		...
		if (skip_confirm) {
			...
			capkpi.confirm(__("Permanently Cancel {0}?", [this.docname]), cancel_doc, me.handle_save_fail(btn, on_error));
		}
	};

	savetrash() {
		this.validate_form_action("Delete");
		capkpi.model.delete_doc(this.doctype, this.docname, function() {
			window.history.back();
		});
	}
	...
	amend_doc() {
		if (!this.fields_dict['amended_from']) {
			capkpi.msgprint(__('"amended_from" field must be present to do an amendment.'));
			return;
		}

		capkpi.xcall('capkpi.client.is_document_amended', {
			'doctype': this.doc.doctype,
			'docname': this.doc.name
		}).then(is_amended => {
			if (is_amended) {
				capkpi.throw(__('This document is already amended, you cannot ammend it again'));
			}
			...
			var fn = function(newdoc) {
				...
				if (me.fields_dict && me.fields_dict['amendment_date'])
					newdoc.amendment_date = capkpi.datetime.obj_to_str(new Date());
			};
			this.copy_doc(fn, 1);
			capkpi.utils.play_sound("click");
		});
	...
	validate_form_action(action, resolve) {
		...
		var perms = capkpi.perm.get_perm(this.doc.doctype)[0];
		...
		if ((capkpi.workflow.is_read_only(this.doctype, this.docname) && (perms["write"] ||
			perms["create"] || perms["submit"] || perms["cancel"])) || !capkpi.workflow.is_read_only(this.doctype, this.docname)) {
	...
	show_submit_message() {
		if(this.meta.is_submittable
			...
			&& !this.is_dirty()
			&& !this.is_new()
			&& !capkpi.model.has_workflow(this.doctype) // show only if no workflow
	...
	has_read_permission() {
		...
		this.perm = capkpi.perm.get_perm(dt, this.doc);
	...
	check_doctype_conflict(docname) {
		if(this.doctype=='DocType' && docname=='DocType') {
			capkpi.msgprint(__('Allowing DocType, DocType. Be careful!'));
		} else if(this.doctype=='DocType') {
			if (capkpi.views.formview[docname] || capkpi.pages['List/'+docname]) {
				window.location.reload();
				//	capkpi.msgprint(__("Cannot open {0} when its instance is open", ['DocType']))
				// throw 'doctype open conflict'
			}
		} else {
			if (capkpi.views.formview.DocType && capkpi.views.formview.DocType.frm.opendocs[this.doctype]) {
				window.location.reload();
				//	capkpi.msgprint(__("Cannot open instance when its {0} is open", ['DocType']))
				// throw 'doctype open conflict'
			}
		}
	}
	...
	rename_notify(dt, old, name) {
		...
		// cleanup
		if(this && this.opendocs[old] && capkpi.meta.docfield_copy[dt]) {
			// delete docfield copy
			capkpi.meta.docfield_copy[dt][name] = capkpi.meta.docfield_copy[dt][old];
			delete capkpi.meta.docfield_copy[dt][old];
		}
		...
		capkpi.re_route[capkpi.router.get_sub_path()] = `${encodeURIComponent(capkpi.router.slug(this.doctype))}/${encodeURIComponent(name)}`;
		!capkpi._from_link && capkpi.set_route('Form', this.doctype, name);
	}
	...
	print_doc() {
		...
		capkpi.set_route('print', this.doctype, this.doc.name);
	}

	navigate_records(prev) {
		let filters, sort_field, sort_order;
		let list_view = capkpi.get_list_view(this.doctype);
		if (list_view) {
			filters = list_view.get_filters_for_args();
			sort_field = list_view.sort_by;
			sort_order = list_view.sort_order;
		} else {
			let list_settings = capkpi.get_user_settings(this.doctype)['List'];
			if (list_settings) {
		...
		capkpi.call('capkpi.desk.form.utils.get_next', args).then(r => {
			if (r.message) {
				capkpi.set_route('Form', this.doctype, r.message);
	...
	rename_doc() {
		capkpi.model.rename_doc(this.doctype, this.docname, () => this.refresh_header());
	}
	...
	email_doc(message) {
		new capkpi.views.CommunicationComposer({
			doc: this.doc,
			frm: this,
	...
	copy_doc(onload, from_amend) {
		...
		var newdoc = capkpi.model.copy_doc(this.doc, from_amend);
	...
	reload_doc() {
		...
		if(!this.doc.__islocal) {
			capkpi.model.remove_from_locals(this.doctype, this.docname);
			return capkpi.model.with_doc(this.doctype, this.docname, () => {
				this.refresh();
			});
		}
	}
	...
	has_perm(ptype) {
		return capkpi.perm.has_perm(this.doctype, 0, ptype, this.doc);
	}
	...
	get_docinfo() {
		return capkpi.model.docinfo[this.doctype][this.docname];
	}
	...
	set_footnote(txt) {
		this.footnote_area = capkpi.utils.set_footnote(this.footnote_area, this.body, txt);
	}
	...
	scroll_to_element() {
		if (capkpi.route_options && capkpi.route_options.scroll_to) {
			var scroll_to = capkpi.route_options.scroll_to;
			delete capkpi.route_options.scroll_to;
			...
			if (selector.length) {
				capkpi.utils.scroll_to(selector);
			}
		}
	}
	...
	show_success_action() {
		...
		const success_action = new capkpi.ui.form.SuccessAction(this);
		success_action.show();
	}
	...
	set_currency_labels(fields_list, currency, parentfield) {
		...
		$.each(fields_list, function(i, fname) {
			var docfield = capkpi.meta.docfield_map[doctype][fname];
			if(docfield) {
	...
	field_map(fnames, fn) {
		...
		for (var i=0, l=fnames.length; i<l; i++) {
			...
			var field = capkpi.meta.get_docfield(this.doctype, fieldname, this.docname);
			if(field) {
				fn(field);
	...
	get_docfield(fieldname1, fieldname2) {
		if(fieldname2) {
			...
			return capkpi.meta.get_docfield(doctype, fieldname2, this.docname);
		} else {
			// for parent
			return capkpi.meta.get_docfield(this.doctype, fieldname1, this.docname);
		}
	}
	...
	set_df_property(fieldname, property, value, docname, table_field, table_row_name=null) {
		...
		if (!docname || !table_field) {
			...
			const filtered_fields = capkpi.utils.filter_dict(grid.docfields, {'fieldname': table_field});
			if (filtered_fields.length) {
				df = capkpi.meta.get_docfield(filtered_fields[0].parent, table_field, table_row_name);
			}
		}
	...
	get_files() {
		return this.attachments
			? capkpi.utils.sort(this.attachments.get_attachments(), "file_name", "string")
			: [] ;
	}
	...
	clear_table(fieldname) {
		capkpi.model.clear_table(this.doc, fieldname);
	}

	add_child(fieldname, values) {
		var doc = capkpi.model.add_child(this.doc, capkpi.meta.get_docfield(this.doctype, fieldname).options, fieldname);
		if(values) {
	...
	set_value(field, value, if_missing) {
		...
		var _set = function(f, v) {
			...
			if(fieldobj) {
				if(!if_missing || !capkpi.model.has_value(me.doctype, me.doc.name, f)) {
					if(capkpi.model.table_fields.includes(fieldobj.df.fieldtype) && $.isArray(v)) {

						capkpi.model.clear_table(me.doc, fieldobj.df.fieldname);

						for (var i=0, j=v.length; i < j; i++) {
							var d = v[i];
							var child = capkpi.model.add_child(me.doc, fieldobj.df.options,
						...
						return capkpi.model.set_value(me.doctype, me.doc.name, f, v);
					}
				}
			} else {
				capkpi.msgprint(__("Field {0} not found.",[f]));
				throw "frm.set_value";
			}
		};
	...
	call(opts, args, callback) {
		...
		if(!opts.doc) {
			if(opts.method.indexOf(".")===-1)
				opts.method = capkpi.model.get_server_module_name(me.doctype) + "." + opts.method;
			...
			opts.callback = function(r) {
				if($.isPlainObject(r.message)) {
					if(opts.child) {
						...
						var std_field_list = ["doctype"].concat(capkpi.model.std_fields_list);
	...
	set_read_only() {
		const docperms = capkpi.perm.get_perm(this.doc.doctype);
	...
	get_formatted(fieldname) {
		return capkpi.format(this.doc[fieldname],
			capkpi.meta.get_docfield(this.doctype, fieldname, this.docname),
			{no_icon:true}, this.doc);
	}

	open_grid_row() {
		return capkpi.ui.form.get_open_grid_form();
	}
	...
	get_selected() {
		...
		capkpi.meta.get_table_fields(this.doctype).forEach(function(df) {
	...
	set_indicator_formatter(fieldname, get_color, get_text) {
		...
		var doctype;
		if (capkpi.meta.docfield_map[this.doctype][fieldname]) {
			doctype = this.doctype;
		} else {
			capkpi.meta.get_table_fields(this.doctype).every(function(df) {
				if (capkpi.meta.docfield_map[df.options][fieldname]) {
					doctype = df.options;
					return false;
				} else {
					return true;
		...
		capkpi.meta.docfield_map[doctype][fieldname].formatter =
			function(value, df, options, doc) {
				if (value) {
					var label;
					if (get_text) {
						label = get_text(doc);
					} else if (capkpi.form.link_formatters[df.options]) {
						label = capkpi.form.link_formatters[df.options](value, doc);
					} else {
						label = value;
					}
					...
					return `
						<a class="indicator ${get_color(doc || {})}"
							href="/app/${capkpi.router.slug(df.options)}/${escaped_name}"
	...
	can_create(doctype) {
		...
		if(!capkpi.model.can_create(doctype)) {
			return false;
		}
	...
	set_link_field(doctype, new_doc) {
		let me = this;
		capkpi.get_meta(doctype).fields.forEach(function(df) {
	...
	scroll_to_field(fieldname) {
		...
		// scroll to input
		capkpi.utils.scroll_to($el, true, 15);
	...
	show_tour(on_finish) {
		const tour_info = capkpi.tour[this.doctype];

		if (!Array.isArray(tour_info)) {
			return;
		}

		const driver = new capkpi.Driver({
			className: 'capkpi-driver',
			allowClose: false,
			padding: 10,
		...
		let steps = tour_info.map(step => {
			...
			return {
				element: `.capkpi-control[data-fieldname='${step.fieldname}']`,
				popover: {
		...
		capkpi.router.on('change', () => driver.reset());
		driver.start();
	}
	...
	setup_docinfo_change_listener() {
		...
		// to avoid duplicates
		capkpi.realtime.off(listener_name);
		capkpi.realtime.on(listener_name, ({doc, key, action='update'}) => {
			let doc_list = (capkpi.model.docinfo[doctype][docname][key] || []);
			if (action === 'add') {
				capkpi.model.docinfo[doctype][docname][key].push(doc);
			}
			...
			if (docindex > -1) {
				if (action === 'update') {
					capkpi.model.docinfo[doctype][docname][key].splice(docindex, 1, doc);
				}
				if (action === 'delete') {
					capkpi.model.docinfo[doctype][docname][key].splice(docindex, 1);
				}
			}
			// no need to update timeline of owner of comment
			// gets handled via comment submit code
			if (!(['add', 'update'].includes(action) && doc.doctype === 'Comment' && doc.owner === capkpi.session.user)) {
	...
	set_fields_as_options(fieldname, reference_doctype, filter_function, default_options=[], table_fieldname) {
		...
		return new Promise(resolve => {
			capkpi.model.with_doctype(reference_doctype, () => {
				capkpi.get_meta(reference_doctype).fields.map(df => {
...
capkpi.validated = 0;
// Proxy for capkpi.validated
Object.defineProperty(window, 'validated', {
	get: function() {
		console.warn('Please use `capkpi.validated` instead of `validated`. It will be deprecated soon.'); // eslint-disable-line
		return capkpi.validated;
	},
	set: function(value) {
		console.warn('Please use `capkpi.validated` instead of `validated`. It will be deprecated soon.'); // eslint-disable-line
		capkpi.validated = value;
		return capkpi.validated;
	}
});
