...
capkpi.provide("capkpi.ui.form.handlers");

capkpi.ui.form.get_event_handler_list = function(doctype, fieldname) {
	if(!capkpi.ui.form.handlers[doctype]) {
		capkpi.ui.form.handlers[doctype] = {};
	}
	if(!capkpi.ui.form.handlers[doctype][fieldname]) {
		capkpi.ui.form.handlers[doctype][fieldname] = [];
	}
	return capkpi.ui.form.handlers[doctype][fieldname];
}

capkpi.ui.form.on = capkpi.ui.form.on_change = function(doctype, fieldname, handler) {
	var add_handler = function(fieldname, handler) {
		var handler_list = capkpi.ui.form.get_event_handler_list(doctype, fieldname);
...
capkpi.ui.form.off = function(doctype, fieldname, handler) {
	var handler_list = capkpi.ui.form.get_event_handler_list(doctype, fieldname);
...
capkpi.ui.form.trigger = function(doctype, fieldname) {
	cur_frm.script_manager.trigger(fieldname, doctype);
}

capkpi.ui.form.ScriptManager = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
	},
	...
	trigger: function(event_name, doctype, name) {
		...
		this.frm.selected_doc = capkpi.get_doc(doctype, name);
	...
	get_handlers: function(event_name, doctype) {
		...
		if(capkpi.ui.form.handlers[doctype] && capkpi.ui.form.handlers[doctype][event_name]) {
			$.each(capkpi.ui.form.handlers[doctype][event_name], function(i, fn) {
	...
	setup: function() {
		...
		$.each(this.frm.fields, function(i, field) {
			...
			if(capkpi.model.table_fields.includes(field.df.fieldtype)) {
				$.each(capkpi.meta.get_docfields(field.df.options, me.frm.docname), function(i, df) {
					setup_add_fetch(df);
				});
			}
		});
		...
		// css
		doctype.__css && capkpi.dom.set_style(doctype.__css);

		this.trigger('setup');
	},
	log_error: function(caller, e) {
		capkpi.show_alert({message: __("Error in Client Script."), indicator: "error"});
	...
	copy_from_first_row: function(parentfield, current_row, fieldnames) {
		...
		$.each(fieldnames, function(i, fieldname) {
			capkpi.model.set_value(current_row.doctype, current_row.name, fieldname,
				data[0][fieldname]);
		});
	}
});
