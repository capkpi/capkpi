...
capkpi.ui.form.save = function (frm, action, callback, btn) {
	...
	var save = function () {
		...
		if ((action !== 'Save' || frm.is_dirty()) && check_mandatory()) {
			_call({
				method: "capkpi.desk.form.save.savedocs",
			...
			!frm.is_dirty() && capkpi.show_alert({message: __("No changes in document"), indicator: "orange"});
	...
	var remove_empty_rows = function() {
		...
		*/
		const docs = capkpi.model.get_all_docs(frm.doc);

		// we should only worry about table data
		const tables = docs.filter(d => {
			return capkpi.model.is_table(d.doctype);
		});

		let modified_table_fields = [];

		tables.map(doc => {
			const cells = capkpi.meta.docfield_list[doc.doctype] || [];
			...
			if (is_empty_row(in_list_view_cells)) {
				capkpi.model.clear_doc(doc.doctype, doc.name);
	...
	var cancel = function () {
		...
		var workflow_state_fieldname = capkpi.workflow.get_state_fieldname(frm.doctype);
		...
		_call({
			method: "capkpi.desk.form.save.cancel",
			args: args,
			callback: function (r) {
				$(document).trigger("save", [frm.doc]);
				callback(r);
	...
	var check_mandatory = function () {
		...
		$.each(capkpi.model.get_all_docs(frm.doc), function (i, doc) {
			var error_fields = [];
			var folded = false;

			$.each(capkpi.meta.docfield_list[doc.doctype] || [], function (i, docfield) {
				if (docfield.fieldname) {
					const df = capkpi.meta.get_docfield(doc.doctype,
					...
					if (is_docfield_mandatory(doc, df) &&
						!capkpi.model.has_value(doc.doctype, doc.name, df.fieldname)) {
						has_errors = true;
			...
			if (error_fields.length) {
				let meta = capkpi.get_meta(doc.doctype);
				if (meta.istable) {
					const table_label = __(capkpi.meta.docfield_map[doc.parenttype][doc.parentfield].label).bold();
	...
	let is_docfield_mandatory = function(doc, df) {
		...
		let parent = capkpi.get_meta(df.parent);
		...
		if (typeof (expression) === 'boolean') {
			...
			try {
				...
				capkpi.throw(__('Invalid "mandatory_depends_on" expression'));
			}
	...
	var _call = function (opts) {
		...
		if ( frm.is_new() ) {
			capkpi.ui.form.remove_old_form_route();
		}
		...
		return capkpi.call({
			...
			always: function (r) {
				...
				if (r) {
					...
					if (doc) {
						capkpi.ui.form.update_calling_link(doc);
					}
				}
			}
		})
...
capkpi.ui.form.remove_old_form_route = () => {
	let current_route = capkpi.get_route().join("/");
	capkpi.route_history = capkpi.route_history
		.filter((route) => route.join("/") !== current_route);
}

capkpi.ui.form.update_calling_link = (newdoc) => {
	if (!capkpi._from_link) return;
	var doc = capkpi.get_doc(capkpi._from_link.doctype, capkpi._from_link.docname);

	let is_valid_doctype = () => {
		if (capkpi._from_link.df.fieldtype==='Link') {
			return newdoc.doctype === capkpi._from_link.df.options;
		} else {
			// dynamic link, type is dynamic
			return newdoc.doctype === doc[capkpi._from_link.df.options];
		}
	};
	...
	if (is_valid_doctype()) {
		...
		if (doc && doc.parentfield) {
			...
			$.each(capkpi._from_link.frm.fields_dict[doc.parentfield].grid.grid_rows, function (index, field) {
				if (field.doc && field.doc.name === capkpi._from_link.docname) {
					capkpi._from_link.set_value(newdoc.name);
				}
			});
		} else {
			capkpi._from_link.set_value(newdoc.name);
		...
		// refresh field
		capkpi._from_link.refresh();

		// if from form, switch
		if (capkpi._from_link.frm) {
			capkpi.set_route("Form",
				capkpi._from_link.frm.doctype, capkpi._from_link.frm.docname)
				.then(() => {
					capkpi.utils.scroll_to(capkpi._from_link_scrollY);
				});
		}

		capkpi._from_link = null;
	}
}
