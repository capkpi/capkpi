...
capkpi.ui.form.Review = class Review {
	constructor({ parent, frm }) {
		this.parent = parent;
		this.frm = frm;
		this.points = capkpi.boot.points;
		this.reviews = this.parent.find('.reviews');
		this.setup_add_review_button();
		this.update_reviewers();
	}
	update_points() {
		return capkpi.xcall('capkpi.social.doctype.energy_point_log.energy_point_log.get_energy_points', {
			user: capkpi.session.user
		}).then(data => {
			capkpi.boot.points = data;
			this.points = data;
		});
	}
	...
	get_involved_users() {
		...
		return involved_users
			.uniqBy(u => u)
			.filter(user => !['Administrator', capkpi.session.user].includes(user))
			.filter(Boolean);
	}
	show_review_dialog() {
		const user_options = this.get_involved_users();
		const review_dialog = new capkpi.ui.Dialog({
			'title': __('Add Review'),
			'fields': [{
				fieldname: 'to_user',
			...
			primary_action: (values) => {
				...
				if (values.points > this.points.review_points) {
					return capkpi.msgprint(__('You do not have enough points'));
				}
				capkpi.xcall('capkpi.social.doctype.energy_point_log.energy_point_log.review', {
					doc: {
	...
	setup_detail_popover(el, data) {
		let subject = '';
		let fullname = capkpi.user.full_name(data.user);
		let timestamp = `<span class="text-muted">${capkpi.datetime.comment_when(data.creation)}</span>`;
			...
			content: () => {
				return `
					<div class="text-medium">
						...
						<div class="subject">
							${capkpi.utils.icon('review')}
							${subject}

							<p class="mt-1">
								<!-- ${capkpi.avatar("shivam@example.com", "avatar-xs")} -->
								<span>${capkpi.user.full_name(data.owner)}</span> - ${timestamp}
							</p>
						</div>
					</div>
				`;
