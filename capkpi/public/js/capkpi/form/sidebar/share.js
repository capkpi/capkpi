...
capkpi.ui.form.Share = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
	...
	render_sidebar: function() {
		...
		this.shares.append(capkpi.avatar_group(shared_users, 5, {'align': 'left', 'overlap': true}));
	},
	show: function() {
		var me = this;
	...
	render_shared: function(shared) {
		...
		$(capkpi.render_template("set_sharing", {frm: this.frm, shared: this.shared, everyone: everyone}))
			.appendTo(d.body);

		if(capkpi.model.can_share(null, this.frm)) {
	...
	make_user_input: function() {
		// make add-user input
		this.dialog.share_with = capkpi.ui.form.make_control({
	...
	set_edit_share_events: function() {
		...
		$(d.body).find(".edit-share").on("click", function() {
			...
			capkpi.call({
				method: "capkpi.share.set_permission",
				args: {
					doctype: me.frm.doctype,
					name: me.frm.doc.name,
					user: user,
