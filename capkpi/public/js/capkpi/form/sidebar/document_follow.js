...
capkpi.ui.form.DocumentFollow = class DocumentFollow {
	constructor(opts) {
		$.extend(this, opts);
		if (!capkpi.boot.user.document_follow_notify) {
	...
	render_sidebar() {
		...
		const document_follow_enabled = capkpi.boot.user.document_follow_notify;
		const document_can_be_followed = capkpi.get_meta(this.frm.doctype).track_changes;
	...
	bind_events() {
		this.follow_document_link.on('click', () => {
			...
			capkpi.call({
				method: 'capkpi.desk.form.document_follow.follow_document',
				args: {
		...
		this.unfollow_document_link.on('click', () => {
			...
			capkpi.call({
				method: 'capkpi.desk.form.document_follow.unfollow_document',
				args: {
					'doctype': this.frm.doctype,
					'doc_name': this.frm.doc.name,
					'user': capkpi.session.user
	...
	get_followed_user() {
		...
		return new Promise(resolve => {
			capkpi.call({
				method: 'capkpi.desk.form.document_follow.get_follow_users',
				args: {
					'doctype': this.frm.doctype,
					'doc_name': this.frm.doc.name,
				},
				...
				for (var d in r.message) {
					...
					if(this.count_others < 4){
						html += capkpi.avatar(r.message[d].user, 'avatar-small');
					}
