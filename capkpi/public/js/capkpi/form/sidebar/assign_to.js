...
capkpi.ui.form.AssignTo = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
	...
	render: function(assignments) {
		...
		let avatar_group = capkpi.avatar_group(assigned_users, 5, {'align': 'left', 'overlap': true});

		assignments_wrapper.show();
		assignments_wrapper.append(avatar_group);
		avatar_group.click(() => {
			new capkpi.ui.form.AssignmentDialog({
	...
	add: function() {
		...
		if (this.frm.is_new()) {
			capkpi.throw(__("Please save the document before assignment"));
			return;
		}

		if (!me.assign_to) {
			me.assign_to = new capkpi.ui.form.AssignToDialog({
	...
	remove: function(owner) {
		if (this.frm.is_new()) {
			capkpi.throw(__("Please save the document before removing assignment"));
			return;
		}

		return capkpi.xcall('capkpi.desk.form.assign_to.remove', {
...
capkpi.ui.form.AssignToDialog = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
	...
	assign_to_me: function() {
		...
		if (me.dialog.get_value("assign_to_me")) {
			assign_to.push(capkpi.session.user);
		}
		...
		return [
			...
			},
			{
				...
				get_data: function(txt) {
					return capkpi.db.get_link_options("User", txt, {user_type: "System User", enabled: 1});
				}
			},
			{
...
capkpi.ui.form.AssignmentDialog = class {
	constructor(opts) {
		this.frm = opts.frm;
	...
	make() {
		...
		this.assignment_list.removeClass('capkpi-control');
	...
	add_assignment(assignment) {
		return capkpi.xcall('capkpi.desk.form.assign_to.add', {
			doctype: this.frm.doctype,
			name: this.frm.docname,
	...
	get_assignment_row(assignment) {
		let row = $(`
			<div class="dialog-assignment-row" data-user="${assignment}">
				<span>
					${capkpi.avatar(assignment)}
					${capkpi.user.full_name(assignment)}
				</span>
			</div>
		`);

		if (assignment === capkpi.session.user || this.frm.perm[0].write) {
			row.append(`
				<span class="remove-btn cursor-pointer">
					${capkpi.utils.icon('close')}
				</span>
			`);
