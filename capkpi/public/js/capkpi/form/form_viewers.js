capkpi.ui.form.FormViewers = class FormViewers {
	...
	refresh() {
		...
		let currently_viewing = users.current.filter(user => user != capkpi.session.user);
		let avatar_group = capkpi.avatar_group(currently_viewing, 5, {'align': 'left', 'overlap': true});
		this.parent.empty().append(avatar_group);
	}
};

capkpi.ui.form.FormViewers.set_users = function(data, type) {
	const doctype = data.doctype;
	const docname = data.docname;
	const docinfo = capkpi.model.get_docinfo(doctype, docname);

	const past_users = ((docinfo && docinfo[type]) || {}).past || [];
	const users = data.users || [];
	const new_users = users.filter(user => !past_users.includes(user));

	capkpi.model.set_docinfo(doctype, docname, type, {
