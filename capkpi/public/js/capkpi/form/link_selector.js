...
capkpi.ui.form.LinkSelector = Class.extend({
	init: function (opts) {
		...
		if (this.doctype != "[Select]") {
			capkpi.model.with_doctype(this.doctype, function (r) {
				me.make();
			});
		} else {
			this.make();
	...
	search: function () {
		...
		capkpi.link_search(this.doctype, args, function (r) {
			...
			if (r.values.length) {
				...
				$('<p><br><span class="text-muted">' + __("No Results") + '</span>'
					+ (capkpi.model.can_create(me.doctype) ?
			...
			capkpi.prompt({
				...
				$.each(me.target.frm.doc[me.target.df.fieldname] || [], function (i, d) {
					if (d[me.fieldname] === value) {
						capkpi.model.set_value(d.doctype, d.name, me.qty_fieldname, data.qty);
						capkpi.show_alert(__("Added {0} ({1})", [value, d[me.qty_fieldname]]));
						updated = true;
						return false;
					}
				});
				if (!updated) {
					capkpi.run_serially([
						...
						() => {
							...
							return capkpi.model.set_value(d.doctype, d.name, args);
						},
						() => capkpi.show_alert(__("Added {0} ({1})", [value, data.qty]))
					]);
				}
			}, __("Set Quantity"), __("Set"));
		} else if (me.dynamic_link_field) {
			var d = me.target.add_new_row();
			capkpi.model.set_value(d.doctype, d.name, me.dynamic_link_field, me.dynamic_link_reference);
			capkpi.model.set_value(d.doctype, d.name, me.fieldname, value);
			capkpi.show_alert(__("{0} {1} added", [me.dynamic_link_reference, value]));
		} else {
			var d = me.target.add_new_row();
			capkpi.model.set_value(d.doctype, d.name, me.fieldname, value);
			capkpi.show_alert(__("{0} added", [value]));
		}
	}
});

capkpi.link_search = function (doctype, args, callback, btn) {
	if (!args) {
		args = {
			txt: ''
		}
