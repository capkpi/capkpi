...
capkpi.ui.form.Footer = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		this.make();
	...
	make: function() {
		this.wrapper = $(capkpi.render_template("form_footer", {}))
	...
	make_comment_box: function() {
		this.frm.comment_box = capkpi.ui.form.make_control({
			...
			on_submit: (comment) => {
				if (strip_html(comment).trim() != "" || comment.includes('img')) {
					...
					capkpi.xcall("capkpi.desk.form.utils.add_comment", {
