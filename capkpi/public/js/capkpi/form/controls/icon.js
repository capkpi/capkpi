...
capkpi.ui.form.ControlIcon = class ControlIcon extends capkpi.ui.form.ControlData {
	make_input() {
	...
	get_all_icons() {
		capkpi.symbols = [];
		$("#capkpi-symbols > symbol[id]").each(function() {
			capkpi.symbols.push(this.id.replace('icon-', ''));
		});
	}
	...
	make_icon_input() {
		...
		if (!this.selected_icon) {
			this.selected_icon = $(`<div class="selected-icon">${capkpi.utils.icon("folder-normal", "md")}</div>`);
