capkpi.ui.form.ControlTableMultiSelect = capkpi.ui.form.ControlLink.extend({
	make_input() {
		this._super();
		...
		this.$input_area.on('click', '.btn-link-to-form', (e) => {
			...
			const value = decodeURIComponent($value.data().value);
			const link_field = this.get_link_field();
			capkpi.set_route('Form', link_field.options, value);
		});
		this.$input.on('keydown', e => {
			// if backspace key pressed on empty input, delete last value
			if (e.keyCode == capkpi.ui.keyCode.BACKSPACE && e.target.value === '') {
	...
	parse(value) {
		...
		if (value) {
			if (this.frm) {
				const new_row = capkpi.model.add_child(this.frm.doc, this.df.options, this.df.fieldname);
	...
	get_pill_html(value) {
		...
		return `
			<button class="data-pill btn tb-selected-value" data-value="${encoded_value}">
				...
				<span class="btn-remove">${capkpi.utils.icon('close')}</span>
			</button>
		`;
	},
	get_options() {
	...
	get_link_field() {
		if (!this._link_field) {
			const meta = capkpi.get_meta(this.df.options);
