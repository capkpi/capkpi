capkpi.ui.form.ControlSelect = capkpi.ui.form.ControlData.extend({
	html_element: 'select',
	make_input: function() {
		this._super();
	...
	set_icon: function(is_xs_input) {
		const select_icon_html =
			`<div class="select-icon ${is_xs_input ? 'xs' : ''}">
				${capkpi.utils.icon('select', is_xs_input ? 'xs' : 'sm')}
			</div>`;
		if (this.only_input) {
	...
	get_file_attachment_list: function() {
		if(!this.frm) return;
		var fl = capkpi.model.docinfo[this.frm.doctype][this.frm.docname];
