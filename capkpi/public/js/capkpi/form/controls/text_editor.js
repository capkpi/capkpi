...
capkpi.ui.form.ControlTextEditor = capkpi.ui.form.ControlCode.extend({
	make_wrapper() {
		this._super();
	},
	...
	bind_events() {
		this.quill.on('text-change', capkpi.utils.debounce((delta, oldDelta, source) => {
		...
		$(this.quill.root).on('keydown', (e) => {
			const key = capkpi.ui.keys && capkpi.ui.keys.get_key(e);
	...
	parse(value) {
		...
		return capkpi.dom.remove_script_and_style(value);
	},
