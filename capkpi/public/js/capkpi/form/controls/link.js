...
capkpi.ui.form.recent_link_validations = {};

capkpi.ui.form.ControlLink = capkpi.ui.form.ControlData.extend({
		...
		$(`<div class="link-field ui-front" style="position: relative;">
			...
			<span class="link-btn">
				<a class="btn-open no-decoration" title="${__("Open Link")}">
					${capkpi.utils.icon('arrow-right', 'xs')}
				</a>
			</span>
		...
		this.$input.on("focus", function() {
			setTimeout(function() {
				if (me.$input.val() && me.get_options()) {
					...
					me.$link.toggle(true);
					me.$link_open.attr('href', capkpi.utils.get_form_link(doctype, name));
				}
	...
	get_reference_doctype() {
		...
		if (this.doctype) return this.doctype;
			...
			return capkpi.get_route && capkpi.get_route()[0] === 'List' ? capkpi.get_route()[1] : null;
		}
	},
	...
	is_translatable() {
		return in_list(capkpi.boot.translated_doctypes || [], this.get_options());
	},
	...
	new_doc: function() {
		...
		if (df && df.get_route_options_for_new_doc) {
			capkpi.route_options = df.get_route_options_for_new_doc(this);
		} else {
			capkpi.route_options = {};
		}

		// partially entered name field
		capkpi.route_options.name_field = this.get_label_value();

		// reference to calling link
		capkpi._from_link = this;
		capkpi._from_link_scrollY = $(document).scrollTop();

		capkpi.ui.form.make_quick_entry(doctype, (doc) => {
	...
	setup_awesomeplete: function() {
		...
		this.$input.on("input", capkpi.utils.debounce(function(e) {
		...
		this.$input.on("awesomplete-select", function(e) {
			...
			if (me.df.remember_last_selected_value) {
				capkpi.boot.user.last_selected_values[me.df.options] = item.value;
			}
	...
	toggle_href(doctype) {
		if (capkpi.model.can_select(doctype) && !capkpi.model.can_read(doctype)) {
	...
	get_filter_description(filters) {
		...
		function get_filter_description(filter) {
			...
			let docfield = capkpi.meta.get_docfield(doctype, fieldname);
			let label = docfield ? docfield.label : capkpi.model.unscrub(fieldname);
	...
	set_custom_query(args) {
		const is_valid_value = (value, key) => {
			...
			if (this.frm) {
				let field = capkpi.meta.get_docfield(this.frm.doctype, key);
	...
	validate_link_and_fetch(df, options, docname, value) {
		...
		if (value) {
			return capkpi.xcall("capkpi.client.validate_link", {
				doctype: options,
				docname: value,
