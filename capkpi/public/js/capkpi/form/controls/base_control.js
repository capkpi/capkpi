capkpi.ui.form.Control = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		this.make();

		// if developer_mode=1, show fieldname as tooltip
		if(capkpi.boot.user && capkpi.boot.developer_mode===1 && this.$wrapper) {
	...
	make_wrapper: function() {
		this.$wrapper = $("<div class='capkpi-control'></div>").appendTo(this.parent);
	...
	get_status: function(explain) {
		...
		var status = capkpi.perm.get_field_display_status(this.df,
			capkpi.model.get_doc(this.doctype, this.docname), this.perm || (this.frm && this.frm.perm), explain);
		...
		if (this.doctype && status==="Read" && !this.only_input
			&& is_null(capkpi.model.get_value(this.doctype, this.docname, this.df.fieldname))
	...
	show_translatable_button(value) {
		...
		if (!capkpi.model
			...
			|| !capkpi.model.can_write('Translation')
			|| !value) return;

		// Disable translation in website
		if (!capkpi.views || !capkpi.views.TranslationManager) return;
		...
		$(translation_btn)
			...
			.on('click', () => {
				if (!this.doc.__islocal) {
					new capkpi.views.TranslationManager({
						'df': this.df,
	...
	set_model_value: function(value) {
		if (this.frm) {
			...
			return capkpi.model.set_value(this.doctype, this.docname, this.df.fieldname,
