capkpi.ui.form.ControlCode = capkpi.ui.form.ControlText.extend({
	make_input() {
	...
	make_ace_editor() {
		...
		// events
		this.editor.session.on('change', capkpi.utils.debounce(() => {
	...
	load_lib() {
		...
		if (capkpi.boot.developer_mode) {
			this.root_lib_path = '/assets/capkpi/node_modules/ace-builds/src-noconflict/';
		} else {
			this.root_lib_path = '/assets/capkpi/node_modules/ace-builds/src-min-noconflict/';
		}

		this.library_loaded = new Promise(resolve => {
			capkpi.require(this.root_lib_path + 'ace.js', () => {
