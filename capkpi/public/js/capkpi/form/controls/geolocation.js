...
capkpi.ui.form.ControlGeolocation = capkpi.ui.form.ControlData.extend({
	horizontal: false,
	...
	make_wrapper() {
		...
		if (this.frm) {
			...
			$(document).on('capkpi.ui.Dialog:shown', () => {
				this.make_map();
			});
		}
	},
	...
	format_for_input(value) {
		...
		if(value) {
			...
			this.map.setView(capkpi.utils.map_defaults.center, capkpi.utils.map_defaults.zoom);
		}
	...
	bind_leaflet_draw_control() {
		...
		var options = {
			...
			draw: {
				polyline: {
					shapeOptions: {
						color: capkpi.ui.color.get('blue'),
						weight: 10
					}
				},
				polygon: {
					...
					drawError: {
						color: capkpi.ui.color.get('orange'), // Color the shape will turn when intersects
