...
capkpi.ui.form.ControlComment = capkpi.ui.form.ControlTextEditor.extend({
	make_wrapper() {
		...
		// wrapper should point to capkpi-control
		this.$wrapper = !this.no_wrapper
			? this.comment_wrapper.find('.capkpi-control')
	...
	bind_events() {
		...
		this.$wrapper.on('keydown', e => {
			const key = capkpi.ui.keys.get_key(e);
			if (key === 'ctrl+enter') {
				e.preventDefault();
				this.submit();
			}
		...
		this.quill.on('text-change', capkpi.utils.debounce(() => {
			this.update_state();
		}, 300));
	},
	...
	get_mention_options() {
		...
		return {
			...
			source: capkpi.utils.debounce(async function(search_term, renderList) {
				let method = me.mention_search_method || 'capkpi.desk.search.get_names_for_mentions';
				let values = await capkpi.xcall(method, {
					search_term
				});
			...
			renderItem(item) {
				...
				return `${value} ${item.is_group ? capkpi.utils.icon('users') : ''}`;
			}
		};
	},
