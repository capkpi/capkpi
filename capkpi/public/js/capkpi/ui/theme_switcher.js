...
capkpi.ui.ThemeSwitcher = class ThemeSwitcher {
	constructor() {
		this.setup_dialog();
		this.refresh();
	}
		...
		const preview = $(`<div class="${this.current_theme == theme.name ? "selected" : "" }">
			<div data-theme=${theme.name}>
				<div class="background">
					<div>
						<div class="preview-check">${capkpi.utils.icon('tick', 'xs')}</div>
					</div>
	...
	toggle_theme(theme) {
		...
		capkpi.show_alert(__("Theme Changed"), 3);

		capkpi.xcall("capkpi.core.doctype.user.user.switch_theme", {
			theme: toTitle(theme)
		});
	}
	show() {
