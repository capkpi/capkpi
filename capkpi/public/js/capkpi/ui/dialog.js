...
capkpi.ui.open_dialogs = [];

capkpi.ui.Dialog = class Dialog extends capkpi.ui.FieldGroup {
	constructor(opts) {
		super();
		this.display = false;
		this.is_dialog = true;
	...
	make() {
		...
		this.$wrapper
			.on("hide.bs.modal", function() {
				...
				if(capkpi.ui.open_dialogs[capkpi.ui.open_dialogs.length-1]===me) {
					capkpi.ui.open_dialogs.pop();
					if(capkpi.ui.open_dialogs.length) {
						window.cur_dialog = capkpi.ui.open_dialogs[capkpi.ui.open_dialogs.length-1];
					} else {
						window.cur_dialog = null;
					}
				}
			...
			.on("shown.bs.modal", function() {
				...
				capkpi.ui.open_dialogs.push(me);
				me.focus_on_first_input();
				me.hide_scrollbar(true);
				me.on_page_show && me.on_page_show();
				$(document).trigger('capkpi.ui.Dialog:shown');
	...
	toggle_minimize() {
		...
		this.get_minimize_btn().html(capkpi.utils.icon(icon));
