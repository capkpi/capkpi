...
capkpi.ui.Tags = class {
	...
	bind() {
		...
		const select_tag = function() {
			const tagValue = capkpi.utils.xss_sanitise(me.$input.val());
			me.addTag(tagValue);
			me.$input.val('');
		};
	...
	removeTag(label) {
		label = capkpi.utils.xss_sanitise(label);
	...
	get_tag(label) {
		let $tag = capkpi.get_data_pill(label, label, (target, pill_wrapper) => {
