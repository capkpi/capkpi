 ...
 *
 * @returns {capkpi.ui.Page}
 */

/**
 * @typedef {Object} capkpi.ui.Page
 */


capkpi.ui.make_app_page = function(opts) {
	opts.parent.page = new capkpi.ui.Page(opts);
	return opts.parent.page;
}

capkpi.ui.pages = {};

capkpi.ui.Page = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		...
		this.make();
		capkpi.ui.pages[capkpi.get_route_str()] = this;
	},
	...
	load_lib: function(callback) {
		capkpi.require(this.required_libs, callback);
	},

	add_main_section: function() {
		$(capkpi.render_template("page", {})).appendTo(this.wrapper);
	...
	setup_page: function() {
		...
		this.card_layout && this.main.addClass('capkpi-card');
	...
	setup_sidebar_toggle() {
		...
		if (this.disable_sidebar_toggle || !sidebar_wrapper.length) {
			...
			sidebar_toggle.click(() => {
				if (capkpi.utils.is_xs() || capkpi.utils.is_sm()) {
	...
	update_sidebar_icon() {
		...
		sidebar_toggle_icon.html(capkpi.utils.icon(is_sidebar_visible ? 'sidebar-collapse' : 'sidebar-expand', 'md'));
	},
	...
	add_action_icon: function(icon, click, css_class='', tooltip_label) {
		const button = $(`
			<button class="text-muted btn btn-default ${css_class} icon-btn">
				${capkpi.utils.icon(icon)}
			</button>
		`);

		button.appendTo(this.icon_group.removeClass("hide"));
		button.click(click);
		button.attr("title", __(tooltip_label || capkpi.unscrub(icon)))
	...
	get_icon_label: function(icon, label) {
		...
		return `${icon ? capkpi.utils.icon(icon_name, size) : ''} <span class="hidden-xs"> ${__(label)} </span>`;
	},
	...
	*/
	add_dropdown_item: function({label, click, standard, parent, shortcut, show_parent=true, icon=null}) {
		...
		if (icon) {
			$icon = `<span class="menu-item-icon">${capkpi.utils.icon(icon)}</span>`;
		}
		...
		if (shortcut) {
			...
			`);
			capkpi.ui.keys.add_shortcut(shortcut_obj);
		} else {
			$li = $(`
				<li>
					<a class="grey-link dropdown-item" href="#" onClick="return false;">
	...
	*/
	...
	set_title: function(title, icon=null, strip=true, tab_title="") {
		...
		this.title = title;
		capkpi.utils.set_title(tab_title || title);
		if (icon) {
			title = `${capkpi.utils.icon(icon)} ${title}`;
		}
	...
	add_button: function(label, click, opts) {
		...
		let button = $(`<button
			class="btn ${opts.btn_class || 'btn-default'} ${opts.btn_size || 'btn-sm'} ellipsis">
				${opts.icon ? capkpi.utils.icon(opts.icon): ''}
				${label}
		</button>`);
	...
	add_dropdown_button: function(parent, label, click, icon) {
		capkpi.ui.toolbar.add_dropdown_button(parent, label, click, icon);
	},
	...
	add_field: function(df, parent) {
		...
		var f = capkpi.ui.form.make_control({
			df: df,
