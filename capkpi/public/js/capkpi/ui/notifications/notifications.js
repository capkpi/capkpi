...
capkpi.ui.Notifications = class Notifications {
	constructor() {
		this.tabs = {};
		this.notification_settings = capkpi.boot.notification_settings;
		this.make();
	}
	...
	setup_headers() {
		...
		$(`<span class="notification-settings pull-right" data-action="go_to_settings">
			${capkpi.utils.icon('setting-gear')}
		</span>`)
			.on('click', (e) => {
				e.stopImmediatePropagation();
				this.dropdown.dropdown('hide');
				capkpi.set_route('Form', 'Notification Settings', capkpi.session.user);
			}).appendTo(this.header_actions)
			.attr('title', __("Notification Settings"))
			.tooltip({ delay: { "show": 600, "hide": 100}, trigger: "hover"  });

		$(`<span class="mark-all-read pull-right" data-action="mark_all_as_read">
			${capkpi.utils.icon('mark-as-read')}
		</span>`)
	...
	mark_all_as_read(e) {
		...
		capkpi.call(
			'capkpi.desk.doctype.notification_log.notification_log.mark_all_as_read',
		);
	}
...
capkpi.ui.notifications = {
	get_notification_config() {
		return capkpi.xcall('capkpi.desk.notifications.get_notification_info').then(r => {
			capkpi.ui.notifications.config = r;
			return r;
		});
	},

	show_open_count_list(doctype) {
		if (!capkpi.ui.notifications.config) {
	...
	route_to_list_with_filters(doctype) {
		let filters = capkpi.ui.notifications.config['conditions'][doctype];
		if (filters && $.isPlainObject(filters)) {
			if (!capkpi.route_options) {
				capkpi.route_options = {};
			}
			$.extend(capkpi.route_options, filters);
		}
...
class NotificationsView extends BaseNotificationsView {
	...
	mark_as_read(docname, $el) {
		capkpi.call(
			'capkpi.desk.doctype.notification_log.notification_log.mark_as_read',
			{ docname: docname }
		).then(() => {
	...
	get_dropdown_item_html(field) {
		...
		message = title ? message.replace(title[1], capkpi.ellipsis(strip_html(title[1]), 100)) : message;

		let timestamp = capkpi.datetime.comment_when(field.creation);
		...
		let user_avatar = capkpi.avatar(user, 'avatar-medium user-avatar');
			...
			if (this.dropdown_items.length) {
				...
				this.container.append($(`<div class="notification-null-state">
					<div class="text-center">
						<img src="/assets/capkpi/images/ui-states/notification-empty-state.svg" alt="Generic Empty State" class="null-state">
	...
	get_notifications_list(limit) {
		return capkpi.db.get_list('Notification Log', {
			fields: ["*"],
			limit: limit,
	...
	get_item_link(notification_doc) {
		...
		return capkpi.utils.get_form_link(
			link_doctype,
			link_docname
		);
	}
	...
	toggle_seen(flag) {
		capkpi.call(
			'capkpi.desk.doctype.notification_settings.notification_settings.set_seen_value',
			{
				value: cint(flag),
	...
	setup_notification_listeners() {
		capkpi.realtime.on('notification', () => {
			this.toggle_notification_icon(false);
			this.update_dropdown();
		});

		capkpi.realtime.on('indicator_hide', () => {
		...
		this.parent.on('show.bs.dropdown', () => {
			...
			if (this.notifications_icon.find('.notifications-unseen').is(':visible')) {
				this.toggle_notification_icon(true);
				capkpi.call(
					'capkpi.desk.doctype.notification_log.notification_log.trigger_indicator_hide'
				);
			}
		});
...
class EventsView extends BaseNotificationsView {
	make() {
		let today = capkpi.datetime.get_today();
		capkpi.xcall('capkpi.desk.doctype.event.event.get_events', {
			start: today,
			end: today
		}).then(event_list => {
	...
	render_events_html(event_list) {
		...
		if (event_list.length) {
			let get_event_html = (event) => {
				let time = __("All Day");
				if (!event.all_day) {
					let start_time = capkpi.datetime.get_time(event.starts_on);
					let days_diff = capkpi.datetime.get_day_diff(event.ends_on, event.starts_on);
					let end_time = capkpi.datetime.get_time(event.ends_on);
					if (days_diff > 1) {
				...
				if (event.particpants) {
					particpants = capkpi.avatar_group(event.particpants, 3);
				}
			...
			html = `
				<div class="notification-null-state">
					<div class="text-center">
					<img src="/assets/capkpi/images/ui-states/event-empty-state.svg" alt="Generic Empty State" class="null-state">
