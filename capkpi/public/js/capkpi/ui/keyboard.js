...
capkpi.provide('capkpi.ui.keys.handlers');

capkpi.ui.keys.setup = function() {
	$(window).on('keydown', function(e) {
		var key = capkpi.ui.keys.get_key(e);
		if(capkpi.ui.keys.handlers[key]) {
			var out = null;
			for(var i=0, l = capkpi.ui.keys.handlers[key].length; i<l; i++) {
				var handler = capkpi.ui.keys.handlers[key][i];
...
capkpi.ui.keys.standard_shortcuts = standard_shortcuts;
capkpi.ui.keys.add_shortcut = ({shortcut, action, description, page, target, condition, ignore_inputs = false} = {}) => {
	...
	capkpi.ui.keys.off(shortcut, page);
	// attach new handler
	capkpi.ui.keys.on(shortcut, handler);
...
capkpi.ui.keys.show_keyboard_shortcut_dialog = () => {
	if (capkpi.ui.keys.is_dialog_shown) return;
		...
		let html = shortcuts
			...
			.map(shortcut => {
				let shortcut_label = shortcut.shortcut
					.split('+')
					.map(capkpi.utils.to_title_case)
					.join('+');
				if (capkpi.utils.is_mac()) {
	...
	let dialog = new capkpi.ui.Dialog({
		...
		on_hide() {
			capkpi.ui.keys.is_dialog_shown = false;
		}
	});
	...
	dialog.show();
	capkpi.ui.keys.is_dialog_shown = true;
}

capkpi.ui.keys.get_key = function(e) {
	var keycode = e.keyCode || e.which;
	var key = capkpi.ui.keys.key_map[keycode] || String.fromCharCode(keycode);
...
capkpi.ui.keys.on = function(key, handler) {
	if(!capkpi.ui.keys.handlers[key]) {
		capkpi.ui.keys.handlers[key] = [];
	}
	capkpi.ui.keys.handlers[key].push(handler);
}

capkpi.ui.keys.off = function(key, page) {
	let handlers = capkpi.ui.keys.handlers[key];
	if (!handlers || handlers.length === 0) return;
	capkpi.ui.keys.handlers[key] = handlers.filter(h => {
...
capkpi.ui.keys.add_shortcut({
	shortcut: 'ctrl+s',
	action: function(e) {
		capkpi.app.trigger_primary_action();
		e.preventDefault();
		return false;
	},
...
capkpi.ui.keys.on('escape', function(e) {
	close_grid_and_dialog();
});

capkpi.ui.keys.on('esc', function(e) {
	close_grid_and_dialog();
});

capkpi.ui.keys.on('enter', function(e) {
	if(window.cur_dialog && cur_dialog.confirm_dialog) {
		cur_dialog.get_primary_btn().trigger('click');
	}
});

capkpi.ui.keys.on('ctrl+down', function(e) {
	var grid_row = capkpi.ui.form.get_open_grid_form();
	grid_row && grid_row.toggle_view(false, function() { grid_row.open_next() });
});

capkpi.ui.keys.on('ctrl+up', function(e) {
	var grid_row = capkpi.ui.form.get_open_grid_form();
...
'abcdefghijklmnopqrstuvwxyz'.split('').forEach((letter, i) => {
	capkpi.ui.keys.key_map[65 + i] = letter;
});
...
$(document).on('keydown', '.dropdown-toggle', (e) => {
	if (e.which === capkpi.ui.keyCode.ESCAPE) {
