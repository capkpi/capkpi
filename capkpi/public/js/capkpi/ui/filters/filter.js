capkpi.ui.Filter = class {
	constructor(opts) {
		...
		this.utils = capkpi.ui.filter_utils;
		this.set_conditions();
	...
	set_conditions_from_config() {
		if (capkpi.boot.additional_filters_config) {
			this.filters_config = capkpi.boot.additional_filters_config;
	...
	make() {
		this.filter_edit_area = $(
			capkpi.render_template('edit_filter', {
	...
	make_select() {
		this.fieldselect = new capkpi.ui.FieldSelect({
	...
	make_field(df, old_fieldtype) {
		...
		let f = capkpi.ui.form.make_control({
			df: df,
			parent: field_area,
			only_input: true,
		});
	...
	toggle_nested_set_conditions(df) {
		let show_condition =
			df.fieldtype === 'Link' &&
			capkpi.boot.nested_set_doctypes.includes(df.options);
...
capkpi.ui.filter_utils = {
	get_formatted_value(field, value) {
		...
		return capkpi.format(value, field.df, { only_value: 1 });
	},
