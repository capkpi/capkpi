...
capkpi.ui.FieldSelect = Class.extend({
	...
	build_options() {
		var me = this;
		me.table_fields = [];
		var std_filters = $.map(capkpi.model.std_fields, function(d) {
		...
		// main table
		var main_table_fields = std_filters.concat(capkpi.meta.docfield_list[me.doctype]);
		$.each(capkpi.utils.sort(main_table_fields, "label", "string"), function(i, df) {
			let doctype = capkpi.get_meta(me.doctype).istable && me.parent_doctype ?
				me.parent_doctype : me.doctype;

			// show fields where user has read access and if report hide flag is not set
			if (capkpi.perm.has_perm(doctype, df.permlevel, "read"))
		...
		$.each(me.table_fields, function(i, table_df) {
			if(table_df.options) {
				let child_table_fields = [].concat(capkpi.meta.docfield_list[table_df.options]);

				if (table_df.fieldtype === "Table MultiSelect") {
					const link_field = capkpi.meta.get_docfields(table_df.options)
						.find(df => df.fieldtype === 'Link');
					child_table_fields = link_field ? [link_field] : [];
				}

				$.each(capkpi.utils.sort(child_table_fields, "label", "string"), function(i, df) {
					let doctype = capkpi.get_meta(me.doctype).istable && me.parent_doctype ?
						me.parent_doctype : me.doctype;
	...
	add_field_option(df) {
		...
		if (df.fieldname == 'docstatus' && !capkpi.model.is_submittable(me.doctype))
			return;
		...
		if(capkpi.model.no_value_type.indexOf(df.fieldtype) == -1 &&
