capkpi.ui.FilterGroup = class {
	...
	add_filters(filters) {
		...
		return capkpi.run_serially(promises).then(() => this.update_filters());
	}
	...
	_push_new_filter(doctype, fieldname, condition, value, hidden = false) {
		...
		let filter = new capkpi.ui.Filter(args);
	...
	filter_exists(filter_value) {
		...
		this.filters
			...
			.map((f) => {
				...
				let equal = capkpi.utils.arrays_equal;
