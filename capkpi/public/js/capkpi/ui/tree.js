...
capkpi.ui.Tree = class {
	constructor({
		...
		if (!icon_set) {
			this.icon_set = {
				open: capkpi.utils.icon('folder-open', 'md'),
				closed: capkpi.utils.icon('folder-normal', 'md'),
				leaf: capkpi.utils.icon('primitive-dot', 'xs')
			};
		}
	...
	get_all_nodes(value, is_root, label) {
		...
		return new Promise(resolve => {
			capkpi.call({
				method: 'capkpi.desk.treeview.get_all_nodes',
				args: args,
				callback: (r) => {
	...
	on_node_click(node) {
		this.expand_node(node);
		capkpi.dom.activate(this.wrapper, node.$tree_link, 'tree-link');
