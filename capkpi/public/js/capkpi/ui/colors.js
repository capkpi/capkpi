...
capkpi.ui.color = {
	get: function(color_name, shade) {
		...
		return capkpi.ui.color_map;
	},
	get_color: function(color_name) {
		const color_names = Object.keys(capkpi.ui.color_map);
		if(color_names.includes(color_name)) {
			return capkpi.ui.color_map[color_name];
		} else {
	...
	all: function() {
		return Object.values(capkpi.ui.color_map)
			.reduce((acc, curr) => acc.concat(curr) , []);
	},
	names: function() {
		return Object.keys(capkpi.ui.color_map);
	},
	...
	get_color_name: function(hex) {
		for (const key in capkpi.ui.color_map) {
			const colors = capkpi.ui.color_map[key];
...
capkpi.ui.color_map = capkpi.ui.color.get_color_map();
