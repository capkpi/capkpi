capkpi.provide('capkpi.ui.keys');

let shortcut_groups = new WeakMap();
let shortcut_group_list = [];
capkpi.ui.keys.shortcut_groups = shortcut_groups;

capkpi.ui.keys.get_shortcut_group = (parent) => {
	// parent must be an object
	if (!shortcut_groups.has(parent)) {
		shortcut_groups.set(parent, new capkpi.ui.keys.AltShortcutGroup());
	}
...
capkpi.ui.keys.bind_shortcut_group_event = () => {
	...
	$(document).on('keydown', (e) => {
		let key = (capkpi.ui.keys.key_map[e.which] || '').toLowerCase();
...
capkpi.ui.keys.AltShortcutGroup = class AltShortcutGroup {
	constructor() {
		this.shortcuts_dict = {};
	...
	is_taken(letter) {
		let is_in_global_shortcut = capkpi.ui.keys.standard_shortcuts
			.filter(s => !s.page)
