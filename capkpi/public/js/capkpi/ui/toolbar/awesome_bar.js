...
capkpi.provide('capkpi.tags');

capkpi.search.AwesomeBar = Class.extend({
	setup: function(element) {
		...
		$input.on("input", capkpi.utils.debounce(function(e) {
			...
			if (txt && txt.length > 1) {
				...
				me.options = me.options.concat(
					me.deduplicate(capkpi.search.utils.get_recent_pages(txt || "")));
				me.options = me.options.concat(capkpi.search.utils.get_frequent_links());
			}
			me.add_help();
		...
		$input.on("awesomplete-select", function(e) {
			...
			if(item.route_options) {
				capkpi.route_options = item.route_options;
			}

			if(item.onclick) {
				item.onclick(item.match);
			} else {
				capkpi.set_route(item.route);
			}
			$input.val("");
		});
		...
		capkpi.search.utils.setup_recent();
		capkpi.tags.utils.fetch_tags();
	},
	...
	add_help: function() {
		this.options.push({
			...
			onclick: function() {
				...
				capkpi.msgprint(txt, __("Search Help"));
			}
		});
	},
	...
	build_options: function(txt) {
		var options = capkpi.search.utils.get_creatables(txt).concat(
			capkpi.search.utils.get_search_in_list(txt),
			capkpi.search.utils.get_doctypes(txt),
			capkpi.search.utils.get_reports(txt),
			capkpi.search.utils.get_pages(txt),
			capkpi.search.utils.get_workspaces(txt),
			capkpi.search.utils.get_dashboards(txt),
			capkpi.search.utils.get_recent_pages(txt || ""),
			capkpi.search.utils.get_executables(txt)
		);
		if (txt.charAt(0) === "#") {
			options = capkpi.tags.utils.get_tags(txt);
		}
	...
	make_global_search: function(txt) {
		...
		// 		<span class="ellipsis">Search for ${capkpi.utils.xss_sanitise(txt).bold()}</span>
		// 		<kbd>↵</kbd>
		// 	</span>
		// `);

		// search_text.click(() => {
		// 	capkpi.searchdialog.search.init_search(txt, "global_search");
		// });
		...
		this.options.push({
			label: `
				<span class="flex justify-between text-medium">
					<span class="ellipsis">${ __('Search for {0}', [capkpi.utils.xss_sanitise(txt).bold()])}</span>
					<kbd>↵</kbd>
				</span>
			`,
	...
	make_search_in_current: function(txt) {
		...
		if(route[0]==="List" && txt.indexOf(" in") === -1) {
			...
			var meta = capkpi.get_meta(capkpi.container.page.list_view.doctype);
		...
		if(first==parseInt(first) || first==="(" || first==="=") {
			...
			try {
				...
				this.options.push({
					...
					default: "Calculator",
					onclick: function() {
						capkpi.msgprint(formatted_value, "Result");
					}
				});
			} catch(e) {
				// pass
	...
	make_random: function(txt) {
		if(txt.toLowerCase().includes('random')) {
			this.options.push({
				...
				value: capkpi.utils.get_random(16),
				onclick: function() {
					capkpi.msgprint(capkpi.utils.get_random(16), "Result");
				}
			})
		}
	}
