// capkpi.ui.Capture
// Author - Achilles Rasquinha <achilles@capkpi.com>
 ...
 *
 * @example
 * capkpi._.get_data_uri(video)
 * // returns "data:image/pngbase64,..."
 */
capkpi._.get_data_uri = element => {
...
/**
 * @description CapKPI's Capture object.
 *
 * @example
 * const capture = capkpi.ui.Capture()
 * capture.show()
 *
 ...
 */
capkpi.ui.Capture = class {
	constructor(options = {}) {
		this.options = capkpi.ui.Capture.OPTIONS;
	...
	set_options(options) {
		this.options = { ...capkpi.ui.Capture.OPTIONS, ...options };
	...
	build_dialog() {
		...
		me.$template = $(capkpi.ui.Capture.TEMPLATE);
	...
	show_for_desktop() {
		...
		this.render_stream()
			...
			.catch(err => {
				if (me.options.error) {
					capkpi.show_alert(capkpi.ui.Capture.ERR_MESSAGE, 3);
				}
	...
	setup_take_photo_action() {
		...
		this.dialog.set_primary_action(__('Take Photo'), () => {
			const data_url = capkpi._.get_data_uri(me.video);
...
capkpi.ui.Capture.OPTIONS = {
	title: __("Camera"),
	animate: false,
	error: false
};
capkpi.ui.Capture.ERR_MESSAGE = __('Unable to load camera.');
capkpi.ui.Capture.TEMPLATE = `
<div class="capkpi-capture">
	<div class="embed-responsive embed-responsive-16by9 fc-stream-container">
		<video class="fc-stream embed-responsive-item">${capkpi.ui.Capture.ERR_MESSAGE}</video>
	</div>
