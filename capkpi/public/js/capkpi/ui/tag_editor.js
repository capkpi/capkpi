capkpi.ui.TagEditor = Class.extend({
	init: function(opts) {
		/* docs:
		Arguments
	...
	setup_tags: function() {
		...
		this.tags = new capkpi.ui.Tags({
			parent: this.wrapper,
			placeholder: __("Add a tag ..."),
			onTagAdd: (tag) => {
				if(me.initialized && !me.refreshing) {
					return capkpi.call({
						method: "capkpi.desk.doctype.tag.tag.add_tag",
			...
			onTagRemove: (tag) => {
				if(!me.refreshing) {
					return capkpi.call({
						method: "capkpi.desk.doctype.tag.tag.remove_tag",
						args: me.get_args(tag),
						callback: function(r) {
	...
	setup_awesomplete: function() {
		...
		$input.on("input", function(e) {
			...
			capkpi.call({
				method: "capkpi.desk.doctype.tag.tag.get_tags",
				args:{
