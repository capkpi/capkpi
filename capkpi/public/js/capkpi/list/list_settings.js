export default class ListSettings {
	constructor({ listview, doctype, meta, settings }) {
		...
		capkpi.model.with_doctype("List View Settings", () => {
			this.make();
	...
	make() {
		...
		let list_view_settings = capkpi.get_meta("List View Settings");
		...
		me.dialog.set_primary_action(__('Save'), () => {
			...
			capkpi.call({
				method: "capkpi.desk.doctype.list_view_settings.list_view_settings.save_listview_settings",
				args: {
					doctype: me.doctype,
		...
		for (let idx in me.fields) {
			...
			fields += `
				<div class="control-input flex align-center form-control fields_order ${is_sortable}"
					...
					<div class="row">
						<div class="col-md-1">
							${capkpi.utils.icon("drag", "xs", "", "", "sortable-handle " + show_sortable_handle)}
						</div>
		...
		d.set_primary_action(__('Save'), () => {
			...
			for (let idx in values) {
				...
				if (me.fields.length === parseInt(me.dialog.get_values().total_fields)) {
					...
					let field = capkpi.meta.get_docfield(me.doctype, value);
					if (field) {
						me.fields.push({
							label: field.label,
	...
	reset_listview_fields(dialog) {
		...
		capkpi.xcall("capkpi.desk.doctype.list_view_settings.list_view_settings.get_default_listview_fields", {
			doctype: me.doctype
		}).then((fields) => {
	...
	set_list_view_fields(meta) {
		...
		meta.fields.forEach(field => {
			if (field.in_list_view && !in_list(capkpi.model.no_value_type, field.fieldtype) &&
	...
	set_subject_field(meta) {
		...
		if (meta.title_field) {
			let field = capkpi.meta.get_docfield(me.doctype, meta.title_field.trim());
	...
	set_status_field() {
		...
		if (capkpi.has_indicator(me.doctype)) {
			me.fields.push({
				type: "Status",
				label: "Status",
	...
	get_doctype_fields(meta, fields) {
		...
		meta.fields.forEach(field => {
			if (!in_list(capkpi.model.no_value_type, field.fieldtype)) {
