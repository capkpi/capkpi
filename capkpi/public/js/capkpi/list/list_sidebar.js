...
capkpi.views.ListSidebar = class ListSidebar {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	make() {
		var sidebar_content = capkpi.render_template("list_sidebar", { doctype: this.doctype });
	...
	setup_views() {
		...
		if (this.doctype === "Communication" && capkpi.boot.email_accounts.length) {
			this.sidebar.find('.list-link[data-view="Inbox"]').removeClass('hide');
			show_list_link = true;
		}

		if (capkpi.treeview_settings[this.doctype] || capkpi.get_meta(this.doctype).is_tree) {
			this.sidebar.find(".tree-link").removeClass("hide");
		}

		this.current_view = 'List';
		var route = capkpi.get_route();
		if (route.length > 2 && capkpi.views.view_modes.includes(route[2])) {
	...
	setup_reports() {
		...
		var reports = Object.values(capkpi.boot.user.all_reports).sort((a,b) => a.title.localeCompare(b.title)) || [];
	...
	setup_kanban_boards() {
		...
		capkpi.views.KanbanView.setup_dropdown_in_sidebar(this.doctype, $dropdown);
	}
	...
	setup_list_group_by() {
		this.list_group_by = new capkpi.views.ListGroupBy({
			doctype: this.doctype,
			sidebar: this,
			list_view: this.list_view,
			page: this.page
	...
	get_stats() {
		var me = this;
		capkpi.call({
			method: 'capkpi.desk.reportview.get_sidebar_stats',
			type: 'GET',
			args: {
				stats: me.stats,
				doctype: me.doctype,
			...
			callback: function(r) {
				...
				capkpi.utils.setup_search(stats_dropdown, '.stat-link', '.stat-label');
			}
		});
	}
	...
	render_stat(stats) {
		...
		let tag_list = $(capkpi.render_template("list_sidebar_stat", args)).on("click", ".stat-link", (e) => {
