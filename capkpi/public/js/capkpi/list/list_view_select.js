...
capkpi.views.ListViewSelect = class ListViewSelect {
	constructor(opts) {
		$.extend(this, opts);
	...
	set_current_view() {
		...
		const view_name = capkpi.utils.to_title_case(route[2] || "");
		if (route.length > 2 && capkpi.views.view_modes.includes(view_name)) {
	...
	setup_views() {
		const views = {
			...
			Calendar: {
				condition: capkpi.views.calendar[this.doctype],
				...
				current_view_handler: () => {
					...
					if (
						...
						default_action = {
							...
							action: () => capkpi.new_doc("Email Account")
						};
					}
			...
			Tree: {
				condition:
					capkpi.treeview_settings[this.doctype] ||
					capkpi.get_meta(this.doctype).is_tree,
			...
			Kanban: {
				...
				current_view_handler: () => {
					capkpi.views.KanbanView.get_kanbans(this.doctype).then(
		...
		capkpi.views.view_modes.forEach(view => {
	...
	setup_kanban_switcher(kanbans) {
		...
		this.page.add_custom_menu_item(
			...
			() => capkpi.views.KanbanView.show_kanban_dialog(this.doctype),
			true
		);
	}

	get_page_name() {
		return capkpi.utils.to_title_case(
			capkpi.get_route().slice(-1)[0] || ""
		);
	}
	...
	get_reports() {
		...
		var reports =
			Object.values(capkpi.boot.user.all_reports).sort((a, b) =>
	...
	setup_kanban_boards() {
			...
			capkpi.db.get_value(
				...
				(board) => {
					if (!$.isEmptyObject(board)) {
						capkpi.set_route("list", doctype, "kanban", board.name);
					} else {
						capkpi.views.KanbanView.show_kanban_dialog(doctype);
					}
				}
			);
		}
		...
		const last_opened_kanban =
			capkpi.model.user_settings[this.doctype]["Kanban"] &&
			capkpi.model.user_settings[this.doctype]["Kanban"].last_kanban_board;
		if (!last_opened_kanban) {
			fetch_kanban_board(this.doctype);
		} else {
			capkpi.db.exists("Kanban Board", last_opened_kanban).then((exists) => {
				if (exists) {
					capkpi.set_route("list", this.doctype, "kanban", last_opened_kanban);
				} else {
					fetch_kanban_board(this.doctype);
				}
			});
	...
	get_calendars() {
		...
		return capkpi.db
			...
			.then(result => {
				...
				if (capkpi.views.calendar[this.doctype]) {
	...
	get_email_accounts() {
		let accounts_to_add = [];
		let accounts = capkpi.boot.email_accounts;
	...
	slug() {
		return capkpi.router.slug(capkpi.router.doctype_layout || this.doctype);
	}
};
