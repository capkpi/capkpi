...
capkpi.views.BaseList = class BaseList {
	constructor(opts) {
	...
	setup_defaults() {
		...
		this.page_title = this.page_title || capkpi.router.doctype_layout || __(this.doctype);
		this.meta = capkpi.get_meta(this.doctype);
		this.settings = capkpi.listview_settings[this.doctype] || {};
		this.user_settings = capkpi.get_user_settings(this.doctype);

		this.start = 0;
		this.page_length = 20;
		this.data = [];
		this.method = "capkpi.desk.reportview.get";

		this.can_create = capkpi.model.can_create(this.doctype);
	...
	get_list_view_settings() {
		return capkpi
			.call("capkpi.desk.listview.get_list_settings", {
	...
	_add_field(fieldname, doctype) {
		...
		const is_valid_field =
			capkpi.model.std_fields_list.includes(fieldname) ||
			capkpi.meta.has_field(doctype, fieldname) ||
			fieldname === "_seen";
	...
	set_stats() {
		...
		this.workflow_state_fieldname = capkpi.workflow.get_state_fieldname(
			this.doctype
		);
		if (this.workflow_state_fieldname) {
			if (!capkpi.workflow.workflows[this.doctype]["override_status"]) {
	...
	setup_page() {
		...
		!this.hide_card_layout && this.page.main.addClass('capkpi-card');
	...
	setup_view_menu() {
		...
		if (capkpi.boot.desk_settings.view_switcher) {
			/* @preserve
			...
			this.views_list = new capkpi.views.ListViewSelect({
				doctype: this.doctype,
	...
	set_breadcrumbs() {
		capkpi.breadcrumbs.add(this.meta.module, this.doctype);
	}

	setup_side_bar() {
		if (this.hide_sidebar || !capkpi.boot.desk_settings.list_sidebar) return;
		this.list_sidebar = new capkpi.views.ListSidebar({
			doctype: this.doctype,
			stats: this.stats,
	...
	setup_list_wrapper() {
		this.$capkpi_list = $('<div class="capkpi-list">').appendTo(
			this.page.main
		);
	}
	...
	setup_result_area() {
		...
		this.$capkpi_list.append(this.$result);
	}
	...
	setup_no_result_area() {
		...
		`).hide();
		this.$capkpi_list.append(this.$no_result);
	}

	setup_freeze_area() {
		this.$freeze = $('<div class="freeze"></div>').hide();
		this.$capkpi_list.append(this.$freeze);
	}
	...
	setup_paging_area() {
		...
		this.$capkpi_list.append(this.$paging_area);
	...
	get_fields() {
		...
		return this.fields.map((f) =>
			capkpi.model.get_full_column_name(f[0], f[1])
		);
	}
	...
	get_group_by() {
		...
		if (name_field) {
			return capkpi.model.get_full_column_name(name_field[0], name_field[1]);
		}
		return null;
	}
	...
	refresh() {
		...
		return capkpi.call(this.get_call_args()).then((r) => {
			// render
			this.prepare_data(r);
	...
	prepare_data(r) {
		...
		data = !Array.isArray(data)
			? capkpi.utils.dict(data.keys, data.values)
			: data;
...
class FilterArea {
	...
	clear(refresh = true) {
		...
		return capkpi.run_serially(promises).then(() => {
			this.trigger_refresh = true;
		});
	}
		...
		fields = fields.concat(
			doctype_fields
				...
				.map((df) => {
					...
					let default_value =
						fieldtype === "Link"
							? capkpi.defaults.get_user_default(options)
							: null;
	...
	make_filter_list() {
		...
		this.filter_list = new capkpi.ui.FilterGroup({
...
capkpi.views.is_valid = (view_mode) =>
	capkpi.views.view_modes.includes(view_mode);
