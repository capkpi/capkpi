...
capkpi.provide("capkpi.web_form_list");

export default class WebFormList {
	constructor(opts) {
		Object.assign(this, opts);
		capkpi.web_form_list = this;
	...
	make_filters() {
		...
		capkpi.call('capkpi.website.doctype.web_form.web_form.get_web_form_filters', {
			...
			fields.forEach(field => {
				...
				let input = capkpi.ui.form.make_control({
					df: {
	...
	get_list_view_fields() {
		return capkpi
			.call({
				method:
					"capkpi.website.doctype.web_form.web_form.get_in_list_view_fields",
	...
	fetch_data() {
		return capkpi.call({
			method: "capkpi.www.list.get_list_data",
			args: {
	...
	more() {
		...
		this.fetch_data().then((res) => {
			if (res.message.length === 0) {
				capkpi.msgprint(__("No more items to display"));
			}
	...
	append_rows(row_data) {
		...
		row_data.forEach((data_item) => {
			...
			let row = new capkpi.ui.WebFormListRow({
				row: row_element,
				doc: data_item,
	...
	make_actions() {
		...
		capkpi.has_permission(this.doctype, "", "delete", () => {
	...
	delete_rows() {
		...
		capkpi
			.call({
				type: "POST",
				method:
					"capkpi.website.doctype.web_form.web_form.delete_multiple",
				args: {
					web_form_name: this.web_form_name,
...
capkpi.ui.WebFormListRow = class WebFormListRow {
	...
	make_row() {
		...
		this.columns.forEach(field => {
			let cell = this.row.insertCell();
			let formatter = capkpi.form.get_formatter(field.fieldtype);
