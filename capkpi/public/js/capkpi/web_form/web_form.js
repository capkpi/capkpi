...
capkpi.provide("capkpi.web_form");
import EventEmitterMixin from '../../capkpi/event_emitter';

export default class WebForm extends capkpi.ui.FieldGroup {
	constructor(opts) {
		super();
		Object.assign(this, opts);
		capkpi.web_form = this;
		capkpi.web_form.events = {};
		Object.assign(capkpi.web_form.events, EventEmitterMixin);
	...
	make() {
		...
		capkpi.init_client_script && capkpi.init_client_script();
		capkpi.web_form.events.trigger('after_load');
	...
	set_default_values() {
		let values = capkpi.utils.get_query_params();
		delete values.new;
	...
	setup_delete_button() {
		capkpi.has_permission(this.doc_type, "", "delete", () => {
			this.add_button_to_header(
				capkpi.utils.icon('delete'),
				"danger",
				() => this.delete()
			);
		});
	...
	is_next_section_empty(section) {
		...
		let visible_controls = _section.find(".capkpi-control:not(.hide-control)");
	...
	save() {
		...
		if (this.validate && !this.validate()) {
			capkpi.throw(__("Couldn't save, please check the data you have entered"), __("Validation Error"));
		}
		...
		// Save
		window.saving = true;
		capkpi.form_dirty = false;

		capkpi.call({
			type: "POST",
			method: "capkpi.website.doctype.web_form.web_form.accept",
			args: {
				data: this.doc,
				web_form: this.name,
			...
			callback: response => {
				...
				if (!response.exc) {
					...
					capkpi.web_form.events.trigger('after_save');
					...
					if (is_new && (response.message.attachment || response.message.file)) {
						capkpi.call({
							type: 'POST',
							method: "capkpi.handler.upload_file",
							args: {
	...
	delete() {
		capkpi.call({
			type: "POST",
			method: "capkpi.website.doctype.web_form.web_form.delete",
			args: {
	...
	handle_success(data) {
		...
		const success_dialog = new capkpi.ui.Dialog({
