...
capkpi.dom = {
	...
	get_unique_id: function() {
		const id = 'unique-' + capkpi.dom.id_count;
		capkpi.dom.id_count++;
		return id;
	},
	set_unique_id: function(ele) {
		...
		var id = 'unique-' + capkpi.dom.id_count;
		$ele.attr('id', id);
		capkpi.dom.id_count++;
		return id;
	},
	...
	add: function(parent, newtag, className, cs, innerHTML, onclick) {
		if(parent && parent.substr)parent = capkpi.dom.by_id(parent);
	...
	freeze: function(msg, css_class) {
		...
		capkpi.dom.freeze_count++;
	},
	unfreeze: function() {
		if(!capkpi.dom.freeze_count) return; // anything open?
		capkpi.dom.freeze_count--;
	...
	scroll_to_section(section_name) {
		setTimeout(() => {
			...
			if (section.length) {
				...
				capkpi.ui.scroll(section.parent().parent());
			}
		}, 200);
	},
...
capkpi.ellipsis = function(text, max) {
	if(!max) max = 20;
	text = cstr(text);
	if(text.length > max) {
...
capkpi.run_serially = function(tasks) {
...
capkpi.load_image = (src, onload, onerror, preprocess = () => {}) => {
...
capkpi.timeout = seconds => {
	return new Promise((resolve) => {
		setTimeout(() => resolve(), seconds * 1000);
	});
};

capkpi.scrub = function(text, spacer='_') {
	return text.replace(/ /g, spacer).toLowerCase();
};

capkpi.unscrub = function(txt) {
	return capkpi.model.unscrub(txt);
};

capkpi.get_data_pill = (label, target_id=null, remove_action=null, image=null) => {
...
capkpi.get_modal = function(title, content) {
				...
				<div class="modal-header">
					...
					<div class="modal-actions">
						<button class="btn btn-modal-minimize btn-link hide">
							${capkpi.utils.icon('collapse')}
						</button>
						<button class="btn btn-modal-close btn-link" data-dismiss="modal">
							${capkpi.utils.icon('close-alt', 'sm', 'close-alt')}
						</button>
					</div>
				</div>
...
capkpi.is_online = function() {
	if (capkpi.boot.developer_mode == 1) {
