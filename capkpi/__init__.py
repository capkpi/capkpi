...
"""
CapKPI - Low Code Open Source Framework in Python and JS

CapKPI, pronounced fra-pay, is a full stack, batteries-included, web
framework written in Python and Javascript with MariaDB as the database.
It is the framework which powers ERP. It is pretty generic and can
be used to build database driven apps.

Read the documentation: https://capkpiframework.com/docs
"""
import importlib
import inspect
import json
...
from capkpi.query_builder import get_query_builder, patch_query_execute
...
__version__ = "13.41.30"

__title__ = "CapKPI Framework"
...
def _(msg, lang=None, context=None) -> str:
	...
	"""
	from capkpi.translate import get_full_dict
...
def get_lang_dict(fortype, name=None):
	...
	from capkpi.translate import get_dict

	return get_dict(fortype, name)


def set_user_lang(user, user_language=None):
	"""Guess and set user language for the session. `capkpi.local.lang`"""
	from capkpi.translate import get_user_lang
...
if typing.TYPE_CHECKING:
	from capkpi.database.mariadb.database import MariaDBDatabase
	from capkpi.database.postgres.database import PostgresDatabase
	from capkpi.query_builder.builder import MariaDB, Postgres
...
def init(site, sites_path=None, new_site=False):
	"""Initialize capkpi for the current site. Reset thread locals `capkpi.local`"""
...
def connect(site=None, db_name=None, set_admin_as_user=True):
	...
	:param site: If site is given, calls `capkpi.init`.
	:param db_name: Optional. Will use from `site_config.json`.
	:param set_admin_as_user: Set Administrator as current user.
	"""
	from capkpi.database import get_db
...
def get_traceback():
	...
	from capkpi.utils import get_traceback
...
def throw(msg, exc=ValidationError, title=None, is_minimizable=None, wide=None, as_list=False):
	...
	:param msg: Message.
	:param exc: Exception class. Default `capkpi.ValidationError`"""
	msgprint(
		msg,
		raise_exception=exc,
		title=title,
...
def create_folder(path, with_init=False):
	...
	from capkpi.utils import touch_file
...
def get_roles(username=None):
	...
	return capkpi.permissions.get_roles(username or local.session.user)
...
def sendmail(
	...
	if as_markdown:
		from capkpi.utils import md_to_html
	...
	from capkpi.email import queue
...
def only_for(roles, message=False):
	"""Raise `capkpi.PermissionError` if the user does not have any of the given **Roles**.
...
def clear_cache(user=None, doctype=None):
	...
	if doctype:
		capkpi.cache_manager.clear_doctype_cache(doctype)
		reset_metadata_version()
	elif user:
		capkpi.cache_manager.clear_user_cache(user)
	else:  # everything
		from capkpi import translate

		capkpi.cache_manager.clear_user_cache()
		capkpi.cache_manager.clear_domain_cache()
...
def only_has_select_perm(doctype, user=None, ignore_permissions=False):
	...
	permissions = capkpi.permissions.get_role_permissions(doctype, user=user)
...
def has_permission(doctype=None, ptype="read", doc=None, user=None, verbose=False, throw=False):
	"""Raises `capkpi.PermissionError` if not permitted.
	...
	out = capkpi.permissions.has_permission(
		doctype, ptype, doc=doc, verbose=verbose, user=user, raise_exception=throw
	)
	if throw and not out:
		if doc:
			capkpi.throw(_("No permission for {0}").format(_(doc.doctype) + " " + doc.name))
		else:
			capkpi.throw(_("No permission for {0}").format(_(doctype)))
...
def new_doc(doctype, parent_doc=None, parentfield=None, as_dict=False):
	...
	from capkpi.model.create_new import get_new_doc

	return get_new_doc(doctype, parent_doc, parentfield, as_dict=as_dict)


def set_value(doctype, docname, fieldname, value=None):
	"""Set document value. Calls `capkpi.client.set_value`"""
	import capkpi.client

	return capkpi.client.set_value(doctype, docname, fieldname, value)
...
def get_doc(*args, **kwargs):
	"""Return a `capkpi.model.document.Document` object of the given type and name.
	...
	Examples:
	        ...
	        todo = capkpi.get_doc({"doctype":"ToDo", "description": "test"})
	        todo.insert()

	        # open an existing document
	        todo = capkpi.get_doc("ToDo", "TD0001")

	"""
	import capkpi.model.document

	doc = capkpi.model.document.get_doc(*args, **kwargs)
...
def get_single(doctype):
	"""Return a `capkpi.model.document.Document` object of the given Single doctype."""
	return get_doc(doctype, doctype)


def get_meta(doctype, cached=True):
	"""Get `capkpi.model.meta.Meta` instance of given doctype name."""
	import capkpi.model.meta

	return capkpi.model.meta.get_meta(doctype, cached=cached)


def get_meta_module(doctype):
	import capkpi.modules

	return capkpi.modules.load_doctype_module(doctype)
...
def delete_doc(
	...
	"""Delete a document. Calls `capkpi.model.delete_doc.delete_doc`.
	...
	import capkpi.model.delete_doc

	capkpi.model.delete_doc.delete_doc(
		doctype,
		name,
		force,
		ignore_doctypes,
...
def reload_doc(module, dt=None, dn=None, force=False, reset_permissions=False):
	...
	return capkpi.modules.reload_doc(module, dt, dn, force=force, reset_permissions=reset_permissions)
...
def rename_doc(*args, **kwargs):
	...
	Calls `capkpi.model.rename_doc.rename_doc`
	"""
...
def get_all_apps(with_internal_apps=True, sites_path=None):
	...
	if "capkpi" in apps:
		apps.remove("capkpi")
	apps.insert(0, "capkpi")

	return apps


def get_installed_apps(sort=False, capkpi_last=False):
	...
	if capkpi_last:
		if "capkpi" in installed:
			installed.remove("capkpi")
		installed.append("capkpi")
...
def get_hooks(hook=None, default=None, app_name=None):
	...
	def load_app_hooks(app_name=None):
		...
		for app in [app_name] if app_name else get_installed_apps(sort=True):
			app = "capkpi" if app == "webnotes" else app
			try:
...
def get_file_items(path, raise_not_found=False, ignore_empty_lines=True):
	...
	if content:
		content = capkpi.utils.strip(content)
...
def compare(val1, condition, val2):
	"""Compare two values using `capkpi.utils.compare`
	...
	return capkpi.utils.compare(val1, condition, val2)
...
def redirect_to_message(title, html, http_status_code=None, context=None, indicator_color=None):
	...
	Example Usage:
	        capkpi.redirect_to_message(_('Thank you'), "<div><p>You will receive an email at test@example.com</p></div>")
...
def build_match_conditions(doctype, as_condition=True):
	...
	return capkpi.desk.reportview.build_match_conditions(doctype, as_condition=as_condition)


def get_list(doctype, *args, **kwargs):
	"""List database query via `capkpi.model.db_query`. Will also check for permissions.
	...
	Example usage:
	        ...
	        # simple dict filter
	        capkpi.get_list("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        capkpi.get_list("ToDo", fields="*", filters = [["modified", ">", "2014-01-01"]])

	        # filter as a list of dicts
	        capkpi.get_list("ToDo", fields="*", filters = {"description": ("like", "test%")})
	"""
	import capkpi.model.db_query

	return capkpi.model.db_query.DatabaseQuery(doctype).execute(*args, **kwargs)


def get_all(doctype, *args, **kwargs):
	"""List database query via `capkpi.model.db_query`. Will **not** check for permissions.
	Parameters are same as `capkpi.get_list`
	...
	Example usage:
	        ...
	        # simple dict filter
	        capkpi.get_all("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        capkpi.get_all("ToDo", fields=["*"], filters = [["modified", ">", "2014-01-01"]])

	        # filter as a list of dicts
	        capkpi.get_all("ToDo", fields=["*"], filters = {"description": ("like", "test%")})
	"""
...
def get_test_records(doctype):
	...
	from capkpi.modules import get_doctype_module, get_module_path
...
def format_value(*args, **kwargs):
	...
	import capkpi.utils.formatters

	return capkpi.utils.formatters.format_value(*args, **kwargs)
...
def attach_print(
	...
	from capkpi.utils import scrub_urls
...
def publish_progress(*args, **kwargs):
	...
	"""
	import capkpi.realtime

	return capkpi.realtime.publish_progress(*args, **kwargs)
...
def publish_realtime(*args, **kwargs):
	...
	return capkpi.realtime.publish_realtime(*args, **kwargs)


def local_cache(namespace, key, generator, regenerate_if_none=False):
	"""A key value store for caching within a request

	:param namespace: capkpi.local.cache[namespace]
	:param key: capkpi.local.cache[namespace][key] used to retrieve value
...
def enqueue(*args, **kwargs):
	...
	"""
	import capkpi.utils.background_jobs

	return capkpi.utils.background_jobs.enqueue(*args, **kwargs)
...
def enqueue_doc(*args, **kwargs):
	...
	return capkpi.utils.background_jobs.enqueue_doc(*args, **kwargs)
...
def logger(
	...
	from capkpi.utils.logger import get_logger
...
def get_active_domains():
	from capkpi.core.doctype.domain_settings.domain_settings import get_active_domains
...
def get_version(doctype, name, limit=None, head=False, raise_err=True):
	...
	Example
	>>> capkpi.get_version('User', 'foobar@gmail.com')
	>>>
	[
	        {
	...
	if meta.track_changes:
		...
		from capkpi.utils import dictify, safe_json_loads, squashify
...
def parse_json(val):
	from capkpi.utils import parse_json
...
def mock(type, size=1, locale="en"):
	...
	from capkpi.utils import squashify

	return squashify(results)


def validate_and_sanitize_search_inputs(fn):
	from capkpi.desk.search import validate_and_sanitize_search_inputs as func
