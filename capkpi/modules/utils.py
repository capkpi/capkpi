...
def export_module_json(doc, is_standard, module):
	...
	if not capkpi.flags.in_import and getattr(capkpi.get_conf(), "developer_mode", 0) and is_standard:
		...
		path = os.path.join(
			capkpi.get_module_path(module), scrub(doc.doctype), scrub(doc.name), scrub(doc.name)
		)
...
def get_doc_module(module, doctype, name):
	...
	module_name = "{app}.{module}.{doctype}.{name}.{name}".format(
		app=capkpi.local.module_app[scrub(module)],
		doctype=scrub(doctype),
		module=scrub(module),
		name=scrub(name),
	)
	return capkpi.get_module(module_name)
...
def export_customizations(module, doctype, sync_on_migrate=0, with_permissions=0):
	...
	if not capkpi.get_conf().developer_mode:
	...
	def add(_doctype):
		custom["custom_fields"] += capkpi.get_all("Custom Field", fields="*", filters={"dt": _doctype})
		custom["property_setters"] += capkpi.get_all(
	...
	if custom["custom_fields"] or custom["property_setters"] or custom["custom_perms"]:
		...
		with open(path, "w") as f:
			f.write(capkpi.as_json(custom))

		capkpi.msgprint(_("Customizations for <b>{0}</b> exported to:<br>{1}").format(doctype, path))
...
def sync_customizations(app=None):
	...
	for app_name in apps:
		for module_name in capkpi.local.app_modules.get(app_name) or []:
			folder = capkpi.get_app_path(app_name, module_name, "custom")
...
def sync_customizations_for_doctype(data, folder):
	...
	def sync(key, custom_doctype, doctype_fieldname):
		...
		def sync_single_doctype(doc_type):
			...
			if custom_doctype != "Custom Field":
				...
				for d in data[key]:
					field = capkpi.db.get_value("Custom Field", {"dt": doc_type, "fieldname": d["fieldname"]})
					if not field:
		...
		for doc_type in doctypes:
			...
			if doc_type == doctype or not os.path.exists(
				os.path.join(folder, capkpi.scrub(doc_type) + ".json")
			):
	...
	if update_schema and not capkpi.db.get_value("DocType", doctype, "issingle"):
		capkpi.db.updatedb(doctype)


def scrub(txt):
	return capkpi.scrub(txt)
...
def get_module_path(module):
	...
	return capkpi.get_module_path(module)
...
def reload_doc(module, dt=None, dn=None, force=False, reset_permissions=False):
	from capkpi.modules.import_file import import_files
...
def export_doc(doctype, name, module=None):
	...
	from capkpi.modules.export_file import write_document_file

	print(doctype, name)

	if not module:
		module = capkpi.db.get_value("DocType", name, "module")
	write_document_file(capkpi.get_doc(doctype, name), module)
...
def get_doctype_module(doctype):
	...
	def make_modules_dict():
		return dict(capkpi.db.sql("select name, module from tabDocType"))

	return capkpi.cache().get_value("doctype_modules", make_modules_dict)[doctype]
...
def load_doctype_module(doctype, module=None, prefix="", suffix=""):
	...
	try:
		if key not in doctype_python_modules:
			doctype_python_modules[key] = capkpi.get_module(module_name)
...
def get_module_app(module):
	return capkpi.local.module_app[scrub(module)]


def get_app_publisher(module):
	app = capkpi.local.module_app[scrub(module)]
	if not app:
		capkpi.throw(_("App not found"))
	app_publisher = capkpi.get_hooks(hook="app_publisher", app_name=app)[0]
...
def make_boilerplate(template, doc, opts=None):
	...
	if not os.path.exists(target_file_path):
		...
		base_class = "Document"
		base_class_import = "from capkpi.model.document import Document"
		if doc.get("is_tree"):
			base_class = "NestedSet"
			base_class_import = "from capkpi.utils.nestedset import NestedSet"
	...
	def get_stats(self, args):
		...
		with open(target_file_path, "w") as target:
			with open(
				...
				target.write(
					capkpi.as_unicode(
						capkpi.utils.cstr(source.read()).format(
							app_publisher=app_publisher,
							year=capkpi.utils.nowdate()[:4],
