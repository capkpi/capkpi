...
const setup_socket_io = () => {
	const assets = [
		"/assets/capkpi/js/lib/socket.io.min.js",
		"/assets/capkpi/js/capkpi/socketio_client.js"
	];

	capkpi.require(assets, () => {
		if (window.dev_server) {
			capkpi.boot.socketio_port = "9000";
		}
		capkpi.socketio.init(9000);
		capkpi.socketio.socket.on("publish_message", (data) => {
...
const publish_message = (data) => {
	...
	if ($(`.discussion-on-page[data-topic=${data.topic_info.name}]`).length) {
		...
		if (data.topic_info.owner == capkpi.session.user) {
			$(".discussion-on-page").collapse();
			$(".sidebar-topic").first().click();
		}
	} else if (data.topic_info.owner == capkpi.session.user) {
		post_message_cleanup();
...
const submit_discussion = (e) => {
	...
	if (reply) {
		...
		capkpi.call({
			method: "capkpi.website.doctype.discussion_topic.discussion_topic.submit_discussion",
			args: {
...
const get_color_from_palette = (element) => {
	const palette = capkpi.get_palette(element.attr("title"));
