...
def add_comment(comment, comment_email, comment_by, reference_doctype, reference_name, route):
	doc = capkpi.get_doc(reference_doctype, reference_name)

	if capkpi.session.user == "Guest" and doc.doctype not in ["Blog Post", "Web Page"]:
		return

	if not comment.strip():
		capkpi.msgprint(_("The comment cannot be empty"))
		return False
	...
	if url_regex.search(comment) or email_regex.search(comment):
		capkpi.msgprint(_("Comments cannot have links or email addresses"))
		return False

	comments_count = capkpi.db.count(
		"Comment",
		{
	...
	if comments_count > 20:
		capkpi.msgprint(_("Hourly comment limit reached for: {0}").format(capkpi.bold(comment_email)))
		return False
	...
	content = (
		...
		+ "<p><a href='{0}/app/Form/Comment/{1}' style='font-size: 80%'>{2}</a></p>".format(
			capkpi.utils.get_request_site_address(), comment.name, _("View Comment")
		)
	)

	# notify creator
	capkpi.sendmail(
		recipients=capkpi.db.get_value("User", doc.owner, "email") or doc.owner,
	...
	template = capkpi.get_template("templates/includes/comments/comment.html")
