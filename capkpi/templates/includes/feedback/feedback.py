...
from capkpi.website.doctype.blog_settings.blog_settings import get_feedback_limit
...
def add_feedback(reference_doctype, reference_name, rating, feedback):
	...
	doc = capkpi.new_doc("Feedback")
	doc.reference_doctype = reference_doctype
	doc.reference_name = reference_name
	doc.rating = rating
	doc.feedback = feedback
	doc.ip_address = capkpi.local.request_ip
...
def update_feedback(reference_doctype, reference_name, rating, feedback):
	...
	d = capkpi.get_all("Feedback", filters=filters, limit=1)
	doc = capkpi.get_doc("Feedback", d[0].name)
	doc.rating = rating
	doc.feedback = feedback
	doc.save(ignore_permissions=True)
...
def send_mail(feedback, subject):
	doc = capkpi.get_doc(feedback.reference_doctype, feedback.reference_name)

	message = "<p>{0} ({1})</p>".format(
		feedback.feedback, feedback.rating
	) + "<p><a href='{0}/app/feedback/{1}' style='font-size: 80%'>{2}</a></p>".format(
		capkpi.utils.get_request_site_address(), feedback.name, _("View Feedback")
	)
