function setup_list_filters() {
	...
	capkpi.call('capkpi.website.doctype.web_form.web_form.get_form_data', {
	...
	.then((r) => {
		...
		web_form_filters = web_form.web_form_fields.filter(df => {
			...
			const value = capkpi.utils.get_query_params()[df.fieldname];
			const f = capkpi.ui.form.make_control({
				df: {
					...
					is_filter: true,
					change: (e) => {
						const query_params = Object.assign(capkpi.utils.get_query_params(), {
							[df.fieldname]: f.get_value()
						})
						const query_string = capkpi.utils.make_query_string(query_params);
...
capkpi.ready(setup_list_filters);
