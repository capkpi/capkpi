...
def apply():
	rate_limit = capkpi.conf.rate_limit
	if rate_limit:
		capkpi.local.rate_limiter = RateLimiter(rate_limit["limit"], rate_limit["window"])
		capkpi.local.rate_limiter.apply()


def update():
	if hasattr(capkpi.local, "rate_limiter"):
		capkpi.local.rate_limiter.update()


def respond():
	if hasattr(capkpi.local, "rate_limiter"):
		return capkpi.local.rate_limiter.respond()
...
class RateLimiter:
	def __init__(self, limit, window):
		...
		timestamp = int(capkpi.utils.now_datetime().timestamp())

		self.window_number, self.spent = divmod(timestamp, self.window)
		self.key = capkpi.cache().make_key(f"rate-limit-counter-{self.window_number}")
		self.counter = cint(capkpi.cache().get(self.key))
	...
	def reject(self):
		raise capkpi.TooManyRequestsError

	def update(self):
		self.end = datetime.utcnow()
		self.duration = int((self.end - self.start).total_seconds() * 1000000)

		pipeline = capkpi.cache().pipeline()
	...
	def ratelimit_decorator(fun):
		...
		def wrapper(*args, **kwargs):
			...
			if (
				methods != "ALL"
				and capkpi.request
				and capkpi.request.method
				and capkpi.request.method.upper() not in methods
			):
				return capkpi.call(fun, **capkpi.form_dict or kwargs)

			_limit = limit() if callable(limit) else limit

			ip = capkpi.local.request_ip if ip_based is True else None

			user_key = capkpi.form_dict[key] if key else None
			...
			if not identity:
				capkpi.throw(_("Either key or IP flag is required."))

			cache_key = f"rl:{capkpi.form_dict.cmd}:{identity}"

			value = capkpi.cache().get(cache_key) or 0
			if not value:
				capkpi.cache().setex(cache_key, seconds, 0)

			value = capkpi.cache().incrby(cache_key, 1)
			if value > _limit:
				capkpi.throw(
