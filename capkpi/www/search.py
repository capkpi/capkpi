...
from capkpi.utils.global_search import web_search
...
def get_context(context):
	...
	if capkpi.form_dict.q:
		...
		context.update(get_search_results(query, capkpi.utils.sanitize_html(capkpi.form_dict.scope)))
	else:
...
def get_search_results(text, scope=None, start=0, as_html=False):
	...
	if as_html:
		out.results = capkpi.render_template("templates/includes/search_result.html", out)
