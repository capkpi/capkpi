...
from capkpi.model.document import Document, get_controller
from capkpi.utils import cint, quoted
from capkpi.website.render import resolve_path
...
def get_context(context, **dict_params):
	...
	capkpi.local.form_dict.update(dict_params)
	doctype = capkpi.local.form_dict.doctype
	context.parents = [{"route": "me", "title": _("My Account")}]
	context.meta = capkpi.get_meta(doctype)
	context.update(get_list_context(context, doctype) or {})
	context.doctype = doctype
	context.txt = capkpi.local.form_dict.txt
	context.update(get(**capkpi.local.form_dict))
...
def get(doctype, txt=None, limit_start=0, limit=20, pathname=None, **kwargs):
	...
	list_context = capkpi.flags.list_context
	...
	for doc in raw_result:
		doc.doctype = doctype
		new_context = capkpi._dict(doc=doc, meta=meta, list_view_fields=list_view_fields)

		if not list_context.get_list and not isinstance(new_context.doc, Document):
			new_context.doc = capkpi.get_doc(doc.doctype, doc.name)
			new_context.update(new_context.doc.as_dict())

		if not capkpi.flags.in_test:
			pathname = pathname or capkpi.local.request.path
			new_context["pathname"] = pathname.strip("/ ")
		new_context.update(list_context)
		set_route(new_context)
		rendered_row = capkpi.render_template(row_template, new_context, is_path=True)
...
def get_list_data(
	...
	if not txt and capkpi.form_dict.search:
		txt = capkpi.form_dict.search
		del capkpi.form_dict["search"]

	controller = get_controller(doctype)
	meta = capkpi.get_meta(doctype)

	filters = prepare_filters(doctype, controller, kwargs)
	list_context = get_list_context(capkpi._dict(), doctype, web_form_name)
	...
	capkpi.flags.list_context = list_context
...
def prepare_filters(doctype, controller, kwargs):
	...
	if filters.pathname:
		...
		for key, val in capkpi.local.form_dict.items():
...
def get_list_context(context, doctype, web_form_name=None):
	...
	list_context = context or capkpi._dict()
	meta = capkpi.get_meta(doctype)
	...
	def update_context_from_module(module, list_context):
		...
		if hasattr(module, "get_list_context"):
			out = capkpi._dict(module.get_list_context(list_context) or {})
			if out:
				list_context = out
		return list_context
	...
	if meta.custom and web_form_name:
		...
		if webform_list_contexts:
			out = capkpi._dict(capkpi.get_attr(webform_list_contexts[0])(meta.module) or {})
			if out:
				list_context = out
