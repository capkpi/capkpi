...
from capkpi.core.doctype.document_share_key.document_share_key import is_expired
from capkpi.modules import get_doc_path
from capkpi.utils import cint, sanitize_html, strip_html
from capkpi.utils.jinja import is_rtl
...
def get_context(context):
	...
	if not ((capkpi.form_dict.doctype and capkpi.form_dict.name) or capkpi.form_dict.doc):
		return {
			"body": sanitize_html(
				"""<h1>Error</h1>
				<p>Parameters doctype and name required</p>
				<pre>%s</pre>"""
				% repr(capkpi.form_dict)
			)
		}

	if capkpi.form_dict.doc:
		doc = capkpi.form_dict.doc
	else:
		doc = capkpi.get_doc(capkpi.form_dict.doctype, capkpi.form_dict.name)

	settings = capkpi.parse_json(capkpi.form_dict.settings)

	letterhead = capkpi.form_dict.letterhead or None
	...
	make_access_log(
		doctype=capkpi.form_dict.doctype, document=capkpi.form_dict.name, file_type="PDF", method="Print"
	)
	...
	try:
		body = get_rendered_template(
			...
			meta=meta,
			trigger_print=capkpi.form_dict.trigger_print,
			no_letterhead=capkpi.form_dict.no_letterhead,
			letterhead=letterhead,
			settings=settings,
		)
		print_style = get_print_style(capkpi.form_dict.style, print_format)
	except capkpi.exceptions.LinkExpired:
		body = capkpi.get_template("templates/print_formats/print_key_expired.html").render({})
		context.http_status_code = 410
		is_invalid_print = True
	except capkpi.exceptions.InvalidKeyError:
		body = capkpi.get_template("templates/print_formats/print_key_invalid.html").render({})
	...
	return {
		...
		"comment": capkpi.session.user,
		"title": capkpi.utils.strip_html(doc.get_title() or doc.name),
		"lang": capkpi.local.lang,
		"layout_direction": "rtl" if is_rtl() else "ltr",
		"is_invalid_print": is_invalid_print,
		"doctype": capkpi.form_dict.doctype,
		"name": capkpi.form_dict.name,
		"key": capkpi.form_dict.get("key"),
	}
...
def get_print_format_doc(print_format_name, meta):
	...
	if not print_format_name:
		print_format_name = capkpi.form_dict.format or meta.default_print_format or "Standard"

	if print_format_name == "Standard":
		return None
	else:
		try:
			return capkpi.get_doc("Print Format", print_format_name)
...
def get_rendered_template(
	...
	print_settings = capkpi.get_single("Print Settings").as_dict()
	...
	if not capkpi.flags.ignore_print_permissions:
		validate_print_permission(doc)

	if doc.meta.is_submittable:
		if doc.docstatus == 0 and not cint(print_settings.allow_print_for_draft):
			capkpi.throw(_("Not allowed to print draft documents"), capkpi.PermissionError)

		if doc.docstatus == 2 and not cint(print_settings.allow_print_for_cancelled):
			capkpi.throw(_("Not allowed to print cancelled documents"), capkpi.PermissionError)
	...
	letter_head = capkpi._dict(get_letter_head(doc, no_letterhead, letterhead) or {})

	if letter_head.content:
		letter_head.content = capkpi.utils.jinja.render_template(
			letter_head.content, {"doc": doc.as_dict()}
		)

	if letter_head.footer:
		letter_head.footer = capkpi.utils.jinja.render_template(
	...
	for field in meta.fields:
		if field.fieldtype == "Text Editor":
			...
			if value and "<!-- markdown -->" in value:
				doc.set(field.fieldname, capkpi.utils.md_to_html(value))
...
def get_html_and_style(
	...
	if isinstance(doc, string_types) and isinstance(name, string_types):
		doc = capkpi.get_doc(doc, name)

	if isinstance(doc, string_types):
		doc = capkpi.get_doc(json.loads(doc))

	print_format = get_print_format_doc(print_format, meta=meta or capkpi.get_meta(doc.doctype))
	...
	try:
		html = get_rendered_template(
			...
			settings=capkpi.parse_json(settings),
		)
	except capkpi.TemplateNotFoundError:
...
def get_rendered_raw_commands(doc, name=None, print_format=None, meta=None, lang=None):
	...
	if not print_format or (print_format and not print_format.raw_printing):
		capkpi.throw(
			_("{0} is not a raw printing format.").format(print_format), capkpi.TemplateNotFoundError
		)
...
def validate_print_permission(doc):
	for ptype in ("read", "print"):
		if capkpi.has_permission(doc.doctype, ptype, doc) or capkpi.has_website_permission(doc):
			return
	...
	if key:
		...
		raise capkpi.PermissionError(_("You do not have permission to view this document"))


def validate_key(key, doc):
	try:
		document_key_expiry = capkpi.get_cached_value(
			"Document Share Key",
			{"reference_doctype": doc.doctype, "reference_docname": doc.name, "key": key},
			"expires_on",
		)
		if is_expired(document_key_expiry):
			raise capkpi.exceptions.LinkExpired
		else:
			return
	except capkpi.DoesNotExistError:
		capkpi.clear_last_message()
	...
	if capkpi.get_system_settings("allow_older_web_view_links") and key == doc.get_signature():
		return

	raise capkpi.exceptions.InvalidKeyError
...
def get_letter_head(doc, no_letterhead, letterhead=None):
	if no_letterhead:
		return {}
	if letterhead:
		return capkpi.db.get_value("Letter Head", letterhead, ["content", "footer"], as_dict=True)
	if doc.get("letter_head"):
		return capkpi.db.get_value("Letter Head", doc.letter_head, ["content", "footer"], as_dict=True)
	else:
		return (
			capkpi.db.get_value("Letter Head", {"is_default": 1}, ["content", "footer"], as_dict=True) or {}
		)
...
def get_print_format(doctype, print_format):
	if print_format.disabled:
		capkpi.throw(
			_("Print Format {0} is disabled").format(print_format.name), capkpi.DoesNotExistError
		)

	# server, find template
	path = os.path.join(
		get_doc_path(
			capkpi.db.get_value("DocType", doctype, "module"), "Print Format", print_format.name
		),
		capkpi.scrub(print_format.name) + ".html",
	)
	...
	if os.path.exists(path):
		...
		capkpi.throw(_("No template found at path: {0}").format(path), capkpi.TemplateNotFoundError)
...
def make_layout(doc, meta, format_data=None):
	...
	for df in format_data or meta.fields:
		...
		if df.fieldtype == "Signature" and not doc.get(df.fieldname):
			placeholder_image = "/assets/capkpi/images/signature-placeholder.png"
...
def get_print_style(style=None, print_format=None, for_legacy=False):
	print_settings = capkpi.get_doc("Print Settings")
	...
	css = capkpi.get_template("templates/styles/standard.css").render(context)

	if style and capkpi.db.exists("Print Style", style):
		css = css + "\n" + capkpi.db.get_value("Print Style", style, "css")
...
def get_visible_columns(data, table_meta, df):
	...
	columns = []
	doc = data[0] or capkpi.new_doc(df.options)
